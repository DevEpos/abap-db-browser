CLASS zcl_dbbr_sql_query_exec DEFINITION
  ...
  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Is called after asynchronous query finishes</p>
    CLASS-EVENTS query_finished
                   EXPORTING
    ...
    "! <p class="shorttext synchronized">Execute the entered query asynchronoulsy</p>
    CLASS-METHODS execute_query_async
      IMPORTING
        io_query              TYPE REF TO zcl_dbbr_sql_query
        iv_row_count          TYPE i         DEFAULT 100
        if_count_only         TYPE abap_bool OPTIONAL
    ...
    "! <p class="shorttext synchronized">Retrieves single column value from result</p>
  ...
  PROTECTED SECTION.
    CLASS-METHODS raise_query_finished
      IMPORTING
...
CLASS zcl_dbbr_sql_query_exec IMPLEMENTATION.
  METHOD execute_query.
    ...
    lo_executor->execute_query( IMPORTING et_data_info      = et_data_info
  ...
  METHOD execute_query_async.
    DATA(lo_executor) = NEW lcl_query_async_executor( io_query              = io_query
                                                      iv_row_count          = iv_row_count
                                                      if_count_only         = if_count_only
                                                      if_show_progress_text = if_show_progress_text ).
  ...
  METHOD get_single_value_from_result.
    " TODO: parameter IT_RESULT_INFO is never used (ABAP cleaner)

    FIELD-SYMBOLS <lt_result> TYPE table.
    FIELD-SYMBOLS <la_result_line> TYPE any.
