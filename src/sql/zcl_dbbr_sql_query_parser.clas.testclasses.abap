...
CLASS ltcl_unit_test DEFINITION FINAL
  ...
  PRIVATE SECTION.
    ...
    METHODS setup.
    METHODS test_normal_select  FOR TESTING RAISING cx_static_check.
    METHODS test_with_select    FOR TESTING RAISING cx_static_check.
    METHODS test_join           FOR TESTING RAISING cx_static_check.
    METHODS test_join2          FOR TESTING RAISING cx_static_check.
    METHODS test_union          FOR TESTING RAISING cx_static_check.
    METHODS test_sub_query      FOR TESTING RAISING cx_static_check.
    METHODS test_params         FOR TESTING RAISING cx_static_check.
    METHODS test_no_terminator  FOR TESTING RAISING cx_static_check.
    METHODS test_is_count_query FOR TESTING RAISING cx_static_check.
...
CLASS ltcl_unit_test IMPLEMENTATION.
  ...
  METHOD test_with_select.
    ...
    lt_query = VALUE #( ( |DATA: carrid type spfli-carrid.| )
                        ( )
                        ( |WITH | )
                        ( |    +cities AS (| )
    ...
    TRY.
        mo_cut->parse( ).
      CATCH zcx_dbbr_application_exc INTO DATA(lx_error). " TODO: variable is assigned but only used in commented-out code (ABAP cleaner)
    ENDTRY.
...
*    ).
  ...
  METHOD test_union.
    ...
    lt_query = VALUE #( ( |SELECT costcenter AS entity, CAST( 'Cost Center' AS CHAR ) AS type| )
                        ( |  FROM /dry/proccstctr| )
                        ( |  UNION| )
  ...
  METHOD test_params.
    ...
    lt_query = VALUE #( ( |DATA: p_plant value '1002' type werks_d,| )
  ...
  METHOD test_no_terminator.
    ...
    lt_query = VALUE #( ( |SELECT matnr, mtart from mara| ) ).
  ...
  METHOD test_is_count_query.
    ...
    TRY.
        ...
        cl_abap_unit_assert=>assert_true( act = lo_query->ms_data-is_single_result_query
