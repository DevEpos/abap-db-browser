...
CLASS lcl_text_field_reader_base IMPLEMENTATION.
  ...
  METHOD determine_text_fields.
    ...
    IF     mt_dtel_with_shlp          IS INITIAL
       AND mt_field_with_shlp         IS INITIAL
       AND mt_field_with_fix_val_shlp IS INITIAL.
      RETURN.
    ENDIF.
    ...
    LOOP AT mt_fields ASSIGNING FIELD-SYMBOL(<ls_field>).
      ...
      IF ls_text_field-selection_type IS INITIAL.
        ...
        IF lr_dtel_shlp IS NOT INITIAL.
          ...
          ELSEIF     lr_dtel_shlp->shlpname IS NOT INITIAL
                 AND lr_dtel_shlp->texttab  IS NOT INITIAL.
            ls_text_field-selection_type = zif_dbbr_c_text_selection_type=>text_table.
      ...
      IF     ls_text_field-selection_type IS NOT INITIAL
         AND ls_text_field-key_field      IS NOT INITIAL
         AND (    (     ls_text_field-text_field     IS NOT INITIAL
                    AND ls_text_field-language_field IS NOT INITIAL )
        ...
        ct_addtext = VALUE #( BASE ct_addtext
                              ( ls_text_field ) ).
      ENDIF.
  ...
  METHOD determine_f_w_dtel_shlp.
    ...
    DELETE lt_f_w_dtel_shlp WHERE     texttab         IS INITIAL
                                  AND dtel_text_table IS INITIAL.
  ...
  METHOD determine_text_table_fields.
    ...
    lt_tablename_range = VALUE #( ( LINES OF VALUE #( FOR <dtel_shlp> IN mt_dtel_with_shlp
                                                      sign   = 'I'
                                                      option = 'EQ'
                                                      ( low = <dtel_shlp>-texttab  )
                                                      ( low = <dtel_shlp>-dtel_text_table ) ) )
...
CLASS lcl_cds_text_field_reader IMPLEMENTATION.
  ...
  METHOD determine_f_w_fixed_val_shlp.
    SELECT fieldname FROM dd03nd AS field
      WHERE strucobjn  = @mv_entity
...
CLASS lcl_table_text_field_reader IMPLEMENTATION.
  ...
  METHOD determine_f_w_fixed_val_shlp.
    SELECT fieldname FROM dd03l AS field
      WHERE tabname    = @mv_entity
        AND shlporigin = @c_shlporigin-fixed_values
        AND as4local   = 'A'
      INTO CORRESPONDING FIELDS OF TABLE @mt_field_with_fix_val_shlp.
