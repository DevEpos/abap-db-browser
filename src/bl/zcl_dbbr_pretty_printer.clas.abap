...
CLASS zcl_dbbr_pretty_printer IMPLEMENTATION.
  METHOD format_source.
    ...
    DATA lf_indentation_wrong TYPE i.

    DATA(lr_settings) = COND #( WHEN ir_settings IS NOT INITIAL
                                THEN ir_settings
                                ELSE NEW cl_pretty_printer_wb_settings( ) ).

    CALL FUNCTION 'PRETTY_PRINTER'
      EXPORTING  inctoo                  = abap_false
                 settings                = lr_settings
      IMPORTING  indentation_maybe_wrong = lf_indentation_wrong
      TABLES     otext                   = ct_source
                 ntext                   = lt_buffer
      EXCEPTIONS OTHERS                  = 1.
