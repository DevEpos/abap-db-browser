CLASS zcl_dbbr_f4_helper DEFINITION
  ...
  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Retrieve infos of Value help</p>
    "!
    ...
    "! <p class="shorttext synchronized">Get value help information for cds view field</p>
    "!
    ...
    "! <p class="shorttext synchronized">Call built in selection field value help</p>
    "!
    ...
    "! <p class="shorttext synchronized">Call built-in value help with multiple selection</p>
    "!
    ...
    "! <p class="shorttext synchronized">Call built-in value help</p>
    "!
    ...
    "! <p class="shorttext synchronized">Call value help for selection field</p>
    ...
    "! <p class="shorttext synchronized">Call value help with internal table</p>
    "!
    ...
    "! <p class="shorttext synchronized">Call value help for specific ddic value help definition</p>
    "!
    ...
    "! <p class="shorttext synchronized">Call value help for table field</p>
    "!
...
CLASS zcl_dbbr_f4_helper IMPLEMENTATION.
  ...
  METHOD get_f4_infos.
    ...
    CALL FUNCTION 'F4IF_DETERMINE_SEARCHHELP'
      ...
      IMPORTING  shlp              = ls_search_help_info
    ...
    TRY.
        ...
        ELSEIF     ls_f4_infos-type = zif_dbbr_c_global=>c_searchhelp_types-search_help
               AND (    ls_f4_infos-sel_method_type = zif_dbbr_c_sh_selmethod_type=>table_selection
                     OR ls_f4_infos-sel_method_type = zif_dbbr_c_sh_selmethod_type=>view_selection
                     OR ls_f4_infos-sel_method_type = zif_dbbr_c_sh_selmethod_type=>with_text_table_selection ).

          DATA(lv_sel_table) = COND #( WHEN ls_f4_infos-text_table IS NOT INITIAL
                                       THEN ls_f4_infos-text_table
                                       ELSE ls_f4_infos-sel_method ).

          DATA(lt_output_fields) = VALUE zdbbr_sh_output_field_itab( FOR field_descr IN ls_search_help_info-fielddescr
                                                                     WHERE (     fieldname <> ls_f4_infos-key_field
                                                                             AND tabname    = lv_sel_table )
          ...
          IF ls_f4_infos-sel_method_type = zif_dbbr_c_sh_selmethod_type=>with_text_table_selection.
            ...
            ELSE.
              zcl_sat_ddic_repo_access=>get_table_field_infos( EXPORTING iv_tablename    = ls_f4_infos-text_table
...
*&---------------------------------------------------------------------*
  ...
  METHOD call_built_in_f4_multi.
    " TODO: parameter IV_TABLENAME is never used (ABAP cleaner)
...
*&---------------------------------------------------------------------*
    ...
    LOOP AT lt_return_tab ASSIGNING FIELD-SYMBOL(<ls_return_value>) WHERE    retfield  = ls_f4_dummy
      ...
      IF     if_do_not_convert_alpha  = abap_false
         AND cs_selfield-fieldname   <> iv_fieldname.
      ...
      IF     <lv_selvalue> <> space
        ...
        IF cs_selfield-is_parameter = abap_true.
          zcl_sat_data_converter=>convert_values_to_int_format( EXPORTING iv_rollname = cs_selfield-rollname
        ...
        ELSE.
          zcl_sat_data_converter=>convert_selopt_to_int_format( EXPORTING iv_tabname   = cs_selfield-tabname
      ...
      ENDIF.
      iv_current_line = iv_current_line + 1.
    ENDLOOP.
  ENDMETHOD.
...
*&---------------------------------------------------------------------*
  ...
  METHOD call_f4_with_search_help.
    " TODO: parameter IV_MAX_RECORDS is never used (ABAP cleaner)

    " TODO: variable is assigned but never used (ABAP cleaner)
    DATA lv_user_reset TYPE char1.

    CALL FUNCTION 'F4IF_FIELD_VALUE_REQUEST'
      EXPORTING  tabname           = space    " Table/structure name from Dictionary
...
*                 dynpprog          = 'SAPLZUITB_GUI_TEMPLATE'
      IMPORTING  user_reset        = lv_user_reset
      TABLES     return_tab        = rt_chosen_value
      EXCEPTIONS field_not_found   = 1
                 no_help_for_field = 2
                 inconsistent_help = 3
                 no_values_found   = 4
...
*&---------------------------------------------------------------------*
    ...
    CALL FUNCTION 'F4IF_INT_TABLE_VALUE_REQUEST'
      EXPORTING  retfield        = iv_return_field_name
                 window_title    = iv_f4_window_title
                 value_org       = 'S'
                 display         = 'F'
      TABLES     value_tab       = it_table_search
  ...
  METHOD call_f4.
    ...
    IF cs_selfield-has_cust_f4_help = abap_false.
      ...
      call_built_in_f4( EXPORTING iv_tablename = lv_tabname
                                  iv_fieldname = lv_fieldname
                        CHANGING  cv_value     = cs_selfield-low ).
    ELSE. " Call custom search help
      " .... 1) read f4 help definition
      io_custom_f4_map->read_custom_f4_definition( EXPORTING iv_tablename             = cs_selfield-tabname
                                                             iv_fieldname             = cs_selfield-fieldname
                                                   ...
                                                   IMPORTING et_custom_f4_definitions = DATA(lt_f4_definition) ).
      IF lt_f4_definition IS INITIAL.
        MESSAGE `Value help in ZDBBR_F4H is not valid` TYPE 'S'.
        RETURN.
      ENDIF.

      zcl_dbbr_custom_f4_helper=>call_custom_f4( EXPORTING it_f4_definition = lt_f4_definition
                                                           if_low           = abap_true
  ...
  METHOD call_selfield_f4.
    ...
    IF cs_selfield-has_cust_f4_help = abap_false.
      ...
      zcl_dbbr_f4_helper=>call_built_in_selfield_f4( EXPORTING if_low                  = if_low
                                                               if_do_not_convert_alpha = abap_true
                                                               iv_selfield_name        = iv_selfield_name
                                                               iv_selvalue             = lv_selvalue
                                                               iv_tablename            = lv_tabname
                                                               iv_fieldname            = lv_fieldname
                                                     CHANGING  cs_selfields            = cs_selfield ).
    ELSE. " Call custom search help
      " 1) read f4 help definition
      ir_custom_f4_map->read_custom_f4_definition( EXPORTING iv_tablename             = cs_selfield-tabname
                                                             iv_fieldname             = cs_selfield-fieldname
                                                             iv_rollname              = cs_selfield-rollname
                                                             is_built_in_type         = VALUE #(
                                                                 datatype = cs_selfield-datatype
      ...
      zcl_dbbr_custom_f4_helper=>call_custom_f4( EXPORTING iv_selfield_name = iv_selfield_name
                                                           iv_selvalue      = lv_selvalue
...
*&---------------------------------------------------------------------*
    ...
    IF     iv_repid IS NOT INITIAL
       AND ( iv_dynpname_fieldname IS NOT INITIAL OR iv_dynpname_tablename IS NOT INITIAL ).
      ...
      IF iv_dynpname_tablename IS NOT INITIAL.
        lt_fieldname_selopt = VALUE #( BASE lt_fieldname_selopt
                                       ( sign = 'I' option = 'EQ' low = iv_dynpname_tablename ) ).
      ENDIF.

      lr_dynpfield_mng->read_values( it_fieldname_selopt = lt_fieldname_selopt
                                     iv_step_loop_index  = iv_current_line ).
