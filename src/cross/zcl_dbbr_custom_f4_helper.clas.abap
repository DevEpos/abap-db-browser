...
CLASS zcl_dbbr_custom_f4_helper IMPLEMENTATION.
  METHOD build_f4_range_table.
    ...
    DATA(lv_option) = COND ddoption( WHEN contains( val = iv_entered_value
                                                    sub = '*' )
                                     THEN 'CP'
                                     ELSE 'EQ' ).

    ct_selected_ranges = VALUE #( ( tablename = iv_table_of_key_field
                                    frange_t  = VALUE #( ( fieldname = iv_key_field
                                                           selopt_t  = VALUE #( ( low    = iv_entered_value
                                                                                  sign   = 'I'
                                                                                  option = lv_option ) ) ) ) ) ).
  ENDMETHOD.
...
*&---------------------------------------------------------------------*
    ...
    IF lines( it_f4_definition ) > 1.
      DATA(lt_f4_helps) = VALUE zdbbr_f4_data_itab( FOR f4_def IN it_f4_definition
                                                    ( CORRESPONDING #( f4_def ) ) ).
    ...
    IF ls_f4_definition-is_built_in = abap_true.
      ...
      IF if_multiple_select = abap_true.
        zcl_dbbr_f4_helper=>call_built_in_f4_multi( EXPORTING if_low          = if_low
    ...
    ELSE.
      ...
      fetch_f4_values( EXPORTING is_custom_f4_definition = ls_f4_definition
                                 iv_entered_value        = iv_selvalue
                       IMPORTING er_dynamic_table        = DATA(lr_dynamic_value_tab)
      ...
      CALL FUNCTION 'F4IF_INT_TABLE_VALUE_REQUEST'
        EXPORTING  retfield        = lv_key_field_name
      ...
      IF sy-subrc = 0 AND lt_return IS NOT INITIAL.
        ...
        IF if_multiple_select = abap_true.
          ...
          LOOP AT lt_return ASSIGNING FIELD-SYMBOL(<ls_return>).
            ...
            IF    ls_f4_definition-perform_alpha_conversion  = abap_true
               OR ls_f4_definition-perform_alpha_conv_assgmt = abap_true.
              zcl_sat_data_converter=>perform_alpha_conversion_input(
                EXPORTING iv_tabname   = ls_f4_definition-fields[ is_search_key = abap_true ]-search_table
        ...
        ELSE.
          ...
          IF    ls_f4_definition-perform_alpha_conversion  = abap_true
            ...
            zcl_sat_data_converter=>perform_alpha_conversion_input(
              ...
              IMPORTING ev_output    = lv_returnval ).
          ENDIF.
...
*&---------------------------------------------------------------------*
    ...
    DATA lt_where TYPE TABLE OF ty_where_line.
    DATA lt_from_clause TYPE string_table.
    DATA lt_sort TYPE TABLE OF string.
    ...
    ELSE.
      ...
      IF iv_entered_value IS NOT INITIAL.
        ...
        DATA(lv_entered_value_low) = COND string(
          ...
          WHEN lf_joins_needed = abap_true
           AND ls_search_field-search_table_alias IS INITIAL THEN
    ...
    LOOP AT lt_f4_fields ASSIGNING FIELD-SYMBOL(<ls_field>).
      ...
      APPEND VALUE abap_componentdescr(
          name = lv_alias_name
          type = CAST cl_abap_datadescr( cl_abap_typedescr=>describe_by_name(
                                             |{ <ls_field>-search_table }-{ <ls_field>-search_field }| ) ) )
             TO lt_component_table.

      " add field to select table
      DATA(lv_select_field) = COND #(
        WHEN lf_joins_needed = abap_true
        THEN |{ lv_sql_name } AS { lv_alias_name }|
        ELSE lv_sql_name ).
      ...
      IF <ls_field>-sort_active = abap_true.
        ...
        lt_sort = VALUE #( BASE lt_sort
                           ( |{ lv_separator }{ lv_sql_name } ASCENDING| ) ).
      ENDIF.
    ENDLOOP.
    ...
    DATA(lr_new_tab) = cl_abap_tabledescr=>create( p_line_type  = lr_new_type
    ...
    TRY.
      ...
      CATCH cx_sy_dynamic_osql_semantics INTO DATA(lr_osql_sem_exc). " TODO: variable is assigned but never used (ABAP cleaner)
...
*&---------------------------------------------------------------------*
    ...
    DATA lt_selection_fields TYPE TABLE OF rsdsfields.
    DATA lt_selected_ranges TYPE rsds_trange.
    DATA lt_free_sel_where TYPE rsds_twhere.
    DATA lv_filter_field_count TYPE i.
    ...
    LOOP AT it_fields ASSIGNING FIELD-SYMBOL(<ls_filter_field>) WHERE allow_restriction = abap_true.
      ...
      IF     <ls_filter_field>-search_field  = iv_key_field
         AND <ls_filter_field>-search_table  = iv_table_of_key_field
         AND iv_entered_value               IS NOT INITIAL.
    ...
    CALL FUNCTION 'FREE_SELECTIONS_INIT'
      EXPORTING  kind         = 'F'
      IMPORTING  selection_id = lv_selection_id
      TABLES     fields_tab   = lt_selection_fields
      EXCEPTIONS OTHERS       = 1.

    """ display free selections dialog
    CALL FUNCTION 'FREE_SELECTIONS_DIALOG'
      EXPORTING  selection_id            = lv_selection_id
      ...
      IMPORTING  field_ranges            = lt_selected_ranges
                 number_of_active_fields = lv_filter_field_count
      TABLES     fields_tab              = lt_selection_fields
      EXCEPTIONS no_action               = 1
                 OTHERS                  = 2.
    ...
    CALL FUNCTION 'FREE_SELECTIONS_RANGE_2_WHERE'
      EXPORTING field_ranges  = lt_selected_ranges
      IMPORTING where_clauses = lt_free_sel_where.
...
*&---------------------------------------------------------------------*
    ...
    CALL FUNCTION 'LVC_SINGLE_ITEM_SELECTION'
      EXPORTING i_title         = |{ 'Choose Value Help'(001) }|
                it_fieldcatalog = lt_fieldcat
      IMPORTING es_selfield     = ls_selfield
                e_exit          = lf_exit
      TABLES    t_outtab        = lt_f4_info.
