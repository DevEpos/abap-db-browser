"! <p class="shorttext synchronized">Map for custom value help</p>
CLASS zcl_dbbr_custom_f4_map DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Reads custom F4 definitions for the given table-field</p>
    ...
    "! <p class="shorttext synchronized">Reads custom f4 definitions for the given table</p>
    METHODS read_custom_f4_definitions
      IMPORTING
        iv_tablename TYPE tabname.

    "! <p class="shorttext synchronized">Read and cache custom f4 w/o necessary field assignment</p>
    "!
    METHODS read_same_type_custom_f4_defs.
    "! <p class="shorttext synchronized">Clears all buffered custom f4 definitions</p>
    METHODS clear.

    "! <p class="shorttext synchronized">Clears all buffered F4 def. for the given table field</p>
    METHODS clear_f4_for_field
      IMPORTING
        iv_tabname   TYPE tabname
        iv_fieldname TYPE fieldname.

    "! <p class="shorttext synchronized">Determine F4 for given table field</p>
    METHODS determine_f4_for_field
      IMPORTING
        iv_tabname   TYPE tabname
        iv_fieldname TYPE fieldname.

    "! <p class="shorttext synchronized">Checks if there is a custom F4 for the given table field</p>
...
CLASS zcl_dbbr_custom_f4_map IMPLEMENTATION.
  ...
  METHOD determine_f4_for_field.
    zcl_dbbr_custom_f4_factory=>find_f4_for_table( EXPORTING iv_tabname = iv_tabname
  ...
  METHOD read_custom_f4_definitions.
    IF line_exists( mt_custom_f4_map[ tabname = iv_tablename ] ).
      RETURN.
    ENDIF.

    zcl_dbbr_custom_f4_factory=>find_f4_for_table( EXPORTING iv_tabname = iv_tablename
                                                   IMPORTING et_f4      = DATA(lt_f4) ).
    ...
    LOOP AT lt_f4 ASSIGNING FIELD-SYMBOL(<ls_f4>)
      ...
      INSERT VALUE #( tabname        = <ls_f4_group>-tabname
                      fieldname      = <ls_f4_group>-fieldname
                      f4_definitions = VALUE zdbbr_f4_data_itab( FOR f4 IN GROUP <ls_f4_group>
                                                                 ( f4 ) ) )
             INTO TABLE mt_custom_f4_map.
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD read_same_type_custom_f4_defs.
    zcl_dbbr_custom_f4_factory=>find_f4_for_datatype( EXPORTING if_apply_to_same_data = abap_true
                                                      IMPORTING et_f4                 = DATA(lt_f4) ).
    LOOP AT lt_f4 ASSIGNING FIELD-SYMBOL(<ls_f4>)
      ...
      INSERT VALUE #( rollname       = <ls_f4_group>-rollname
                      datatype       = <ls_f4_group>-datatype
                      length         = <ls_f4_group>-length
                      f4_definitions = VALUE #( FOR f4 IN GROUP <ls_f4_group>
                                                ( f4 )  ) )
