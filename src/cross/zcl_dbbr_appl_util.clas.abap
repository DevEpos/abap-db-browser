...
*&---------------------------------------------------------------------*
    ...
    DATA(lv_option) = COND #( WHEN contains( val = iv_value
                                             sub = '*' )
                              THEN 'CP'
                              ELSE 'EQ' ).
  ...
  METHOD build_selopt_tab_from_table.
    LOOP AT it_table_data ASSIGNING FIELD-SYMBOL(<ls_line>).
      ...
      APPEND VALUE zif_sat_ty_global=>ty_s_selopt( sign   = 'I'
                                                   option = 'EQ'
                                                   low    = <lv_low_val> )
             TO rt_selopt.
    ENDLOOP.
...
*                 doktyp            =
  ...
  METHOD get_program_variables.
    ...
    CALL FUNCTION 'RS_PROGRAM_INDEX'
      EXPORTING  pg_name      = iv_repid
      TABLES     compo        = rt_variables
                 inc          = lt_includes
    ...
    DELETE rt_variables WHERE    name CP '<*'
                              OR type <> 'D'.
  ENDMETHOD.
  ...
  METHOD translate_first_letter.
    ...
    IF ev_output+lv_pos(1) CA 'ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÜ'.
      ev_output+lv_pos(1) = to_lower( ev_output+lv_pos(1) ).
    ELSEIF ev_output+lv_pos(1) CA 'abcdefghijklmnopqrstuvwxyzäöü'.
      ev_output+lv_pos(1) = to_upper( ev_output+lv_pos(1) ).
    ENDIF.
  ENDMETHOD.
ENDCLASS.
