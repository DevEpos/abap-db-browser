"! <p class="shorttext synchronized">Retrieves free Alias for Join definition</p>
...
CLASS zcl_dbbr_entity_alias_util DEFINITION
  ...
  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Initialize aliases</p>
    "!
    CLASS-METHODS initialize_aliases
      IMPORTING
        if_alv_alias_only TYPE abap_bool OPTIONAL.

    "! <p class="shorttext synchronized">Retrieves the next free ALV Alias</p>
    "!
    CLASS-METHODS get_next_free_alv_alias
      RETURNING
        VALUE(rv_alias) TYPE zsat_entity_alias_alv.

    "! <p class="shorttext synchronized">Creates alias for the entity and registers it</p>
    "!
    ...
    "! <p class="shorttext synchronized">Add new Alias</p>
    "!
    ...
    "! <p class="shorttext synchronized">Unregisters given alias from Alias pool</p>
    "!
    CLASS-METHODS unregister_alv_alias
      IMPORTING
        iv_alias TYPE zsat_entity_alias_alv.

    "! <p class="shorttext synchronized">Unregister long alias name</p>
    "!
    ...
    "! <p class="shorttext synchronized">Add ALV Alias for entity</p>
    "!
...
CLASS zcl_dbbr_entity_alias_util IMPLEMENTATION.
  METHOD add_entity_alv_alias.
    ...
    IF NOT line_exists( gt_alv_alias_in_use[ table_line = iv_alias ] ).
      gt_alv_alias_in_use = VALUE #( BASE gt_alv_alias_in_use
                                     ( iv_alias ) ).
      rf_added = abap_true.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD add_entity_alias.
    ...
    IF NOT line_exists( gt_alias_in_use[ table_line = iv_entity_alias ] ).
      gt_alias_in_use = VALUE #( BASE gt_alias_in_use
                                 ( iv_entity_alias ) ).
      rf_added = abap_true.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD check_entity_alias.
    ...
    zcx_sat_validation_exception=>raise_with_text( iv_text = |{ 'Alias' } { iv_alias } { 'is already in use!' }| ).
  ENDMETHOD.
