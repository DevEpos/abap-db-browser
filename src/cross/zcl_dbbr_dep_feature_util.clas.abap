"! <p class="shorttext synchronized">Util for dependent features</p>
CLASS zcl_dbbr_dep_feature_util DEFINITION
  PUBLIC FINAL
  CREATE PRIVATE.

  PUBLIC SECTION.
    CONSTANTS c_cds_annotation_definition TYPE string VALUE 'DDLA_RT_HEADER'.
    CONSTANTS c_if_sadl_exit_calc_element TYPE string VALUE 'IF_SADL_EXIT_CALC_ELEMENT_READ'.

    CLASS-METHODS class_constructor.

    "! <p class="shorttext synchronized">Returns 'X' if SE16N exists in the system</p>
    CLASS-METHODS is_se16n_available
      RETURNING
        VALUE(rf_available) TYPE abap_bool.

    "! <p class="shorttext synchronized">Returns 'X' CDS virtual elements are supported</p>
    CLASS-METHODS is_cds_virtelem_supported
      RETURNING
        VALUE(rf_available) TYPE abap_bool.

    "! <p class="shorttext synchronized">Check if CDS virtual elements are supported</p>
...
CLASS zcl_dbbr_dep_feature_util IMPLEMENTATION.
  ...
  METHOD check_cds_virtelem_supported.
    ...
    TRY.
        cl_abap_typedescr=>describe_by_name( EXPORTING  p_name         = c_if_sadl_exit_calc_element
                                             RECEIVING  p_descr_ref    = DATA(lo_inf)
                                             EXCEPTIONS type_not_found = 1 ).

        IF sy-subrc = 0 AND lo_inf->kind = cl_abap_typedescr=>kind_intf.
          SELECT SINGLE @abap_true
            FROM (c_cds_annotation_definition)
