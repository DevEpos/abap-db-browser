"! <p class="shorttext synchronized">Fills tree nodes/items out of table field list</p>
CLASS zcl_dbbr_table_treeno_fill DEFINITION
  ...
  PRIVATE SECTION.
    ...
    ALIASES tt_node_map FOR zif_dbbr_tree_node_filler~tt_node_map.
    ...
    DATA mt_matched_fields TYPE STANDARD TABLE OF ty_s_matched_type_fields WITH EMPTY KEY.

    "! <p class="shorttext synchronized">Create field nodes for given table</p>
    ...
    "! <p class="shorttext synchronized">Create node for fieldnames with domain match / type match</p>
...
CLASS zcl_dbbr_table_treeno_fill IMPLEMENTATION.
  ...
  METHOD create_matched_node_and_fields.
    ...
    ir_nodes->add_node( iv_node_key          = c_matched_node
    ...
    SORT mt_matched_fields BY domain_match DESCENDING
                              prio ASCENDING.
    LOOP AT mt_matched_fields ASSIGNING FIELD-SYMBOL(<ls_matched_fields>).
      DATA(lv_icon) = COND tv_image(
        WHEN <ls_matched_fields>-domain_match = abap_true
        THEN zcl_dbbr_icon_handler=>create_icon_with_tip( iv_icon     = icon_led_green
                                                          iv_quicktip = 'Domain matches' )
        ELSE zcl_dbbr_icon_handler=>create_icon_with_tip( iv_icon     = icon_led_yellow
      ...
      ir_nodes->add_node(
          ...
          iv_image             = lv_icon
          iv_style             = COND #( WHEN <ls_matched_fields>-domain_match = abap_true
                                          AND <ls_matched_fields>-fieldname    = ms_join_field_info-fieldname
                                         THEN zif_uitb_c_ctm_style=>light_green )
          it_item_table        = VALUE #(
              font  = cl_item_tree_model=>item_font_prop
              class = cl_item_tree_model=>item_class_text
  ...
  METHOD create_table_field_nodes.
    zcl_sat_ddic_repo_access=>get_table_field_infos( EXPORTING iv_tablename    = is_entity-entity_id
    ...
    LOOP AT lt_table_fields ASSIGNING FIELD-SYMBOL(<ls_table_field>) WHERE     tabname    = is_entity-entity_id
      ...
      IF     <ls_table_field>-datatype = ms_join_field_info-datatype
         AND <ls_table_field>-leng     = ms_join_field_info-leng.
      ...
      ir_nodes->add_node( iv_node_key          = lv_field_node_key
                          iv_relative_node_key = iv_table_node
                          if_folder            = abap_false
                          iv_image             = lv_key_image
                          it_item_table        = VALUE #( font  = cl_item_tree_model=>item_font_prop
                                                          class = cl_item_tree_model=>item_class_text
                                                          ( item_name = zcl_uitb_column_tree_model=>c_hierarchy_column
      ...
      mv_field_node_counter = mv_field_node_counter + 1.
  ...
  METHOD zif_dbbr_tree_node_filler~fill_node_item_tables.
    ...
    TRY.
        LOOP AT mt_join_tables ASSIGNING FIELD-SYMBOL(<ls_join_table_sel>).
          ...
          DATA(ls_tab_info) = zcl_sat_ddic_repo_access=>get_table_info( |{ <ls_join_table_sel>-entity_id }| ).
          ...
          ir_nodes->add_node( iv_node_key       = lv_table_node_key
                              if_folder         = abap_true
                              iv_image          = lv_icon
          ...
          create_table_field_nodes( EXPORTING ir_nodes      = ir_nodes
        ...
        create_matched_node_and_fields( EXPORTING ir_nodes    = ir_nodes
                                        CHANGING  ct_node_map = rt_node_map ).
