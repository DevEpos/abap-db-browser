CLASS zcl_dbbr_screen_helper DEFINITION
  ...
  PRIVATE SECTION.
    CLASS-DATA sf_program_quit_disabled TYPE c LENGTH 1.
ENDCLASS.
...
CLASS zcl_dbbr_screen_helper IMPLEMENTATION.
  ...
  METHOD set_parameter_value.
    CALL FUNCTION 'SMAN_SET_USER_PARAMETER'
      EXPORTING  parameter_id    = CONV memoryid( iv_parameter_id )
...
*&---------------------------------------------------------------------*
    ...
    CALL FUNCTION 'SAPGUI_PROGRESS_INDICATOR'
      EXPORTING percentage = lv_progress
                text       = iv_text.
  ENDMETHOD.
ENDCLASS.
