"! <p class="shorttext synchronized">Utilities for CDS View Tablefield list</p>
CLASS zcl_dbbr_cds_tabfield_util DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Adds columns to tab field list</p>
    "!
    ...
    "! <p class="shorttext synchronized">Add parameters to tab field list</p>
    "!
...
CLASS zcl_dbbr_cds_tabfield_util IMPLEMENTATION.
  METHOD add_parameters.
    ...
    LOOP AT it_parameters ASSIGNING FIELD-SYMBOL(<ls_param>) WHERE has_system_anno = abap_false.
      ...
      lv_rollname = COND rollname(
        WHEN <ls_param>-rollname IS NOT INITIAL THEN <ls_param>-rollname
        WHEN <ls_param>-datatype = 'LANG'       THEN 'LANGU'
        WHEN <ls_param>-datatype = 'DATS'       THEN 'DATS'
        WHEN <ls_param>-datatype = 'TIMS'       THEN 'TIMS' ).
      ...
      IF io_custom_f4_map IS BOUND.
        ls_tabfield-has_custom_f4 = io_custom_f4_map->entry_exists( iv_tabname       = <ls_param>-strucobjn
    ...
    rs_entity = VALUE zdbbr_entity_info( active_selection = abap_true
                                         tabname          = zif_dbbr_c_global=>c_parameter_dummy_table
  ...
  METHOD add_view_colums.
    ...
    lo_addtext_bl->determine_text_fields( iv_entity      = io_cds_view->mv_view_name
    ...
    LOOP AT io_cds_view->get_columns( ) ASSIGNING FIELD-SYMBOL(<ls_column>).
      ...
      " .... determine the rollname of the parameter
      lv_rollname = COND rollname(
        WHEN <ls_column>-rollname IS NOT INITIAL THEN <ls_column>-rollname
        WHEN <ls_column>-datatype = 'LANG'       THEN 'LANGU' ).

      DATA(ls_altcoltext) = lo_altcoltext_f->find_alternative_text( iv_tabname   = <ls_column>-strucobjn
      ...
      IF io_custom_f4_map IS BOUND.
        ls_tabfield-has_custom_f4 = io_custom_f4_map->entry_exists( iv_tabname       = io_cds_view->mv_view_name
                                                                    iv_fieldname     = ls_tabfield-fieldname
                                                                    iv_rollname      = ls_tabfield-rollname
                                                                    is_built_in_type = VALUE #(
                                                                        datatype = ls_tabfield-datatype
      ...
      IF ls_tabfield-rollname IS NOT INITIAL.
        ...
        ls_tabfield-convexit        = ls_datel-convexit.
        ls_tabfield-inttype         = zcl_sat_ddic_repo_access=>get_dtel_inttype(
                                          iv_data_element = ls_datel-rollname  ).
      ELSE.
        TRY.
            DATA(lr_typedescr) = CAST cl_abap_elemdescr( cl_abap_typedescr=>describe_by_name(
                                                             |{ <ls_column>-strucobjn }-{ <ls_column>-fieldname }| ) ).
      ...
      DATA(lr_new_field) = CAST zdbbr_tabfield_info_ui( ir_tabfield_list->zif_uitb_data_ref_list~add(
                                                            REF #( ls_tabfield ) ) ).

      IF     ls_datel IS NOT INITIAL
         AND lo_addtext_bl->text_exists( VALUE #( tabname   = io_cds_view->mv_view_name
                                                  fieldname = ls_tabfield-fieldname ) ).

        lo_addtext_bl->add_text_fields_to_list( ir_tabfields    = ir_tabfield_list
