"! <p class="shorttext synchronized">Helper for DDIC objects</p>
CLASS zcl_dbbr_ddic_util DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Check if there is a single DB entry for the given entity</p>
    ...
    "! <p class="shorttext synchronized">Retrieve all mappings of domain to convexit</p>
    "!
    "! @parameter rt_convext2domain | <p class="shorttext synchronized"></p>
    CLASS-METHODS get_domain_to_convexit_entries
      RETURNING
        VALUE(rt_convext2domain) TYPE tt_domain2convext.

    "! <p class="shorttext synchronized">Retrieve table/view/cds from clipboard</p>
    "!
    ...
    "! <p class="shorttext synchronized">Create initial RAW value</p>
    ...
    "! <p class="shorttext synchronized">Navigate to Table/View via SE11</p>
    "!
    ...
    "! <p class="shorttext synchronized">Checks if type is numeric</p>
    ...
    "! <p class="shorttext synchronized">Checks if Data element is numeric</p>
    "!
    "! @parameter iv_dtel_name  | <p class="shorttext synchronized"></p>
    "! @parameter rf_is_numeric | <p class="shorttext synchronized"></p>
    ...
    "! <p class="shorttext synchronized">Validates the given table name</p>
    ...
    "! <p class="shorttext synchronized">Validates table field</p>
    ...
    "! <p class="shorttext synchronized">Call table field f4</p>
    ...
    "! <p class="shorttext synchronized">Call Table F4</p>
    ...
    "! <p class="shorttext synchronized">Build dynamic standard table</p>
    ...
    "! <p class="shorttext synchronized">Build dynamic hash table</p>
    ...
    "! <p class="shorttext synchronized">Build dynamic table with keys</p>
    ...
    "! <p class="shorttext synchronized">Fills data cache for additional texts</p>
    CLASS-METHODS fill_data_cache_for_add_texts
      CHANGING
        ct_add_texts TYPE zdbbr_additional_text_itab.

    "! <p class="shorttext synchronized">Validates data element name</p>
    CLASS-METHODS validate_data_element
      IMPORTING
        iv_data_element TYPE rollname.

    "! <p class="shorttext synchronized">Trigger where used </p>
    CLASS-METHODS cross_reference_table
      IMPORTING
        iv_tabname TYPE tabname.

    "! <p class="shorttext synchronized">Check if possible system type</p>
    ...
    "! <p class="shorttext synchronized">Check if the given transaction is valid</p>
    ...
    "! <p class="shorttext synchronized">Builds dynamic standard table</p>
    ...
    "! <p class="shorttext synchronized">Call the customizing view of the given table</p>
    CLASS-METHODS call_customizing_view
      IMPORTING
        iv_view_name TYPE tabname.

    "! <p class="shorttext synchronized">Checks the given package</p>
...
CLASS zcl_dbbr_ddic_util IMPLEMENTATION.
  METHOD build_dynamic_hash_table.
    result = create_dynamic_table( it_fields       = it_fields
...
*& data elements.
*&---------------------------------------------------------------------*
    rr_table_ref = create_dynamic_table( it_fields       = it_fields
  ...
  METHOD call_customizing_view.
    ...
    DATA lt_exclude_cua_functions TYPE STANDARD TABLE OF vimexclfun.

    CALL FUNCTION 'AUTHORITY_CHECK_TCODE'
      EXPORTING  tcode  = 'SM30'
      EXCEPTIONS ok     = 0
                 not_ok = 1.
    IF sy-subrc <> 0.
    ...
    CALL FUNCTION 'VIEW_MAINTENANCE_CALL'
      ...
      TABLES    dba_sellist         = lt_selection_list
...
*&---------------------------------------------------------------------*
    ...
    DATA lv_field TYPE dynfieldvalue.
    DATA lt_values TYPE TABLE OF lty_value_tab.
    DATA lt_return TYPE TABLE OF ddshretval.
    DATA lf_reset TYPE boolean.

    DATA(lr_dynpfield_mng) = NEW zcl_uitb_screen_field_manager( iv_repid = iv_repid ).
    lr_dynpfield_mng->read_values( it_fieldname_selopt = VALUE #( sign   = 'I'
                                                                  option = 'EQ'
                                                                  ( low = iv_dynpname_tablename )
                                                                  ( low = iv_dynpname_fieldname ) )
    ...
    zcl_sat_ddic_repo_access=>get_table_field_infos( EXPORTING iv_tablename    = lv_table
    ...
    CALL FUNCTION 'F4IF_INT_TABLE_VALUE_REQUEST'
      EXPORTING  retfield        = 'FIELDNAME'
                 ...
                 value           = lv_field
      IMPORTING  user_reset      = lf_reset
      TABLES     value_tab       = lt_values
  ...
  METHOD check_package.
    cl_package_helper=>check_package_existence( EXPORTING  i_package_name          = iv_package
                                                IMPORTING  e_package_exists        = DATA(lf_exists)
                                                EXCEPTIONS intern_err              = 1
    ...
    ELSE.
      IF lf_exists = abap_false.
        RAISE EXCEPTION TYPE zcx_sat_validation_exception
          EXPORTING textid = zcx_sat_validation_exception=>package_not_existing
                    msgv1  = |{ iv_package }|.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD create_dynamic_table.
    ...
    DATA lt_key_table TYPE abap_keydescr_tab.
    ...
    LOOP AT it_fields ASSIGNING FIELD-SYMBOL(<ls_field>).
      ...
      cl_abap_typedescr=>describe_by_name( EXPORTING  p_name         = lv_type_name
      ...
      APPEND VALUE abap_componentdescr( name = <ls_field>-fieldname
    ...
    DATA(lr_new_tab) = COND #(
      WHEN it_key_defs IS INITIAL
      THEN cl_abap_tabledescr=>create(
               p_line_type  = lr_new_type
               p_table_kind = COND #( WHEN if_sorted_table = abap_true   THEN cl_abap_tabledescr=>tablekind_sorted
                                      WHEN if_hashed_table = abap_true   THEN cl_abap_tabledescr=>tablekind_hashed
                                      WHEN if_standard_table = abap_true THEN cl_abap_tabledescr=>tablekind_std )
               p_key_kind   = COND #( WHEN if_standard_table = abap_true
                                      THEN cl_abap_tabledescr=>keydefkind_default
                                      ELSE cl_abap_tabledescr=>keydefkind_user )
      ...
      ELSE cl_abap_tabledescr=>create_with_keys( p_line_type = lr_new_type
                                                 p_keys      = it_key_defs ) ).
...
*&---------------------------------------------------------------------*
    LOOP AT ct_add_texts ASSIGNING FIELD-SYMBOL(<ls_add_text>).
      CASE <ls_add_text>-selection_type.
        ...
        WHEN zif_dbbr_c_text_selection_type=>domain_value.
          DATA(lr_elemdescr) = CAST cl_abap_elemdescr( cl_abap_elemdescr=>describe_by_name(
                                                           <ls_add_text>-id_field_rollname ) ).
        ...
        WHEN zif_dbbr_c_text_selection_type=>table OR
          ...
          DATA(lt_fields) = VALUE zsat_dfies_itab( ( tabname   = <ls_add_text>-text_table
          ...
          <ls_add_text>-table_cache = build_dynamic_sorted_table( it_fields     = lt_fields
  ...
  METHOD is_type_numeric.
    IF    iv_internal_type = cl_abap_datadescr=>typekind_int
       OR iv_internal_type = cl_abap_datadescr=>typekind_float
       OR iv_internal_type = cl_abap_datadescr=>typekind_packed.
...
*     WITH SY-MSGV1 SY-MSGV2 SY-MSGV3 SY-MSGV4.
  ...
  METHOD validate_table_field.
    result = zcl_sat_ddic_repo_access=>get_table_field_info( iv_tablename = iv_table_name
                                                             iv_fieldname = iv_table_field ).

    IF result IS INITIAL.
      MESSAGE e018(zdbbr_info) WITH iv_table_field iv_table_name INTO DATA(lv_dummy) ##NEEDED.
      zcx_sat_validation_exception=>raise_from_sy( iv_parameter = iv_dynfname
...
*&---------------------------------------------------------------------*
    ...
    IF ls_table_info IS NOT INITIAL.
      IF     ls_table_info-tabclass <> 'TRANSP'
         AND ls_table_info-tabclass <> 'VIEW'
         AND ls_table_info-tabclass <> 'POOL'.
        MESSAGE e001(wusl) WITH iv_table_name lv_object_type INTO DATA(lv_message) ##NEEDED.
        zcx_sat_validation_exception=>raise_from_sy( iv_parameter = iv_dynpro_fieldname ).
      ELSE.
        IF ( ls_table_info-tabclass = 'VIEW' AND ls_table_info-viewclass = 'C' )
           AND if_customizing_view_allowed = abap_false.
    ...
    ls_table_info-tabclass = 'VIEW'.
    ef_is_view = ls_table_info-tabclass.
  ENDMETHOD.

  METHOD create_initial_raw_value.
    DATA lr_v_raw TYPE REF TO data.
    DATA lv_raw TYPE c LENGTH 128.
  ...
  METHOD get_entities_from_clipboard.
    ...
    DATA lv_entity TYPE tabname.
    DATA lt_possible_entities TYPE STANDARD TABLE OF tabname.
    " TODO: variable is assigned but never used (ABAP cleaner)
    DATA lv_rest_content TYPE string.
    ...
    LOOP AT lt_clipboard_data ASSIGNING FIELD-SYMBOL(<ls_clipboard_data>).
      ...
      lt_possible_entities = VALUE #( BASE lt_possible_entities
                                      ( lv_entity ) ).
    ENDLOOP.
    ...
    IF sy-subrc = 0.
      rt_entities = VALUE #( FOR query IN lt_queries_from_clipboard
    ...
    ELSE.
      LOOP AT lt_conv2domain ASSIGNING <ls_conv2dom>.
        IF line_exists( lt_found_func_output[ table_line = <ls_conv2dom>-funcname ] ).
          rt_convext2domain = VALUE #( BASE rt_convext2domain
                                       ( CORRESPONDING #( <ls_conv2dom> ) ) ).
        ENDIF.
      ENDLOOP.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD exists_data_for_entity.
    ...
    zcl_sat_ddic_repo_access=>get_table_field_infos( EXPORTING iv_tablename    = iv_entity_id
    ...
    cl_abap_typedescr=>describe_by_name( EXPORTING  p_name      = |{ to_upper( iv_entity_id ) }-{ ls_field-fieldname }|
    ...
    TRY.
        ...
        ENDIF.
      CATCH cx_root INTO DATA(lx_root). " TODO: variable is assigned but only used in commented-out code (ABAP cleaner)
