"! <p class="shorttext synchronized">Wrapper for list of table fields</p>
CLASS zcl_dbbr_tabfield_list DEFINITION
  ...
  PUBLIC SECTION.
    ...
    ALIASES add FOR zif_uitb_data_ref_list~add.

    "! <p class="shorttext synchronized">Create list from serialized data</p>
    ...
    "! <p class="shorttext synchronized">Adds table to table list</p>
    METHODS add_table
      IMPORTING
    ...
    "! <p class="shorttext synchronized">Converts list to deep structure</p>
    ...
    "! <p class="shorttext synchronized">Deletion of tables/fields by custom criteria</p>
    "!
    ...
    "! <p class="shorttext synchronized">Get iterator instance for list</p>
    ...
    "! <p class="shorttext synchronized">Get alias name for internal table name</p>
    "!
    "! @parameter iv_tabname    | <p class="shorttext synchronized"></p>
    "! @parameter rv_alias_name | <p class="shorttext synchronized"></p>
    ...
    "! <p class="shorttext synchronized">Retrieve reference to all tables in the list</p>
    "!
    ...
    "! <p class="shorttext synchronized">Checks if the given table exists in the list</p>
    "!
    METHODS has_table
      IMPORTING
    ...
    "! <p class="shorttext synchronized">Checks if this list has multiple tables</p>
    "!
    ...
    "! <p class="shorttext synchronized">Load fields for table</p>
    METHODS load_fields
      IMPORTING
        ir_fields        TYPE REF TO zcl_dbbr_tabfield_list
        iv_tabname_alias TYPE tabname.

    "! <p class="shorttext synchronized">Overwrite current data with data from passed list</p>
    METHODS overwrite
      IMPORTING
    ...
    "! <p class="shorttext synchronized">Sets the entity type attribute</p>
    ...
    "! <p class="shorttext synchronized">Sort tables by active selection</p>
    ...
    "! <p class="shorttext synchronized">Replace table alias and update fields</p>
    "!
    ...
    "! <p class="shorttext synchronized">Updates the table list (Aliases and indexes)</p>
    "! @parameter if_force_update | <p class="shorttext synchronized">Forces the alias/index update even if not needed</p>
    METHODS update_tables
      IMPORTING
        if_force_update TYPE abap_bool OPTIONAL.

    "! <p class="shorttext synchronized">Update the active selection/selection order for a table</p>
    ...
    "! <p class="shorttext synchronized">Set the grouping to active</p>
    METHODS set_multi_table_mode
      IMPORTING
        if_active_grouping TYPE abap_bool DEFAULT abap_true.

    "! <p class="shorttext synchronized">Fills missing attributes of table fields</p>
    "!
    METHODS complete_field_infos.

    "! <p class="shorttext synchronized">Returns all fields</p>
    METHODS get_fields
      RETURNING
        VALUE(rt_fields) TYPE zdbbr_tabfield_info_ui_itab.

    "! <p class="shorttext synchronized">Returns only checked fields (i.e. sorted, output, selection)</p>
  ...
  PRIVATE SECTION.
    TYPES mty_fieldname_itab TYPE STANDARD TABLE OF fieldname.
    ...
    "! <p class="shorttext synchronized">List of Entity infos</p>
...
CLASS zcl_dbbr_tabfield_list IMPLEMENTATION.
  METHOD active_field_exists.
    CASE mv_mode.
      WHEN zif_dbbr_c_global=>c_field_chooser_modes-output.
        rf_exists = xsdbool( line_exists( mt_fields[ KEY unique
                                                     tabname_alias = iv_tabname
  ...
  METHOD add_fields.
    ...
    mt_fields = VALUE #( BASE mt_fields
                         ( LINES OF lt_fields ) ).
  ...
  METHOD build_table_to_alias_map.
    ...
    IF if_update_tables = abap_true.
      update_tables( ).
      rt_table_to_alias_map = VALUE #( FOR table IN mt_tables
                                       ( alias   = table-alias
    ...
    IF sy-subrc = 0.
      rt_table_to_alias_map = VALUE #( FOR tab IN mt_tables
                                       WHERE ( alias <> space )
                                       ( alias   = tab-alias
    ...
    ELSE.
      ...
      LOOP AT mt_tables ASSIGNING <ls_table> WHERE     tabname <> zif_dbbr_c_global=>c_formula_dummy_table
        ...
        INSERT VALUE #( tabname = <ls_table>-tabname_alias
                        alias   = zcl_dbbr_alias_map=>get_alias( lv_table_counter ) )
               INTO TABLE rt_table_to_alias_map.
        lv_table_counter = lv_table_counter + 1.
      ENDLOOP.
    ENDIF.
  ENDMETHOD.

  METHOD checked_field_exists.
    " TODO: parameter IV_SPECIFIC_TABLE is never used (ABAP cleaner)
  ...
  METHOD clear_active_flag.
    LOOP AT mt_fields ASSIGNING FIELD-SYMBOL(<ls_field>).
      IF     iv_tablename IS NOT INITIAL
         AND iv_tablename <> <ls_field>-tabname_alias.
        CONTINUE.
      ENDIF.
  ...
  METHOD complete_field_infos.
    LOOP AT mt_fields ASSIGNING FIELD-SYMBOL(<ls_field>).
      ...
      IF    <ls_field>-is_numeric  = abap_true
         OR <ls_field>-rollname   IS INITIAL.
        CONTINUE.
      ENDIF.

      DATA(lo_elem_descr) = CAST cl_abap_elemdescr( cl_abap_typedescr=>describe_by_name( <ls_field>-rollname ) ).

      lo_elem_descr->get_ddic_fixed_values( RECEIVING  p_fixed_values = DATA(lt_fix_values)
                                            EXCEPTIONS not_found      = 1
                                                       no_ddic_type   = 2
                                                       OTHERS         = 3 ).
      IF sy-subrc = 0.
        <ls_field>-is_checkbox = xsdbool(     lines( lt_fix_values ) = 2
  ...
  METHOD convert_to_structure.
    result = VALUE zdbbr_tabfield_list_data( fields      = mt_fields
                                             tables      = mt_tables
  ...
  METHOD create_from_serialized.
    rr_list = NEW zcl_dbbr_tabfield_list( it_fields      = is_serialized_tabfields-fields
  ...
  METHOD create_order_from_active.
    ...
    LOOP AT mt_fields ASSIGNING FIELD-SYMBOL(<ls_field>).
      ...
      <lv_order_field> = COND #( WHEN <lv_active_flag> = abap_true
                                 THEN lv_index
                                 ELSE 0 ).
  ...
  METHOD delete_custom.
    ...
    IF if_delete_formfields = abap_true.
      lt_tabname_range = VALUE #( BASE lt_tabname_range
                                  ( sign = 'I' option = 'EQ' low = zif_dbbr_c_global=>c_formula_dummy_table ) ).
    ENDIF.

    IF if_delete_params = abap_true.
      lt_tabname_range = VALUE #( BASE lt_tabname_range
                                  ( sign = 'I' option = 'EQ' low = zif_dbbr_c_global=>c_parameter_dummy_table ) ).
    ELSE.
      " .. Exclude parameters from being deleted until it is clear that no entity exists any more that needs them
      lt_tabname_range = VALUE #( BASE lt_tabname_range
                                  ( sign = 'E' option = 'EQ' low = zif_dbbr_c_global=>c_parameter_dummy_table ) ).
    ENDIF.
    ...
    IF if_keep_primary = abap_true AND line_exists( mt_tables[ is_primary = abap_true ] ).
      lt_tabname_range = VALUE #( BASE lt_tabname_range
                                  ( sign   = 'E'
                                    option = 'EQ'
                                    low    = mt_tables[ is_primary = abap_true ]-tabname_alias ) ).
    ENDIF.
    ...
    IF         if_delete_params = abap_false
       AND NOT line_exists( mt_tables[ has_params = abap_true ] ).
  ...
  METHOD extract_fields.
    ...
    LOOP AT mt_tables ASSIGNING FIELD-SYMBOL(<ls_table>) WHERE tabname_alias IN it_tabname_selopt.
      lt_tables = VALUE #( BASE lt_tables
                           ( <ls_table> ) ).
      DELETE mt_tables.
    ENDLOOP.

    rr_fields = NEW zcl_dbbr_tabfield_list( it_fields = lt_fields
                                            it_tables = lt_tables
                                            iv_mode   = mv_mode ).
  ENDMETHOD.
  ...
  METHOD fill_full_fieldnames.
    cs_field-sql_fieldname_long = COND #( WHEN (    is_multi_table_list( )
                                                 OR mf_multi_table_mode = abap_true )
                                           AND cs_field-tabname_alias IS NOT INITIAL
                                          THEN |{ cs_field-tabname_alias }~{ cs_field-fieldname_raw }|
                                          ELSE cs_field-fieldname_raw ).

    cs_field-sql_fieldname      = COND #( WHEN cs_field-alias IS NOT INITIAL
                                          THEN |{ cs_field-alias }~{ cs_field-fieldname_raw }|
                                          ELSE cs_field-fieldname_raw ).
    " possible crop for alv name
    cs_field-alv_fieldname      = COND #( WHEN cs_field-alias IS NOT INITIAL
                                          THEN |{ cs_field-alias }_{ cs_field-fieldname }|
                                          ELSE cs_field-fieldname ).
  ENDMETHOD.
  ...
  METHOD get_conditional_fields.
    result = VALUE #( FOR <ls_field> IN mt_fields WHERE ( is_virtual_join_field = abap_true )
                      ( <ls_field> ) ).
  ENDMETHOD.
  ...
  METHOD get_field.
    ...
    ELSE.
      rs_tabfield = VALUE #( mt_fields[ tabname_alias = iv_tabname
                                        fieldname     = iv_fieldname ] OPTIONAL ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD get_field_ref.
    IF iv_tabname_alias IS INITIAL.
      rr_tabfield = REF #( mt_fields[ fieldname     = iv_fieldname
                                      is_text_field = if_is_text_field ] ).
    ELSE.
      rr_tabfield = REF #( mt_fields[ KEY unique
  ...
  METHOD get_field_ref_by_alv_name.
    rr_tabfield = REF #( mt_fields[ KEY alv_fieldname
                                    alv_fieldname = iv_alv_fieldname ] ).
  ENDMETHOD.
  ...
  METHOD load_fields.
    ...
    IF lr_s_table IS NOT BOUND.
      RETURN.
    ENDIF.
    IF lr_s_table->fields_are_loaded = abap_true.
      RETURN.
    ENDIF.
...
*&---------------------------------------------------------------------*
  ...
  METHOD sort_in_custom_order.
    CASE mv_mode.
      WHEN zif_dbbr_c_global=>c_field_chooser_modes-output.
        SORT mt_fields BY output_active DESCENDING
                          output_order ASCENDING.

      WHEN zif_dbbr_c_global=>c_field_chooser_modes-selection.
        SORT mt_fields BY alias
                          selection_active DESCENDING
                          selection_order ASCENDING.

      WHEN zif_dbbr_c_global=>c_field_chooser_modes-sort.
        SORT mt_fields BY sort_active DESCENDING
  ...
  METHOD sort_in_ddic_order.
    SORT mt_fields BY alias
                      ddic_order
                      is_text_field ASCENDING.
  ENDMETHOD.

  METHOD sort_tables_by_active.
    SORT mt_tables BY active_selection DESCENDING
                      selection_order ASCENDING.
  ...
  METHOD update_tables.
    ...
    IF         lines( mt_tables ) > 1
       AND NOT line_exists( mt_tables[ alias = space ] )
       AND     if_force_update = abap_false.
      RETURN.
    ENDIF.
...
*&---------------------------------------------------------------------*
    LOOP AT mt_fields ASSIGNING FIELD-SYMBOL(<ls_field>) WHERE has_text_field = abap_true.
      ...
      LOOP AT mt_fields ASSIGNING FIELD-SYMBOL(<ls_text_field>) USING KEY unique WHERE     tabname_alias = <ls_field>-tabname_alias
        ...
        IF <ls_text_field>-output_active = abap_true.
          lv_active_text_field_count = lv_active_text_field_count + 1.
        ENDIF.
  ...
  METHOD zif_uitb_data_ref_list~add.
    TRY.
        ...
        IF NOT line_exists( mt_fields[ KEY unique
                                       tabname_alias = lr_s_tabfield->tabname_alias
    ...
    IF iv_where IS NOT INITIAL.
      ...
      ELSE.
        RAISE EXCEPTION TYPE zcx_uitb_element_not_found
          EXPORTING textid = zcx_uitb_element_not_found=>condition_access
                    index  = iv_index
                    cond   = iv_where.
      ENDIF.
  ...
  METHOD zif_uitb_data_ref_list~get_iterator.
    rr_iterator = zcl_uitb_data_ref_iterator=>create( ir_list  = me
                                                      iv_where = iv_where ).
  ENDMETHOD.
  ...
  METHOD has_virtual_element_fields.
    rf_virtual_element = SWITCH #( if_consider_output_only
                                   WHEN abap_true
                                   THEN xsdbool( line_exists( mt_fields[ is_virtual_element = abap_true ] ) )
