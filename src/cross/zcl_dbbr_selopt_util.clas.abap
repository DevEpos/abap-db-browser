...
*.. Show popup with the options and give one back
  ...
  METHOD create_option_icon.
    DATA(lv_icon_name) = get_option_icon(
        ...
        iv_option = COND #( WHEN iv_option IS INITIAL THEN zif_dbbr_global_consts=>gc_options-default ELSE iv_option )
    ).
  ...
  METHOD get_opt_icon_info.
    ...
    rs_icon-option = COND #( WHEN iv_option IS INITIAL THEN zif_dbbr_global_consts=>gc_options-default ELSE iv_option ).
    DATA(lv_icon_name) = get_option_icon(
