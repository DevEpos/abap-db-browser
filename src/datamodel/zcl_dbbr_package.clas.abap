...
CLASS zcl_dbbr_package IMPLEMENTATION.
  ...
  METHOD get_sub_package_list.
    ...
    IF mf_subpackages_read = abap_false.
      ...
      mr_package_int->get_sub_packages( IMPORTING  e_sub_packages   = DATA(lt_packages)
                                        EXCEPTIONS object_invalid   = 1
                                                   leaf_package     = 2
                                                   unexpected_error = 3
                                                   OTHERS           = 4 ).
      ...
      LOOP AT lt_packages ASSIGNING FIELD-SYMBOL(<lr_package>).
        mr_subpackage_list->add( NEW zcl_dbbr_package( iv_package_name = <lr_package>->package_name
...
*                                            i_check_existence = TPAK_C_FALSE    " Indicator
                                  IMPORTING e_elements = mt_elements ).
  ENDMETHOD.

  METHOD read_package.
    CHECK mr_package_int IS INITIAL.

    cl_package=>load_package( EXPORTING  i_package_name             = mv_package
*                                         i_force_reload             = " Force System to Read from Database (X=Yes)
                              IMPORTING  e_package                  = mr_package_int
