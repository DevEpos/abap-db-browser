...
CLASS zcl_dbbr_formula IMPLEMENTATION.
  METHOD add_field.
    IF is_field-is_description = abap_true.
      update_description_of_field( iv_fieldname         = is_field-field
    ...
    ELSEIF is_field-is_unit = abap_true.
      update_unit_field_of_field( iv_fieldname = is_field-field
                                  iv_unit      = is_field-unit_field ).
    ELSE.
      INSERT is_field INTO TABLE mt_defined_fields.
      IF sy-subrc <> 0.
        RAISE EXCEPTION TYPE zcx_dbbr_formula_exception
          EXPORTING textid = zcx_dbbr_formula_exception=>duplicate_form_field
  ...
  METHOD get_field.
    TRY.
      ...
      CATCH cx_sy_itab_line_not_found.
        RAISE EXCEPTION TYPE zcx_dbbr_formula_exception
          EXPORTING textid = zcx_dbbr_formula_exception=>form_field_not_defined
                    msgv1  = |{ iv_field }|.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD add_stmnt.
    mt_formula_stmnt = VALUE #( BASE mt_formula_stmnt
                                ( is_statement ) ).
  ...
  METHOD evaluate_stmnt.
    IF    is_stmnt-is_function_call = abap_true
       OR is_stmnt-type             = 'U'. " definition of formula field
      mv_executable_stmnt_count = mv_executable_stmnt_count + 1.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
