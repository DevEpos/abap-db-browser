...
CLASS zcl_dbbr_favmenu_factory IMPLEMENTATION.
  ...
  METHOD favorite_exists.
    SELECT SINGLE object_id FROM zdbbr_favmenu
  ...
  METHOD get_favorites.
    ...
    SELECT * FROM zdbbr_favmenu
      INTO CORRESPONDING FIELDS OF TABLE et_favmenu_entries
  ...
  METHOD get_most_used_favorites.
    ...
    SELECT * FROM zdbbr_mostused
      WHERE username = @sy-uname
      ORDER BY changed_date DESCENDING,
               changed_time DESCENDING
  ...
  METHOD get_next_object_id.
    IF if_global = abap_true.
      SELECT MAX( object_id ) FROM zdbbr_favmenu
        INTO @DATA(lv_max_object_id)
        WHERE uname = @space.
    ELSE.
      SELECT MAX( object_id ) FROM zdbbr_favmenu
        INTO @lv_max_object_id
  ...
  METHOD global_favorites_exist.
    SELECT COUNT( * ) FROM zdbbr_favmenu
      INTO @DATA(lv_count)
      WHERE uname = @space.
  ...
  METHOD refresh_most_used.
    ...
    SELECT SINGLE * INTO @DATA(ls_most_used_entry)
      FROM zdbbr_mostused
      WHERE username        = @sy-uname
    ...
    LOOP AT lt_favmenu_all ASSIGNING FIELD-SYMBOL(<ls_favmenu>)
      ...
      LOOP AT GROUP <ls_favmenu_group> ASSIGNING FIELD-SYMBOL(<ls_favmenu_group_entry>).
        ...
        TRY.
            ...
            lt_favmenu_delete = VALUE #( BASE lt_favmenu_delete
                                         ( <ls_favmenu_group_entry> ) ).
  ...
  METHOD update_parent_id_local.
    LOOP AT ct_favmenu ASSIGNING FIELD-SYMBOL(<ls_favmenu>)
      ...
      IF NOT line_exists( ct_deleted_entries[ object_id = <ls_favmenu>-object_id ] ).
        ct_deleted_entries = VALUE #( BASE ct_deleted_entries
                                      ( <ls_favmenu> ) ).
      ENDIF.
      ...
      ELSE.
        ...
        ct_new_entries = VALUE #( BASE ct_new_entries
                                  ( ls_new_favmenu ) ).
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
