...
*&---------------------------------------------------------------------*
  ...
  METHOD fill_corresponding_data.
    ...
    SELECT * FROM zdbbr_tabf
      INTO CORRESPONDING FIELDS OF TABLE cs_variant-fields
      WHERE ref_id = cs_variant-variant_id.

    " fill variant data information
    SELECT * FROM zdbbr_vardata
      INTO CORRESPONDING FIELDS OF TABLE cs_variant-variant_data
      WHERE ref_variant_id = cs_variant-variant_id.
  ENDMETHOD.

  METHOD find_default_query_variant.
    SELECT SINGLE variant_id FROM zdbbr_variant
  ...
  METHOD find_variants.
    DATA(lt_entity_id_sel) = COND #( WHEN iv_entity_id IS NOT INITIAL
                                     THEN VALUE zif_sat_ty_global=>ty_t_selopt(
                                                    ( sign = 'I' option = 'EQ' low = iv_entity_id ) ) ).
    DATA(lt_entity_type_sel) = COND #( WHEN iv_entity_type IS NOT INITIAL
                                       THEN VALUE zif_sat_ty_global=>ty_t_selopt(
                                                      ( sign = 'I' option = 'EQ' low = iv_entity_type ) ) ).

    IF iv_variant_name IS NOT INITIAL.
      DATA(lv_option) = COND #( WHEN contains( val = iv_variant_name
                                               sub = '*' )
                                THEN 'CP'
                                ELSE 'EQ' ).
      DATA(lt_variant_selopt) = VALUE zif_sat_ty_global=>ty_t_selopt(
                                          ( sign = 'I' option = lv_option low = iv_variant_name ) ).
    ENDIF.

    SELECT * FROM zdbbr_variant
      INTO CORRESPONDING FIELDS OF TABLE et_variant_info
  ...
  METHOD find_variant_infos_for_type.
    ...
    IF iv_entity_type = zif_sat_c_entity_type=>query.
      NEW zcl_dbbr_query_factory( )->find_queries( EXPORTING iv_query_name = lv_entity_id
                                                   IMPORTING et_queries    = DATA(lt_queries) ).
  ...
  METHOD get_variant.
    " TODO: parameter IF_LOAD_COMPLETELY is never used (ABAP cleaner)

    IF iv_variant_id IS NOT INITIAL.
      " read variant for technical key
      SELECT SINGLE * FROM zdbbr_variant
        INTO CORRESPONDING FIELDS OF es_variant
  ...
  METHOD get_automatic_variant.
    ...
    " read variant for technical key
    SELECT SINGLE * FROM zdbbr_variantaf
  ...
  METHOD get_default_variant.
    " read variant for technical key
    SELECT SINGLE * FROM zdbbr_variantd
      WHERE entity_id   = @iv_entity_id
        AND entity_type = @iv_entity_type
        AND created_by  = @sy-uname
      INTO CORRESPONDING FIELDS OF @rs_variant.
    ...
    IF sy-subrc <> 0.
      RETURN.
    ENDIF.
    IF if_load_completely = abap_false.
      RETURN.
    ENDIF.
  ...
  METHOD delete_default_variant.
    SELECT SINGLE variant_id FROM zdbbr_variantd
      WHERE entity_id   = @iv_entity_id
        AND entity_type = @iv_entity_type
        AND created_by  = @sy-uname
  ...
  METHOD delete_auto_variant.
    SELECT SINGLE variant_id FROM zdbbr_variantaf
      WHERE entity_id   = @iv_entity_id
        AND entity_type = @iv_entity_type
        AND created_by  = @sy-uname
...
*&---------------------------------------------------------------------*
    ...
    SELECT COUNT( * ) FROM zdbbr_variant
      INTO @DATA(lv_count)
  ...
  METHOD default_variant_exists.
    SELECT SINGLE @abap_true FROM zdbbr_variantd
      WHERE entity_id   = @iv_entity_id
        AND entity_type = @iv_entity_type
      INTO @rf_exists.
  ENDMETHOD.
