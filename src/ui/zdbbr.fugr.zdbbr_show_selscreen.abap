...
*"----------------------------------------------------------------------
  ...
  CALL FUNCTION 'RFC_GET_ATTRIBUTES'
    IMPORTING  caller_destination        = lv_caller_destination
    EXCEPTIONS system_call_not_supported = 1
  ...
  IF if_skip_selscreen = abap_true.
    DATA(lr_variant_starter) = zcl_dbbr_variant_starter_fac=>create_variant_starter(
        iv_variant_id        = COND #( WHEN iv_variant_id IS NOT INITIAL
                                       THEN iv_variant_id
                                       ELSE zif_dbbr_c_global=>c_dummy_variant )
  ...
  gr_selscreen_controller = NEW #( iv_mode                = lv_entity_type
  ...
  IF gs_data-called_from_adt = abap_true.
    gr_selscreen_controller->load_entity( if_fill_history = abap_true
