...
DATA gv_entity_type TYPE char20.
DATA gv_entity_type_sh TYPE zsat_entity_type.
DATA gv_selmask_entity_text TYPE ddtext.
...
DATA gt_multi_or TYPE zdbbr_selfield_itab.
DATA gs_multi_or TYPE zdbbr_selfield.
DATA gv_or_tuple_number TYPE zdbbr_seq_number.
DATA gs_selfields TYPE zdbbr_selfield.
DATA gv_selfields_title TYPE char128.
DATA gt_selection_fields_multi TYPE zdbbr_selfield_itab.
DATA gv_linecount LIKE sy-tabix.
DATA gt_multi_select TYPE zdbbr_selfield_itab.
DATA gs_multi_select TYPE zdbbr_selfield.
DATA gv_fieldname TYPE fieldname.
DATA gv_field_descr TYPE scrtext_m.
DATA gv_multi_select_lines LIKE sy-tabix.
DATA gv_option_template TYPE ddoption.
...
DATA ok_code_multi LIKE sy-ucomm.
DATA save_code_multi LIKE sy-ucomm.

" dynamic texts for gui status
DATA gs_sel_text_gui_text TYPE smp_dyntxt.
DATA multi_or_icon TYPE smp_dyntxt.
DATA gs_sel_text_gui_queryeditshow TYPE smp_dyntxt.
DATA entity_top_menu TYPE smp_dyntxt.
DATA save_dyn TYPE smp_dyntxt.
DATA savenew_dyn TYPE smp_dyntxt.
DATA entityfc01 TYPE smp_dyntxt.
DATA entityfc02 TYPE smp_dyntxt.
DATA entityfc03 TYPE smp_dyntxt.
DATA entityfc04 TYPE smp_dyntxt.
DATA entityfc05 TYPE smp_dyntxt.
DATA entityfc11 TYPE smp_dyntxt.
DATA entityfc12 TYPE smp_dyntxt.

""" data for query manager
" ---------------------------------------------------------------------
DATA gv_tree_screen TYPE sy-dynnr.
DATA gv_active_tab TYPE sy-ucomm.

""" data for first screen input (query / table mode)
" ---------------------------------------------------------------------
DATA gs_browser_mode TYPE zdbbr_browser_mode_data.
DATA gs_entity_info TYPE zdbbr_entity_info_simple.
DATA gs_entity TYPE zdbbr_browser_mode_data.

""" data for alternative columns texts
DATA gt_altcoltext TYPE zdbbr_altcoltext_data_itab.
DATA gs_altcoltext TYPE zdbbr_altcoltext_data.
" ---------------------------------------------------------------------
...
DATA gr_edit_join_table_controller TYPE REF TO zcl_dbbr_edit_join_table_ctrl.
DATA gr_edit_join_cond_view TYPE REF TO zcl_dbbr_edit_join_cond_view.
DATA gr_choose_entity_controller TYPE REF TO zcl_dbbr_choose_object_ctrl.
DATA gr_f4_screen_controller TYPE REF TO zcl_dbbr_generic_f4_sc.
DATA gr_multi_select_controller TYPE REF TO zcl_dbbr_multi_select_ctlr.
DATA gr_multi_select_table TYPE REF TO zcl_dbbr_multi_select_table.
DATA gr_selscreen_controller TYPE REF TO zcl_dbbr_selscreen_controller.
DATA gr_selscreen_table TYPE REF TO zcl_dbbr_selscreen_table.
DATA gr_sort_controller TYPE REF TO zcl_dbbr_field_sorter_ctrl.
DATA gr_save_query_controller TYPE REF TO zcl_dbbr_save_query_ctrl.
DATA gr_save_sql_query_controller TYPE REF TO zcl_dbbr_save_sql_query_ctrl.
DATA gr_variant_controller TYPE REF TO zcl_dbbr_variant_controller.
DATA gr_multi_or_table TYPE REF TO zcl_dbbr_multi_or_table.
DATA gr_multi_or_controller TYPE REF TO zcl_dbbr_multi_or_controller.
DATA gr_addtextfield_controller TYPE REF TO zcl_dbbr_addtextfield_ctrl.
DATA gr_copy_query_controller TYPE REF TO zcl_dbbr_copy_query_ctrl.
DATA gr_altcoltext_controller TYPE REF TO zcl_dbbr_altcoltxt_controller.
DATA gr_obj_brws_search_enter_ctrl TYPE REF TO zcl_dbbr_obj_brws_search_sc.
DATA gr_altcoltext_table TYPE REF TO zcl_dbbr_altcoltext_table.
...
SELECTION-SCREEN BEGIN OF SCREEN 1100 TITLE TEXT-t01 AS WINDOW.
  ...
  SELECTION-SCREEN BEGIN OF BLOCK user_param WITH FRAME TITLE TEXT-t02 NO INTERVALS.
    PARAMETERS: p_xsort  TYPE zdbbr_defined_sortfields AS CHECKBOX MODIF ID par,
                ...
                p_xcolwd TYPE zdbbr_has_def_col_widths AS CHECKBOX MODIF ID alv.
...
SELECTION-SCREEN BEGIN OF SCREEN 1200 TITLE tvar_t.
  ...
  SELECTION-SCREEN BEGIN OF BLOCK variant.
    SELECTION-SCREEN BEGIN OF LINE.
      SELECTION-SCREEN COMMENT 1(10) TEXT-t13 FOR FIELD p_varnam MODIF ID vrn.
      SELECTION-SCREEN POSITION 25.
      PARAMETERS p_varnam TYPE zdbbr_variant_name OBLIGATORY MODIF ID vrn.
    SELECTION-SCREEN END OF LINE.
    SELECTION-SCREEN BEGIN OF LINE.
