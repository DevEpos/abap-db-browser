CLASS zcl_dbbr_object_history_tree DEFINITION
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Builds history tree</p>
    "!
    METHODS build_tree.

    "! <p class="shorttext synchronized">Clears all nodes from the tree</p>
    "!
    METHODS clear_nodes
      IMPORTING
    ...
    "!
    METHODS create_tree.
    "! <p class="shorttext synchronized">Delete selected nodes</p>
    "!
    METHODS delete_selected_nodes.
    "! <p class="shorttext synchronized">Fill toolbar with buttons</p>
    "!
    METHODS fill_toolbar.

    "! <p class="shorttext synchronized">Handle function</p>
    "!
    ...
    "! <p class="shorttext synchronized">Loads history nodes</p>
    "!
    METHODS load_nodes.
    ...
    "! <p class="shorttext synchronized">Start node with default variant</p>
    "!
    METHODS start_with_default_variant
      IMPORTING
        iv_node_key TYPE tm_nodekey.

    "! <p class="shorttext synchronized">Shows history entry in object browser</p>
    "!
...
CLASS zcl_dbbr_object_history_tree IMPLEMENTATION.
  ...
  METHOD create_tree.
    ...
    mo_tree = NEW zcl_uitb_column_tree_model( ir_parent           = mo_parent_container
                                              ...
                                              is_hierarchy_header = VALUE #( heading = |{ 'Object Name'(002) }| )
  ...
  METHOD delete_selected_nodes.
    ...
    DATA lt_nodes_to_delete TYPE treemnotab.
    ...
    LOOP AT lt_selected_nodes ASSIGNING FIELD-SYMBOL(<lo_node>).
      ...
      TRY.
          ...
          lt_mostused_to_delete = VALUE #( BASE lt_mostused_to_delete
                                           ( username        = sy-uname
                                             type            = lr_most_used->type
                                             most_used_entry = lr_most_used->most_used_entry ) ).
          lt_nodes_to_delete = VALUE #( BASE lt_nodes_to_delete
                                        ( <lo_node>->mv_node_key ) ).
  ...
  METHOD fill_toolbar.
    ...
    lo_toolbar->add_button( iv_fcode = c_context_codes-delete_list
                            iv_icon  = icon_delete
                            iv_text  = |{ 'Delete all'(001) }| ).
    lo_toolbar->add_button( iv_fcode = c_context_codes-refresh
                            iv_icon  = icon_refresh
                            iv_text  = |{ 'Refresh'(003) }| ).
    lo_toolbar->add_button( iv_fcode     = c_context_codes-full_history_flag
  ...
  METHOD load_nodes.
    ...
    LOOP AT mo_favmenu_f->get_most_used_favorites( if_all = mf_full_history ) ASSIGNING FIELD-SYMBOL(<ls_most_used>).
      ...
      DATA(lo_history_node) = mo_tree->get_nodes( )->add_node( if_folder         = <ls_most_used>-has_variants
  ...
  METHOD on_expand_no_children.
    ...
    LOOP AT lt_variants ASSIGNING FIELD-SYMBOL(<ls_variant>).
      ...
      DATA(lo_variant_node) = mo_tree->get_nodes( )->add_node(
                                  ...
                                  it_item_table        = VALUE #( ( item_name = mo_tree->c_hierarchy_column
                                                                    class     = cl_item_tree_model=>item_class_text
                                                                    font      = cl_item_tree_model=>item_font_prop
  ...
  METHOD on_node_context_menu_request.
    ...
    er_menu->add_function( fcode = c_context_codes-delete_entry
    ...
    IF lines( lt_selected_nodes ) = 1.
      ...
      er_menu->add_function( fcode = c_context_codes-show_in_object_list
  ...
  METHOD on_node_double_click.
    ...
    IF <ls_node_map>-is_variant = abap_true.
      ...
      zcl_dbbr_selscr_nav_events=>raise_variant_entry_chosen( iv_entity_id   = lr_variant_info->entity_id
    ...
    ELSE.
      ...
      zcl_dbbr_selscr_nav_events=>raise_entity_chosen( iv_entity_id   = lr_most_used_entry->most_used_entry
  ...
  METHOD on_node_enter_key.
    ...
    ELSEIF ev_key = cl_list_tree_model=>key_delete.
      on_node_context_menu_select( ev_fcode    = c_context_codes-delete_entry
  ...
  METHOD show_in_object_browser.
    ...
    IF mo_parent_view IS BOUND.
      mo_parent_view->execute_command( NEW zcl_uitb_gui_simple_command(
                                               ...
                                               ir_params   = NEW zsat_entity(
                                                                     entity_id   = <ls_node_map>-entity_id
  ...
  METHOD start_with_default_variant.
    ...
    DATA lv_variant TYPE zdbbr_variant_id VALUE zif_dbbr_c_global=>c_dummy_variant.
    ...
    zcl_dbbr_selscr_nav_events=>raise_variant_entry_chosen( iv_entity_id    = lv_entity_id
