...
CLASS zcl_dbbr_favorites_tree DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized" lang="en">Frees all used controls and removes the tree from the screen</p>
    METHODS free .
  ...
  PRIVATE SECTION.
    ...
    METHODS create_multiple_faventries
      IMPORTING
        ...
        !ir_selected_node        TYPE REF TO zcl_uitb_ctm_node
...
*.. create dummy favmenu entry data for top node
    ...
    DATA(lr_root_dnd_behavior) = NEW cl_dragdrop( ).

    lr_root_dnd_behavior->add(
        flavor          = 'Node'
        dragsrc         = abap_false
        droptarget      = abap_true
        effect          = cl_dragdrop=>move
    ).
    lr_root_dnd_behavior->get_handle( IMPORTING handle = DATA(lv_dnd_handle) ).
...
*.. fill tree with saved nodes
    ...
    lr_nodes->expand_node( c_top_node ).

    mr_tree_model->get_selections( )->select_nodes( VALUE #( ( c_top_node ) ) ).
  ...
  METHOD collapse_node.
    DATA(lt_selected_node) = mr_tree_model->get_selections( )->get_selected_nodes( ).

    CHECK: lt_selected_node IS NOT INITIAL,
           lines( lt_selected_node ) = 1.

    DATA(lr_selected_node) = lt_selected_node[ 1 ].

    IF lr_selected_node IS INITIAL OR NOT lr_selected_node->is_folder( ).
      RETURN.
    ENDIF.

    " expand the node
    mr_tree_model->get_nodes( )->collapse_node(
      iv_node_key         = lr_selected_node->mv_node_key
  ...
  METHOD constructor.
    mr_favmenu_f = NEW zcl_dbbr_favmenu_factory( ).
    mr_query_f = NEW #( ).
    mr_variant_f = NEW #( ).
    mr_parent_container = ir_parent.
  ...
  METHOD create_multiple_faventries.
    ...
    DATA(lv_relative_key) = ir_selected_node->mv_node_key.
    DATA(lv_new_fav_id) = mr_favmenu_f->get_next_object_id( if_global = mf_global_fav_mode ).
...
*.... add node to node table
      ...
      mr_tree_model->get_nodes( )->add_node(
...
*.... fill new id values
    ...
    mr_favmenu_f->update_favorites( lt_favmenu_new ).
  ENDMETHOD.
  ...
  METHOD create_new_favorite.
    ...
    IF lr_selected_node IS INITIAL OR
       zcl_dbbr_tree_helper=>is_variant_node( lr_selected_node ).
      RETURN.
    ENDIF.

    IF NOT favorite_exists_in_node( lr_selected_node ).
    ...
    ELSE.
      DATA(lr_choose_tab_query_ctrl) = NEW zcl_dbbr_choose_object_ctrl(
              if_global_fav_mode = mf_global_fav_mode
              if_for_fav_menu    = abap_true
          ).
      lr_choose_tab_query_ctrl->zif_uitb_screen_controller~call_screen( ).

      IF NOT lr_choose_tab_query_ctrl->zif_uitb_screen_controller~was_not_cancelled( ).
        RETURN.
      ENDIF.

      lr_choose_tab_query_ctrl->get_chosen_entry(
    ...
    IF lr_selected_node->is_folder( ).
      ...
      DATA(lr_parent_favnode) = lr_selected_node.
    ELSE.
      lv_relationship = cl_list_tree_model=>relat_next_sibling.
      lr_parent_favnode = lr_selected_node->get_parent( ).
    ENDIF.

    DATA(ls_parent_favnode_data) = get_node_favmenu_data( lr_parent_favnode ).
    ...
    IF lv_node_description IS INITIAL OR iv_favorite_raw IS INITIAL.
      CASE lv_favorite_type.
        ...
        WHEN zif_dbbr_c_favmenu_type=>query.
          ...
          DATA(ls_query) = lr_query_f->get_query( iv_query_name = lv_favorite_name ).
    ...
    mr_favmenu_f->update_favorite( ls_favmenu_entry_new ).
    ...
    mr_tree_model->get_nodes( )->add_node(
      ...
      iv_relative_node_key       = lr_selected_node->mv_node_key
      iv_relationship            = lv_relationship
    ...
    mr_tree_model->get_selections( )->select_nodes( VALUE #( ( lv_new_node_key ) ) ).

    " update sort order of all childs
    update_sortorder_of_children( lr_parent_favnode->mv_node_key ).
...
*.. set event handler methods to tree control
    SET HANDLER:
      on_tree_drag FOR lr_events,
      on_tree_drag_multiple FOR lr_events,
      on_tree_drop_complete_multiple FOR lr_events,
      on_tree_drop FOR lr_events,
      on_drop_complete FOR lr_events,
  ...
  METHOD delete_favorite.
    DATA: lv_answer                  TYPE char1,
          lr_selected_node           TYPE REF TO zcl_uitb_ctm_node,
          lv_new_node_after_deletion TYPE tm_nodekey.

    IF ir_node IS BOUND.
      lr_selected_node = ir_node.
    ELSE.
      ...
      ELSEIF lines( lt_selected_nodes ) = 1.
        lr_selected_node = lt_selected_nodes[ 1 ].
      ENDIF.
    ENDIF.

    IF lr_selected_node IS INITIAL OR
       lr_selected_node->mv_node_key = c_top_node OR
       zcl_dbbr_tree_helper=>is_variant_node( lr_selected_node ).
      RETURN.
    ENDIF.
    ...
    IF lr_selected_node->has_children( ).
...
*.. 1) delete favorites from ZDBBR_favmenu
    delete_favorite_node( lr_selected_node ).

*.. 2) determine new selection node
    DATA(lr_previous_node) = lr_selected_node->get_previous_sibling( ).
    IF lr_previous_node IS NOT INITIAL.
      lv_new_node_after_deletion = lr_previous_node->mv_node_key.
    ELSE.
*.... get the parent node
      lv_new_node_after_deletion = lr_selected_node->get_parent( )->mv_node_key.
    ENDIF.

*.. 3) delete the node and all subnodes
    mr_tree_model->get_nodes( )->delete_node( lr_selected_node->mv_node_key ).

    IF if_update_selection = abap_true.
*.... 4) select new node
      mr_tree_model->get_selections( )->select_nodes( VALUE #( ( lv_new_node_after_deletion ) ) ).
    ENDIF.
  ENDMETHOD.
...
*.. 1) delete all sub nodes (recursively first
    IF ir_selected_node->has_children( ).
      LOOP AT ir_selected_node->get_children( ) ASSIGNING FIELD-SYMBOL(<lr_child>).
        delete_favorite_node( <lr_child> ).
      ENDLOOP.
    ENDIF.

*.. delete entry from node map
    DELETE mt_node_map WHERE node_key = ir_selected_node->mv_node_key.

*.. 2) delete the node itself
    mr_favmenu_f->delete_favorite( get_node_favmenu_data( ir_selected_node ) ).
  ENDMETHOD.


  METHOD delete_selected_nodes.
    DATA(lt_nodes) = mr_tree_model->get_selections( )->get_selected_nodes( ).
    DATA(lv_node_count) = lines( lt_nodes ).
    LOOP AT lt_nodes ASSIGNING FIELD-SYMBOL(<lr_node>).
      delete_favorite(
          ir_node             = <lr_node>
  ...
  METHOD edit_favorite.
    ...
    IF lr_selected_node IS INITIAL OR
       lr_selected_node->mv_node_key = c_top_node.
      RETURN.
    ENDIF.
    ...
    DATA(ls_favmenu_data) = get_node_favmenu_data( lr_selected_node ).
    ...
    IF lr_selected_node->is_folder( ).
      lr_selected_node->get_item( mr_tree_model->c_hierarchy_column )->set_text( |{ lv_new_favorite_text }| ).
    ELSE.
      lr_selected_node->get_item( c_hierarchy_node2 )->set_text( |{ lv_new_favorite_text }| ).
    ENDIF.

    mr_favmenu_f->update_favorite( ls_favmenu_data ).
    lr_selected_node->set_user_object( NEW zcl_dbbr_favmenu_entry( ls_favmenu_data ) ).
...
*.. expand the node
  ...
  METHOD favorite_exists_in_node.
    IF ir_node->mv_node_key = c_top_node.
      rf_fav_exists = abap_true.
      RETURN.
    ENDIF.

    rf_fav_exists = mr_favmenu_f->favorite_exists(
      get_node_favmenu_data( ir_node )
    ).
  ENDMETHOD.
  ...
  METHOD fill_toolbar.
    ...
    lr_toolbar->add_buttons( lt_buttons ).
...
*... clear all nodes
  ...
  METHOD get_node_favmenu_data.
    ...
    TRY.
        rs_favmenu = CAST zcl_dbbr_favmenu_entry( lr_user_object )->get_favmenu_data( ).
  ...
  METHOD get_selected_favorite.
    rr_node = mr_tree_model->get_selections( )->get_selected_node( ).
  ENDMETHOD.
...
*.. 7) create node/favmenu entries for all db tables in package
    DATA(lv_first_new_node) = create_multiple_faventries(
        ...
        ir_selected_node       = lr_selected_node
        is_parent_favmenu      = ls_parent_favnode_data
        iv_parent_relationship = lv_relationship
    ).

*.. expand  select the newly created folder
    mr_tree_model->get_selections( )->select_nodes( VALUE #( ( lv_first_new_node ) ) ).
...
*.. 1) get the selected node
    ...
    DATA(lr_next_sibling) = lr_selected_node->get_next_sibling( ).
    IF lr_next_sibling IS INITIAL.
      RETURN.
    ENDIF.

    lv_relationship = cl_list_tree_model=>relat_next_sibling.
    lv_relatkey = lr_next_sibling->mv_node_key.

    lr_selected_node->move_node_to(
        iv_relative_node    = lv_relatkey
        iv_relationship     = lv_relationship
    ).

*.. update sort order of sub nodes of parent
    update_sortorder_of_children( lr_selected_node->get_parent( )->mv_node_key ).

    mr_tree_model->get_selections( )->select_nodes( VALUE #( ( lr_selected_node->mv_node_key ) ) ).
  ENDMETHOD.
...
*.. 1) get the selected node
    ...
    DATA(lr_previous_sibling) = lr_selected_node->get_previous_sibling( ).
    IF lr_previous_sibling IS INITIAL.
      RETURN.
    ENDIF.

    lr_selected_node->move_node_to(
      iv_relationship  = cl_tree_model=>relat_prev_sibling
      iv_relative_node = lr_previous_sibling->mv_node_key
    ).
...
*.. update sort order of sub nodes of parent
  ...
  METHOD on_entity_deleted.
    ...
    mr_tree_model->get_nodes( )->delete_nodes( lt_nodes ).
  ENDMETHOD.
  ...
  METHOD on_node_context_menu_request.
    ...
    CHECK NOT zcl_dbbr_tree_helper=>is_variant_node( lr_selected_node ).

    DATA(lr_parent_node) = lr_selected_node->get_parent( ).
    DATA(lv_parent_node) = COND #( WHEN lr_parent_node IS BOUND THEN lr_parent_node->mv_node_key ).

    DATA(lt_expanded_nodes) = mr_tree_model->get_nodes( )->get_expanded_nodes( ).
...
*... create new action for importing db tables from package
    ...
    IF NOT lr_selected_node->is_folder( ).
      er_menu->add_separator( ).
...
*.. Get the selected node from the tree
    ...
    DATA(lr_user_object) = lr_selected_node->get_user_object( ).
    IF lr_selected_node->is_folder( ).
*.... Check whether the node is truly a folder or just a favorite with variants
      IF lr_user_object IS BOUND.
        DATA(lv_fav_type) = CAST zcl_dbbr_favmenu_entry( lr_user_object )->get_favmenu_data( )-favtype.
    ...
    IF lf_is_favorite = abap_true.
      IF zcl_dbbr_tree_helper=>is_variant_node( lr_selected_node ).
...
*.... check if folder is expanded/collapsed
      IF lr_selected_node->is_expanded( ).
        collapse_node( ).
      ELSE.
        expand_node( ).
      ENDIF.
...
*.. first check if the node to be dragged still exists
    IF NOT favorite_exists_in_node( mr_tree_model->get_nodes( )->get_node( ev_node_key ) ).
...
*.. create drag and drop object
  ...
  METHOD on_tree_drop.
    ...
    DATA(lr_nodes) = mr_tree_model->get_nodes( ).
    DATA(lr_target_node) = lr_nodes->get_node( ev_node_key ).

*.. does the target still exist
    IF NOT favorite_exists_in_node( lr_target_node ).
      er_drag_drop_object->abort( ).
      RETURN.
    ENDIF.

    DATA(ls_favmenu_target) = get_node_favmenu_data( lr_target_node ).

    IF lr_target_node->is_folder( ).
...
*.... also read the parent of the target node
      DATA(lr_target_parent_node) = lr_nodes->get_node( lr_target_node->get_parent( )->mv_node_key ).
      DATA(ls_favmenu_target_parent) = get_node_favmenu_data( lr_target_parent_node ).
    ENDIF.

*.. get drag and drop object
    TRY.
        DATA(lr_dnd_object) = CAST zcl_dbbr_tree_dnd_object( er_drag_drop_object->object ).

        DATA(lv_source_node) = lr_dnd_object->get_next_node( ).
        DATA(lr_source_node) = lr_nodes->get_node( lv_source_node ).

        lr_source_node->move_node_to(
            iv_relative_node    = lr_target_node->mv_node_key
            iv_relationship     = lv_relationship
        ).
      CATCH cx_sy_move_cast_error.
        er_drag_drop_object->abort( ).
...
*.. drop was successful
    DATA(ls_favmenu_source) = get_node_favmenu_data( lr_source_node ).

*...-> check which attributes of the favmenu entry need to be adjusted
*.. 1) has the parent changed of the source
    IF lr_source_node->get_parent( )->mv_node_key = lr_target_node->mv_node_key.
*.... only sort order must be changed
      update_sortorder_of_children( lr_target_node->mv_node_key ).

    ELSE.
*.... source gets new parent -> delete current favmenu entry and create new one
      mr_favmenu_f->delete_favorite( ls_favmenu_source ).
      ls_favmenu_source-parent_id = COND #( WHEN lr_target_node->is_folder( ) THEN
                                              ls_favmenu_target-object_id
                                            ELSE
                                              ls_favmenu_target_parent-object_id ).
      ls_favmenu_source-menu_level = lv_menulevel.
      mr_favmenu_f->update_favorite( ls_favmenu_source ).
      lr_source_node->set_user_object( NEW zcl_dbbr_favmenu_entry( ls_favmenu_source ) ).
...
*.... check which node's children need an updated sort order
      IF lr_target_node->is_folder( ).
        update_sortorder_of_children( lr_target_node->mv_node_key ).
      ELSE.
        update_sortorder_of_children( lr_target_parent_node->mv_node_key ).
      ENDIF.
    ENDIF.
  ...
  METHOD set_node_favmenu_data.
    ir_node->set_user_object( NEW zcl_dbbr_favmenu_entry( is_favmenu ) ).
  ENDMETHOD.
  ...
  METHOD start_with_default_variant.
    ...
    DATA(lr_selected_node) = get_selected_favorite( ).
    CHECK lr_selected_node IS NOT INITIAL.

    DATA(lr_user_object) = lr_selected_node->get_user_object( ).
    CHECK lr_user_object IS NOT INITIAL.
    DATA(lr_fav_entry) = CAST zcl_dbbr_favmenu_entry( lr_user_object ).

    DATA(ls_favmenu_data) = lr_fav_entry->get_favmenu_data( ).
  ...
  METHOD update_menulevel_of_children.
    DATA(lr_node) = mr_tree_model->get_nodes( )->get_node( iv_node_key ).

    CHECK lr_node->is_folder( ).

*.. starting menulevel from node
    DATA(ls_favmenu_data) = get_node_favmenu_data( lr_node ).
    DATA(lv_menulevel) = ls_favmenu_data-menu_level + 1.

    LOOP AT lr_node->get_children( ) ASSIGNING FIELD-SYMBOL(<lr_child>).
      ls_favmenu_data = get_node_favmenu_data( <lr_child> ).
      ls_favmenu_data-menu_level = lv_menulevel.
      mr_favmenu_f->update_favorite( ls_favmenu_data ).
      <lr_child>->set_user_object( NEW zcl_dbbr_favmenu_entry( ls_favmenu_data ) ).

*.... also update child nodes
      IF <lr_child>->is_folder( ).
        update_menulevel_of_children( <lr_child>->mv_node_key ).
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD update_sortorder_of_children.
    ...
    DATA(lr_node) = mr_tree_model->get_nodes( )->get_node( iv_node_key  = iv_node_key ).

    DATA(lv_sortorder) = 10.
    DATA(lr_favmenu_f) = NEW zcl_dbbr_favmenu_factory( ).

    LOOP AT lr_node->get_children( ) ASSIGNING FIELD-SYMBOL(<lr_child>).
      DATA(ls_favmenu_data) = get_node_favmenu_data( <lr_child> ).
      ls_favmenu_data-sort_order = lv_sortorder.

      lr_favmenu_f->update_favorite( ls_favmenu_data ).
      <lr_child>->set_user_object( NEW zcl_dbbr_favmenu_entry( ls_favmenu_data ) ).
