"! <p class="shorttext synchronized">Favorites tree controller</p>
CLASS zcl_dbbr_favorites_tree DEFINITION
  ...
  PUBLIC SECTION.
    ...
    ALIASES free      FOR zif_uitb_gui_control~free.
    ...
    "! <p class="shorttext synchronized">Build/Re Build the favorite tree</p>
    METHODS build_tree
      IMPORTING
    ...
    "! <p class="shorttext synchronized">Collapse the current node</p>
    ...
    "! <p class="shorttext synchronized">Create new folder</p>
    METHODS create_new_fav_folder.
    "! <p class="shorttext synchronized">Creates the tree</p>
    METHODS create_tree.
    "! <p class="shorttext synchronized">Edits the current selected favorite</p>
    METHODS edit_favorite.

    "! <p class="shorttext synchronized">Expands the current node</p>
    METHODS expand_node
      IMPORTING
        if_expand_all TYPE boolean OPTIONAL.

    "! <p class="shorttext synchronized">Retrieves the currently selected node</p>
    METHODS get_selected_favorite
      RETURNING
        VALUE(rr_node) TYPE REF TO zcl_uitb_ctm_node.

    "! <p class="shorttext synchronized">Moves the current entry one position down</p>
    METHODS move_favorite_down.
    "! <p class="shorttext synchronized">Moves the current entry one position up</p>
    METHODS move_favorite_up.

    "! <p class="shorttext synchronized">Shows the tree</p>
    METHODS show
      IMPORTING
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Creates multiple favorite entries from table entries</p>
    ...
    "! <p class="shorttext synchronized">Deletes the current favorite</p>
    ...
    "! <p class="shorttext synchronized">Deletes several selected favorite entries</p>
    METHODS delete_selected_nodes.

    "! <p class="shorttext synchronized">Checks if the favorite exists in the node</p>
    ...
    "! <p class="shorttext synchronized">Fill the toolbar with functions</p>
    METHODS fill_toolbar.

    "! <p class="shorttext synchronized">Gets favorite menu data for node</p>
    ...
    "! <p class="shorttext synchronized">Get the selection mode</p>
    ...
    "! <p class="shorttext synchronized">handles a ui function code</p>
    "!
    ...
    "! <p class="shorttext synchronized">Loads all favorite nodes into the tree</p>
    METHODS load_favorite_nodes
      RAISING
        zcx_uitb_tree_error.

    "! <p class="shorttext synchronized">Handler for when drop completed</p>
    ...
    "! <p class="shorttext synchronized">Handler for deleted entity -&gt; remove invalid nodes</p>
    ...
    "! <p class="shorttext synchronized">Handler for Tree drag was started</p>
    ...
    "! <p class="shorttext synchronized">Handler fro Multiple Tree drag was started</p>
    ...
    "! <p class="shorttext synchronized">Handler for a single drop</p>
    ...
    "! <p class="shorttext synchronized">Handler for multiple drops completed</p>
    ...
    "! <p class="shorttext synchronized">Checks if node should be displayed as folder</p>
    ...
    "!
    "! @parameter io_node             | <p class="shorttext synchronized"></p>
    "! @parameter if_update_selection | <p class="shorttext synchronized"></p>
    ...
    "! <p class="shorttext synchronized">Start entity behind favorite with default variant</p>
    METHODS start_with_default_variant.

    "! <p class="shorttext synchronized">Updates the menulevel of the child nodes of a node</p>
    METHODS update_menulevel_of_children
      IMPORTING
        iv_node_key TYPE tm_nodekey.

    "! <p class="shorttext synchronized">Updates the sort order of the child nodes of a node</p>
    METHODS update_sortorder_of_children
      IMPORTING
        iv_node_key TYPE tm_nodekey.

    "! <p class="shorttext synchronized">Import favorites from Clipboard</p>
    "!
    "! <p class="shorttext synchronized">Import favorites from Clipboard</p>
...
CLASS zcl_dbbr_favorites_tree IMPLEMENTATION.
  METHOD build_tree.
    ...
    DATA(lv_node_text) = COND string( WHEN if_global = abap_true
                                      THEN 'Favorites (Global)'
                                      ELSE |Favorites for { sy-uname }| ).

    " .. create dummy favmenu entry data for top node
    DATA(lr_favmenu_data) = NEW zdbbr_favmenu( uname      = COND #( WHEN mf_global_fav_mode = abap_false THEN sy-uname )
    ...
    lo_root_dnd_behavior->add( flavor     = 'Node'
                               dragsrc    = abap_false
    ...
    lo_nodes->add_node( iv_node_key     = c_top_node
                        if_folder       = abap_true
                        ir_user_data    = lr_favmenu_data
                        iv_drag_drop_id = lv_dnd_handle
                        it_item_table   = VALUE #( ( item_name = zcl_uitb_column_tree_model=>c_hierarchy_column
  ...
  METHOD collapse_node.
    ...
    " expand the node
    mo_tree_model->get_nodes( )->collapse_node( iv_node_key         = lo_selected_node->mv_node_key
  ...
  METHOD create_multiple_faventries.
    ...
    LOOP AT it_favinfo ASSIGNING FIELD-SYMBOL(<ls_tabinfo>).
      DATA(lr_favmenu_entry) = NEW zdbbr_favmenu( uname         = lv_user
      ...
      lt_favmenu_new = VALUE #( BASE lt_favmenu_new
                                ( lr_favmenu_entry->* ) ).
      ...
      mo_tree_model->get_nodes( )->add_node(
          ...
          it_item_table        = VALUE #( class = cl_item_tree_model=>item_class_text
  ...
  METHOD create_new_favorite.
    ...
    DATA lv_favorite_name TYPE tabname.
    DATA lv_favorite_name_raw TYPE zsat_entity_id_raw.
    DATA lv_favorite_type TYPE zsat_favmenu_type.

    DATA(lo_selected_node) = mo_tree_model->get_selections( )->get_selected_node( ).

    IF    lo_selected_node IS INITIAL
       OR is_variant_node( lo_selected_node ).
      RETURN.
    ENDIF.
    ...
    ELSE.
      DATA(lo_choose_tab_query_ctrl) = NEW zcl_dbbr_choose_object_ctrl( if_global_fav_mode = mf_global_fav_mode
      ...
      lo_choose_tab_query_ctrl->get_chosen_entry( IMPORTING ev_entry = lv_favorite_name
  ...
  METHOD create_new_fav_folder.
    ...
    DATA(lv_folder_name) = zcl_uitb_pgv_factory=>create_single_field_popup( iv_title = 'Enter name of folder'
                                                                            is_field = VALUE #(
                                                                                tabname   = 'dd03t'
                                                                                fieldname = 'ddtext'
    ...
    mo_tree_model->get_nodes( )->add_node( iv_node_key          = lv_new_node_key
                                           iv_relative_node_key = lo_selected_node->mv_node_key
                                           iv_relationship      = lv_relationship
                                           if_folder            = abap_true
                                           iv_drag_drop_id      = lv_dnd_handle
...
*&---------------------------------------------------------------------*
    ...
    mo_tree_model = NEW zcl_uitb_column_tree_model( ir_parent           = mo_parent_container
    ...
    SET HANDLER on_tree_drag FOR lo_events.
    SET HANDLER on_tree_drag_multiple FOR lo_events.
    SET HANDLER on_tree_drop_complete_multiple FOR lo_events.
    SET HANDLER on_tree_drop FOR lo_events.
    SET HANDLER on_drop_complete FOR lo_events.
  ...
  METHOD delete_favorite.
    DATA lo_selected_node TYPE REF TO zcl_uitb_ctm_node.
    DATA lv_new_node_after_deletion TYPE tm_nodekey.
    ...
    IF    lo_selected_node              IS INITIAL
       OR lo_selected_node->mv_node_key  = c_top_node
       OR is_variant_node( lo_selected_node ).
      RETURN.
    ENDIF.
      ...
      TRY.
          ...
          IF lr_fav_data->favtype = zif_dbbr_c_favmenu_type=>folder.
            IF zcl_dbbr_appl_util=>popup_to_confirm(
                   iv_title     = 'Delete favorite folder'
  ...
  METHOD delete_selected_nodes.
    ...
    LOOP AT lt_nodes ASSIGNING FIELD-SYMBOL(<lo_node>).
      delete_favorite( io_node             = <lo_node>
  ...
  METHOD edit_favorite.
    ...
    DATA(lo_popup) = zcl_uitb_pgv_factory=>create_single_field_popup( iv_title = 'Edit Text'
                                                                      is_field = VALUE #(
                                                                          tabname   = 'dd03t'
                                                                          fieldname = 'ddtext'
                                                                          fieldtext = 'Text'
  ...
  METHOD expand_node.
    ...
    " .. expand the node
    mo_tree_model->get_nodes( )->expand_node( iv_node_key       = lo_selected_node->mv_node_key
  ...
  METHOD favorite_exists_in_node.
    ...
    rf_fav_exists = mo_favmenu_f->favorite_exists( get_node_favmenu_data( io_node ) ).
  ENDMETHOD.

  METHOD fill_toolbar.
    DATA(lo_toolbar) = mo_tree_model->get_toolbar( ).

    DATA(lt_buttons) = VALUE ttb_button( ( function  = c_context_codes-personal_favorites
  ...
  METHOD import_from_clipboard.
    ...
    DATA(lo_node_for_import) = COND #( WHEN iv_node_key IS NOT INITIAL
                                       THEN mo_tree_model->get_nodes( )->get_node( iv_node_key )
                                       ELSE mo_tree_model->get_selections( )->get_selected_node( ) ).

    IF    lo_node_for_import IS INITIAL
       OR is_variant_node( lo_node_for_import ).
      RETURN.
    ENDIF.
    ...
    DATA(lt_entities) = zcl_dbbr_ddic_util=>get_entities_from_clipboard(
                          IMPORTING ev_clipboard_count = lv_clipboard_count ).
    ...
    DATA(lv_first_new_node) = create_multiple_faventries( it_favinfo             = VALUE #(
  ...
  METHOD load_favorite_nodes.
    ...
    mo_favmenu_f->get_favorites( EXPORTING if_global_favorites = mf_global_fav_mode
                                 IMPORTING et_favmenu_entries  = DATA(lt_favmenu_entries) ).
    ...
    LOOP AT lt_favmenu_entries ASSIGNING FIELD-SYMBOL(<ls_favmenu>)
      ...
      DATA(lt_menu_entries) = VALUE zdbbr_favorite_menu_itab( FOR entry IN GROUP <ls_favmenu_group>
                                                              ( entry ) ).
      ...
      LOOP AT lt_menu_entries ASSIGNING FIELD-SYMBOL(<ls_group_entry>).
        ...
        DATA(lf_is_folder) = should_display_as_folder( iv_fav_type     = <ls_group_entry>-favtype
                                                       if_has_variants = <ls_group_entry>-has_variants ).

        lv_icon = zcl_dbbr_tree_helper=>get_tree_node_icon( <ls_group_entry>-favtype ).

        lt_items = VALUE #( ( item_name = zcl_uitb_column_tree_model=>c_hierarchy_column
                              class     = cl_item_tree_model=>item_class_text
                              font      = cl_item_tree_model=>item_font_prop
                              text      = COND #( WHEN <ls_group_entry>-favtype = zif_dbbr_c_favmenu_type=>folder
                                                  THEN <ls_group_entry>-text
                                                  ELSE <ls_group_entry>-fav_entry_raw ) ) ).
        ...
        mo_tree_model->get_nodes( )->add_node( iv_node_key          = |{ <ls_group_entry>-object_id }|
  ...
  METHOD move_favorite_down.
    ...
    lo_selected_node->move_node_to( iv_relative_node = lv_relatkey
                                    iv_relationship  = lv_relationship ).
  ...
  METHOD move_favorite_up.
    ...
    lo_selected_node->move_node_to( iv_relationship  = cl_tree_model=>relat_prev_sibling
  ...
  METHOD on_entity_deleted.
    ...
    LOOP AT mt_node_map ASSIGNING FIELD-SYMBOL(<ls_node_map>) WHERE     entity_id   = ev_entity_id
      ...
      lt_nodes = VALUE #( BASE lt_nodes
                          ( <ls_node_map>-node_key ) ).
    ENDLOOP.
  ...
  METHOD on_expand_no_children.
    ...
    LOOP AT lt_variants ASSIGNING FIELD-SYMBOL(<ls_variant>).
      ...
      IF <ls_variant>-description IS NOT INITIAL.
        DATA(lt_items_for_description) = VALUE treemcitab( ( item_name = c_hierarchy_node2
                                                             style     = zif_uitb_c_ctm_style=>inverted_gray
                                                             font      = cl_item_tree_model=>item_font_prop
                                                             class     = cl_item_tree_model=>item_class_text
      ...
      mo_tree_model->get_nodes( )->add_node( iv_node_key          = mv_last_var_node_key
      ...
      mv_last_var_node_key = mv_last_var_node_key + 1.
    ENDLOOP.
  ...
  METHOD on_node_context_menu_request.
    ...
    IF lo_selected_node->is_folder( ).
      IF line_exists( lt_expanded_nodes[ table_line = ev_node_key ] ).
        er_menu->add_function( fcode = c_context_codes-collapse_node
                               text  = |{ 'Collapse folder'(002) }| ).
      ELSE.
        er_menu->add_function( fcode = c_context_codes-expand_node
    ...
    er_menu->add_function( fcode = c_context_codes-insert_current
                           text  = |{ 'Insert Current Entity'(008) }| ).
    er_menu->add_separator( ).
    er_menu->add_function( fcode = c_context_codes-insert_folder
                           text  = |{ 'Insert Folder'(009) }| ).
    er_menu->add_function( fcode = c_context_codes-insert_favorite
                           text  = |{ 'Insert Favorite'(010) }| ).
    IF ev_node_key <> c_top_node.
      er_menu->add_function( fcode = c_context_codes-edit_favorite
                             text  = |{ 'Change Favorite'(011) }| ).
      er_menu->add_function( fcode = c_context_codes-delete_favorite
    ...
    er_menu->add_function( fcode = c_context_codes-import_from_clipboard
    ...
    IF NOT lo_selected_node->is_folder( ).
      ...
      er_menu->add_function( fcode = c_context_codes-show_in_object_browser
                             text  = |{ 'Show in Object Browser'(014) }| ).

      " ... create new function for running the selected table/script and build_tree the results
      er_menu->add_function( fcode = c_context_codes-start_with_default_variant
  ...
  METHOD on_node_context_menu_select.
    handle_gui_function( iv_code     = ev_fcode
  ...
  METHOD on_node_double_click.
    ...
    IF lf_is_favorite = abap_true.
      IF is_variant_node( lo_selected_node ).
        ...
        zcl_dbbr_selscr_nav_events=>raise_variant_entry_chosen( iv_entity_id   = ls_variant_info-entity_id
      ...
      ELSE.
        ...
        zcl_dbbr_selscr_nav_events=>raise_entity_chosen( iv_entity_id   = lr_fav_entry->fav_entry
  ...
  METHOD on_node_enter_key.
    ...
    ELSEIF ev_key = cl_list_tree_model=>key_delete.
      on_node_context_menu_select( ev_fcode    = c_context_codes-delete_favorite
  ...
  METHOD on_tree_drag.
    ...
    er_drag_drop_object->object = NEW zcl_dbbr_tree_dnd_object( VALUE #( ( ev_node_key ) ) ).
  ENDMETHOD.

  METHOD on_tree_drag_multiple.
    " .. create drag and drop object
    er_drag_drop_object->object = NEW zcl_dbbr_tree_dnd_object( et_node_key_table ).
  ENDMETHOD.

  METHOD on_tree_drop.
    DATA lv_relationship TYPE i.
    DATA lv_menulevel TYPE zdbbr_favmenu-menu_level.
    ...
    TRY.
        ...
        lo_source_node->move_node_to( iv_relative_node = lo_target_node->mv_node_key
                                      iv_relationship  = lv_relationship ).
      CATCH cx_sy_move_cast_error.
        er_drag_drop_object->abort( ).
        RETURN.
    ...
    IF     NOT lo_target_node->is_folder( )
       AND     lo_source_node->get_parent( )->mv_node_key = lo_target_node->mv_node_key.
    ...
    ELSE.
      ...
      ls_favmenu_source-parent_id  = COND #( WHEN lo_target_node->is_folder( )
                                             THEN ls_favmenu_target-object_id
                                             ELSE ls_favmenu_target_parent-object_id ).
  ...
  METHOD should_display_as_folder.
    ...
    IF    iv_fav_type     = zif_dbbr_c_favmenu_type=>folder
       OR if_has_variants = abap_true.
      result = abap_true.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD start_with_default_variant.
    ...
    zcl_dbbr_selscr_nav_events=>raise_variant_entry_chosen( iv_entity_id    = CONV #( lv_fav_id )
                                                            iv_entity_type  = ls_favmenu_data-favtype
