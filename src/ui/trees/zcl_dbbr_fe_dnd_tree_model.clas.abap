CLASS zcl_dbbr_fe_dnd_tree_model DEFINITION
  ...
  PRIVATE SECTION.
    ...
    DATA mo_settings_f TYPE REF TO zcl_dbbr_usersettings_factory.
    ...
    DATA mt_userobject_map TYPE HASHED TABLE OF ty_userobject_map WITH UNIQUE KEY node_key.
    ...
    DATA mo_tree_model TYPE REF TO cl_list_tree_model.
    DATA mo_parent TYPE REF TO cl_gui_container.
    DATA mo_tree_control TYPE REF TO cl_gui_control.
    DATA mo_tree_wrapper TYPE REF TO zcl_dbbr_list_tree_wrapper.
    DATA mo_example_dnd_behaviour TYPE REF TO cl_dragdrop.
    DATA mo_element_dnd_behaviour TYPE REF TO cl_dragdrop.
...
CLASS zcl_dbbr_fe_dnd_tree_model IMPLEMENTATION.
  METHOD constructor.
    mo_parent = ir_parent.
    mo_tabfield_list = ir_tabfield_list.
  ...
  METHOD create_available_stmtns_nodes.
    ...
    LOOP AT zcl_dbbr_fe_templates=>gt_valid_keywords_range ASSIGNING FIELD-SYMBOL(<ls_valid_keyword>).
      create_text_node( iv_parent = mv_availbl_stmnts_top_node
                        iv_text   = CONV #( <ls_valid_keyword>-low ) ).
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD create_color_node.
    ...
    mo_element_dnd_behaviour->get_handle( IMPORTING handle = DATA(lv_dnd_handle) ).
    ...
    mo_tree_model->add_node( node_key          = lv_node_key
                             ...
                             item_table        = VALUE treemlitab( class = cl_gui_list_tree=>item_class_text
                                                                   ( item_name = 1
                                                                     length    = 5
                                                                     text      = iv_color_code )
                                                                   ( item_name = 2
  ...
  METHOD create_color_nodes.
    ...
    create_color_node( iv_color_code = 'C300'
                       iv_style      = cl_list_tree_model=>style_emphasized ).
    create_color_node( iv_color_code = 'C500'
                       iv_style      = cl_list_tree_model=>style_emphasized_positive ).
    create_color_node( iv_color_code = 'C100'
                       iv_style      = cl_list_tree_model=>style_emphasized_a ).
    create_color_node( iv_color_code = 'C400'
                       iv_style      = cl_list_tree_model=>style_emphasized_b ).
    create_color_node( iv_color_code = 'C700'
                       iv_style      = cl_list_tree_model=>style_emphasized_c ).
    create_color_node( iv_color_code = 'C600'
                       iv_style      = cl_list_tree_model=>style_emphasized_negative ).
  ENDMETHOD.
  ...
  METHOD create_example_nodes.
    ...
    mo_example_dnd_behaviour->get_handle( IMPORTING handle = DATA(lv_handle) ).

    create_simple_node( iv_parent_node = mv_examples_top_node
                        iv_description = 'Short Manual'
                        iv_dnd_handle  = lv_handle
                        ir_user_object = NEW zcl_dbbr_fe_dnd_object(
                                                 iv_text         = zcl_dbbr_fe_templates=>gv_introduction_template
    ...
    create_simple_node( iv_parent_node = mv_examples_top_node
                        ...
                        iv_dnd_handle  = lv_handle
                        ir_user_object = NEW zcl_dbbr_fe_dnd_object(
                                                 iv_text         = zcl_dbbr_fe_templates=>gv_calc_difference_exmple
                                                 if_is_long_text = abap_true ) ).
    ...
    create_simple_node( iv_parent_node = mv_examples_top_node
                        ...
                        iv_dnd_handle  = lv_handle
                        ir_user_object = NEW zcl_dbbr_fe_dnd_object(
                                                 iv_text         = zcl_dbbr_fe_templates=>gv_icon_example
                                                 if_is_long_text = abap_true ) ).
    ...
    create_simple_node( iv_parent_node = mv_examples_top_node
                        ...
                        iv_dnd_handle  = lv_handle
                        ir_user_object = NEW zcl_dbbr_fe_dnd_object(
                                                 iv_text         = zcl_dbbr_fe_templates=>gv_icon_quicktip_example
                                                 if_is_long_text = abap_true ) ).
  ENDMETHOD.
  ...
  METHOD create_icon_nodes.
    ...
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_okay
                      iv_text   = 'ICON_OKAY' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_cancel
    ...
    create_text_node( iv_parent = mv_icon_top_node
                      ...
                      iv_text   = 'ICON_SUM' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_sum_red
                      iv_text   = 'ICON_SUM_RED' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_arrow_left
                      iv_text   = 'ICON_ARROW_LEFT' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_arrow_right
                      iv_text   = 'ICON_ARROW_RIGHT' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_location
                      iv_text   = 'ICON_LOCATION' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_release
                      iv_text   = 'ICON_RELEASE' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_defect
                      iv_text   = 'ICON_DEFECT' ).
    create_text_node( iv_parent = mv_icon_top_node
    ...
    create_text_node( iv_parent = mv_icon_top_node
                      ...
                      iv_text   = 'ICON_ALARM' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_warning
                      iv_text   = 'ICON_WARNING' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_hint
                      iv_text   = 'ICON_HINT' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_breakpoint
                      iv_text   = 'ICON_BREAKPOINT' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_system_help
                      iv_text   = 'ICON_SYSTEM_HELP' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_led_green
                      iv_text   = 'ICON_LED_GREEN' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_led_red
                      iv_text   = 'ICON_LED_RED' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_led_yellow
                      iv_text   = 'ICON_LED_YELLOW' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_date
                      iv_text   = 'ICON_DATE' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_transport
                      iv_text   = 'ICON_TRANSPORT' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_trend_up
                      iv_text   = 'ICON_TREND_UP' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_trend_rising
                      iv_text   = 'ICON_TREND_RISING' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_trend_unchanged
                      iv_text   = 'ICON_TREND_UNCHANGED' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_trend_decreasing
                      iv_text   = 'ICON_TREND_DECREASING' ).
    create_text_node( iv_parent = mv_icon_top_node
                      iv_icon   = icon_trend_down
                      iv_text   = 'ICON_TREND_DOWN' ).
  ENDMETHOD.

  METHOD create_row_nodes.
    DATA(lr_fields) = mo_tabfield_list->get_fields_ref( ).
    ...
    LOOP AT lr_fields->* ASSIGNING FIELD-SYMBOL(<ls_field>) WHERE     is_text_field    = abap_false
      ...
      DATA(lt_fields) = VALUE zdbbr_tabfield_info_ui_itab( FOR <ls_fld> IN GROUP <ls_field_group>
                                                           ( <ls_fld> ) ).
      ...
      DATA(lv_table_node) = create_simple_folder_node(
          ...
          iv_description = COND #( WHEN <ls_field_group>-alias IS NOT INITIAL
                                   THEN |ROW-{ <ls_field_group>-alias }-Felder - { <ls_field_group>-tabname } - { lv_tab_description }|
                                   ELSE |ROW-Felder - { <ls_field_group>-tabname } - { lv_tab_description }| ) ).
  ...
  METHOD create_saved_formula_node.
    ...
    mo_tree_model->add_node( node_key          = lv_node_key
                             ...
                             user_object       = NEW zcl_dbbr_fe_dnd_object( iv_text         = lv_formula_string
                                                                             if_is_long_text = abap_true
  ...
  METHOD create_saved_formula_nodes.
    ...
    mo_example_dnd_behaviour->get_handle( IMPORTING handle = DATA(lv_dnd_handle) ).
    ...
    LOOP AT lt_formulas ASSIGNING FIELD-SYMBOL(<ls_formula>).
      create_saved_formula_node( is_formula    = <ls_formula>
  ...
  METHOD create_simple_folder_node.
    ...
    mo_tree_model->add_node( node_key          = rv_new_node_key
                             relative_node_key = iv_parent_node
                             relationship      = cl_item_tree_model=>relat_last_child
                             isfolder          = abap_true
                             item_table        = VALUE treemlitab( ( item_name = c_column_names-favorite_id
  ...
  METHOD create_special_functions_nodes.
    ...
    create_form_command_node( iv_parent_node = mv_special_func_top_node
                              iv_keyword     = zif_dbbr_c_fe_keywords=>set_icon_value
                              iv_description = 'Set Icon with Quickinfo'
                              iv_handle      = lv_handle
                              ir_user_object = NEW #( iv_text = zcl_dbbr_fe_templates=>gv_set_icon_tmplt ) ).
    create_form_command_node( iv_parent_node = mv_special_func_top_node
                              iv_keyword     = zif_dbbr_c_fe_keywords=>set_row_color
                              iv_description = 'Color the current Row'
                              iv_handle      = lv_handle
                              ir_user_object = NEW #( iv_text = zcl_dbbr_fe_templates=>gv_set_row_color_template ) ).
    create_form_command_node( iv_parent_node = mv_special_func_top_node
                              iv_keyword     = zif_dbbr_c_fe_keywords=>set_cell_color
                              iv_description = 'Color a Cell in the current Row'
                              iv_handle      = lv_handle
                              ir_user_object = NEW #( iv_text = zcl_dbbr_fe_templates=>gv_set_cell_color_template ) ).
  ENDMETHOD.
  ...
  METHOD create_text_node.
    ...
    mo_tree_model->add_node( node_key          = lv_node_key
                             ...
                             item_table        = VALUE treemlitab( ( item_name = 1
                                                                     class     = cl_gui_list_tree=>item_class_text
                                                                     font      = cl_gui_list_tree=>item_font_prop
                                                                     length    = 30
                                                                     text      = iv_text ) ) ).
  ...
  METHOD create_tree.
    ...
    mo_tree_model = NEW cl_list_tree_model( node_selection_mode = cl_list_tree_model=>node_sel_mode_single
                                            item_selection      = abap_false
                                            with_headers        = abap_false ).

    """ create drag-n-drop behavior object
    mo_example_dnd_behaviour = NEW #( ).

    mo_example_dnd_behaviour->add( flavor     = zcl_uitb_gui_code_editor=>c_dnd_flavor-replace
                                   dragsrc    = abap_true
                                   droptarget = abap_false
                                   effect     = cl_dragdrop=>copy ).

    mo_element_dnd_behaviour = NEW #( ).
    mo_element_dnd_behaviour->add( flavor     = zcl_uitb_gui_code_editor=>c_dnd_flavor-insert
                                   dragsrc    = abap_true
                                   droptarget = abap_false
                                   effect     = cl_dragdrop=>copy ).

    mo_tree_wrapper = NEW zcl_dbbr_list_tree_wrapper( mo_tree_model ).

    mo_tree_model->add_key_stroke( key = cl_list_tree_model=>key_enter ).

    lt_events = VALUE #( appl_event = abap_true
                         ( eventid = cl_list_tree_model=>eventid_node_context_menu_req )
                         ( eventid = cl_list_tree_model=>eventid_node_double_click )
                         ( eventid = cl_list_tree_model=>eventid_node_keypress ) ).

    mo_tree_model->set_registered_events( lt_events ).

    " create tree control from model
    mo_tree_model->create_tree_control( EXPORTING parent  = mo_parent
                                        IMPORTING control = mo_tree_control ).
    ...
    SET HANDLER on_node_double_click FOR mo_tree_model.
    SET HANDLER on_node_enter_key FOR mo_tree_model.
  ENDMETHOD.

  METHOD delete_formulas.
    DATA(lv_query) = COND #( WHEN if_all = abap_true
                             THEN |Are you sure you want to delete all saved Formulas?|
                             ELSE |Are you sure you want to delete the selected Formula?| ).

    IF zcl_dbbr_appl_util=>popup_to_confirm( iv_title                 = 'Delete Formulas'
                                             iv_query                 = lv_query
    ...
    IF if_all = abap_true.
      ...
      IF lt_children IS NOT INITIAL.
        mo_tree_model->delete_nodes( lt_children ).
        lr_formula_f->delete_formulas_for_user( ).
      ENDIF.
    ELSE.
      mo_tree_model->node_get_user_object( EXPORTING node_key    = iv_node_key
      ...
      IF lr_user_object IS BOUND.
        ...
        mo_tree_model->delete_node( iv_node_key ).
  ...
  METHOD expand_top_nodes.
    mo_tree_model->expand_nodes( VALUE #( ( mv_top_node              )
  ...
  METHOD fill_tree.
    ...
    create_icon_nodes( ).
    create_color_nodes( ).
    create_saved_formula_nodes( EXPORTING if_create_top = abap_true
  ...
  METHOD on_node_context_menu_request.
    ...
    IF node_key = mv_saved_formulas_top_node.
      menu->add_function( fcode = 'DEL_SAVED_FORM_ALL'
    ...
    menu->add_function( fcode = 'DEL_SAVED_FORM'
                        text  = 'Delete Formula' ).
  ENDMETHOD.
  ...
  METHOD on_node_double_click.
    DATA(lo_selected_node) = mo_tree_wrapper->get_selected_node( ).
    IF lo_selected_node->is_folder( ).
      " check if folder is expanded/collapsed
      IF mo_tree_wrapper->is_node_expanded( lo_selected_node->get_node_key( ) ).
        mo_tree_model->collapse_node( node_key ).
      ELSE.
        mo_tree_model->expand_node( node_key ).
      ENDIF.
    ELSE.
      mo_tree_model->node_get_user_object( EXPORTING node_key    = node_key
                                           IMPORTING user_object = DATA(lo_user_object) ).
      IF lo_user_object IS BOUND.
        DATA(lo_fe_dnd_object) = CAST zcl_dbbr_fe_dnd_object( lo_user_object ).
        IF lo_fe_dnd_object->is_long_text( ).
          zcl_uitb_abap_code_viewer=>show_code( iv_title  = 'Detail View'
                                                iv_theme  = mo_settings_f->get_settings( )-code_viewer_theme
                                                iv_code   = lo_fe_dnd_object->get_text( )
                                                iv_width  = 800
                                                iv_height = 400 ).
        ENDIF.
      ENDIF.
  ...
  METHOD refresh_saved_formulas.
    ...
    IF lt_saved_formula_nodes IS NOT INITIAL.
      mo_tree_model->delete_nodes( lt_saved_formula_nodes ).
    ENDIF.

    create_saved_formula_nodes( EXPORTING if_create_top = abap_false
                                          iv_user_name  = sy-uname
                                CHANGING  cv_top_node   = mv_saved_formulas_top_node ).

    mo_tree_model->expand_node( mv_saved_formulas_top_node ).
  ENDMETHOD.
