...
*&---------------------------------------------------------------------*
    ...
    IF mv_mode = zif_dbbr_c_global=>c_field_chooser_modes-output.
      mo_tree_model->add_column( name           = c_column_names-text_field_column
                                 width          = 7
                                 alignment      = cl_column_tree_model=>align_center
    ...
    IF mf_alv_output = abap_true.
      mo_tree_model->add_column( name           = c_column_names-sum_column
                                 width          = 7
                                 header_text    = space
...
*&---------------------------------------------------------------------*
    ...
    LOOP AT lt_table_list ASSIGNING FIELD-SYMBOL(<ls_table>) WHERE     no_output          = abap_false
      ...
      IF <ls_table>-tabname = zif_dbbr_c_global=>c_formula_dummy_table.
        create_table_node( iv_tablename  = <ls_table>-tabname_alias
...
*    ELSE.
    lv_node_key = COND #( WHEN ir_tabfield_info->alias IS NOT INITIAL
                          THEN |{ ir_tabfield_info->alias }~{ ir_tabfield_info->fieldname_raw }|
                          ELSE |{ ir_tabfield_info->fieldname_raw }| ).
*    ENDIF.

    " create item table of node
    DATA(lt_item_table) = VALUE treemcitab( ( item_name = c_column_names-field_check_column
    ...
    IF     mv_mode = zif_dbbr_c_global=>c_field_chooser_modes-output
       AND ir_tabfield_info->has_text_field = abap_true.

      APPEND VALUE #( item_name = c_column_names-text_field_column
                      class     = cl_list_tree_model=>item_class_checkbox
    ...
    APPEND VALUE #( item_name = c_column_names-description_column
                    class     = cl_list_tree_model=>item_class_text
                    font      = cl_list_tree_model=>item_font_prop
    ...
    mo_tree_model->add_node(
      EXPORTING  node_key          = lv_node_key
                 relative_node_key = CONV #( COND #( WHEN ir_tabfield_info->tabname_alias IS NOT INITIAL
                                                     THEN ir_tabfield_info->tabname_alias
                                                     ELSE mc_formula_node ) )
                 ...
                 user_object       = NEW zcl_dbbr_tabfield( ir_table_field_info = ir_tabfield_info
                                                            iv_mode             = mv_mode           )
                 style             = COND #( WHEN ir_tabfield_info->is_key = abap_true
  ...
  METHOD create_toolbar.
    ...
    DATA(lt_buttons) = VALUE ttb_button( butn_type = cntb_btype_button
                                         ( function = 'EXPAND_ALL'
  ...
  METHOD create_tree.
    ...
    mo_container = NEW cl_gui_custom_container( container_name = mc_field_props_container ).
    mo_tree_model = NEW cl_column_tree_model( node_selection_mode   = cl_column_tree_model=>node_sel_mode_multiple
                                              item_selection        = abap_true
                                              hierarchy_header      = VALUE treemhhdr( heading = 'Table / Field'
                                                                                       width   = 75 )
    ...
    lt_events = VALUE #( appl_event = abap_true
                         ...
                         ( eventid = cl_column_tree_model=>eventid_node_context_menu_req ) ).

    mo_tree_model->set_registered_events( lt_events ).

    SET HANDLER on_checkbox_change FOR mo_tree_model.
    SET HANDLER on_button_click FOR mo_tree_model.
  ...
  METHOD deselect_all_fields.
    " TODO: parameter IV_NODE_KEY is never used (ABAP cleaner)

    select_fields_internal( if_select = abap_false ).
  ENDMETHOD.
  ...
  METHOD on_button_click.
    CASE item_name.
      WHEN c_column_names-sel_all_column.
        select_fields_internal( iv_node_key = node_key ).
  ...
  METHOD on_checkbox_change.
    ...
    ELSEIF item_name = c_column_names-text_field_column.
      mo_fields->set_all_text_fields( iv_tabname      = ls_tabfield_info-tabname_alias
  ...
  METHOD on_node_context_menu_request.
    ...
    menu->add_function( fcode = 'SELECT'
                        text  = CONV #( TEXT-007 ) ).
    menu->add_function( fcode = 'DESELECT'
                        text  = CONV #( TEXT-008 ) ).
  ENDMETHOD.
  ...
  METHOD on_node_context_menu_select.
    ...
    LOOP AT lt_selected_nodes ASSIGNING FIELD-SYMBOL(<lv_node>).
      CASE fcode.
        WHEN 'SELECT'.
          select_field_internal( iv_node_key            = <lv_node>
  ...
  METHOD refresh_from_model.
    ...
    LOOP AT mt_top_nodes ASSIGNING FIELD-SYMBOL(<lv_top_node>).
      ...
      mo_tree_model->node_get_children( EXPORTING node_key       = <lv_top_node>
                                        IMPORTING node_key_table = DATA(lt_children) ).

      LOOP AT lt_children ASSIGNING FIELD-SYMBOL(<lv_child_node>).
        mo_tree_model->node_get_user_object( EXPORTING node_key    = <lv_child_node>
                                             IMPORTING user_object = DATA(lr_user_object) ).
        ...
        CASE mv_mode.
          WHEN zif_dbbr_c_global=>c_field_chooser_modes-output.
            ...
            IF ls_tabfield_info-has_text_field = abap_true.
              DATA(lr_textfield_ref) = mo_fields->get_field_ref( iv_tabname_alias = ls_tabfield_info-tabname_alias
                                                                 iv_fieldname     = ls_tabfield_info-fieldname
                                                                 if_is_text_field = abap_true ).
              mo_tree_model->item_set_chosen( node_key  = <lv_child_node>
  ...
  METHOD select_all_text_fields.
    select_fields_internal( if_select      = abap_true
                            if_text_select = abap_true ).
  ENDMETHOD.
  ...
  METHOD select_fields_internal.
    ...
    LOOP AT lt_nodes ASSIGNING FIELD-SYMBOL(<lv_top_node>).
      ...
      LOOP AT lt_children ASSIGNING FIELD-SYMBOL(<lv_child_node>).
        select_field_internal( iv_node_key            = <lv_child_node>
                               if_select              = if_select
                               if_custom_order_exists = mo_fields->custom_order_exists( )
                               if_text_select         = if_text_select
                               if_key_select          = if_key_select ).
...
*&---------------------------------------------------------------------*
    ...
    IF     if_text_select                  = abap_true
       AND ls_tabfield_info-has_text_field = abap_false.
      RETURN.
    ENDIF.
    ...
    mo_tree_model->item_set_chosen( node_key  = iv_node_key
                                    item_name = COND #( WHEN if_text_select = abap_true
                                                        THEN c_column_names-text_field_column
                                                        ELSE c_column_names-field_check_column )
                                    chosen    = lf_checked ).
  ...
  METHOD select_key_fields.
    ...
    select_fields_internal( if_key_select = abap_true ).
  ENDMETHOD.
  ...
  METHOD zif_uitb_content_searcher~search.
    ...
    IF lv_result_type <> 0 AND lt_result_item IS NOT INITIAL.
      mo_tree_model->select_item( node_key  = lt_result_item[ 1 ]-node_key
