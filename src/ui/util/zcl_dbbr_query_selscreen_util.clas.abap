CLASS zcl_dbbr_query_selscreen_util DEFINITION
  PUBLIC
  INHERITING FROM zcl_dbbr_table_selscreen_util FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS get_entity_type_for_join                       REDEFINITION.
  ...
  PROTECTED SECTION.
    ...
    METHODS create_table_header  REDEFINITION.
    METHODS finish_loading       REDEFINITION.
  ...
  PRIVATE SECTION.
    ...
    DATA mv_sql_query_content TYPE zdbbr_query_data-source.

    "! <p class="shorttext synchronized">Delete the query</p>
    "!
    METHODS delete_query.
    "! <p class="shorttext synchronized">Copies the query</p>
    "!
    METHODS copy_query.
    "! <p class="shorttext synchronized">Maintain jump definitions of query</p>
    "!
    METHODS maintain_jumps.
    "! <p class="shorttext synchronized">Export query to file</p>
    "!
    METHODS download_query.

    "! <p class="shorttext synchronized">Loads a standard query</p>
    "!
    METHODS load_standard_query
      IMPORTING
        is_query_data TYPE zdbbr_query_data.

    "! <p class="shorttext synchronized">Loads a custom query</p>
    "!
    METHODS load_custom_query
      IMPORTING
        is_query_data TYPE zdbbr_query_data.

    "! <p class="shorttext synchronized">Create fields for query parameters in tabfield list</p>
    "!
    METHODS fill_query_parameters.
    "! <p class="shorttext synchronized">Edit the current sql query</p>
    "!
    METHODS edit_sql_query.
    "! <p class="shorttext synchronized">Create new SQL Query</p>
    "!
...
*            rv_message      =
  ...
  METHOD create_table_header.
    ...
    IF iv_tabname = zif_dbbr_c_global=>c_parameter_dummy_table.
      rs_table_header = VALUE zdbbr_selfield( tabname         = iv_tabname
                                              ...
                                              ddic_order      = 0 ).
    ELSE.
      rs_table_header = super->create_table_header( iv_tabname       = iv_tabname
  ...
  METHOD constructor.
    ...
    mo_custom_menu = NEW #( ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>edit_jump_fields
                                  text  = |{ 'Define Jumps'(002) }| ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>copy_query
                                  text  = |{ 'Copy query'(003) }| ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>export_query
                                  text  = |{ 'Download query to PC'(004) }| ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>delete_query
  ...
  METHOD copy_query.
    ...
    IF NOT lr_query_copier_view->was_copied( ).
      RETURN.
    ENDIF.
    ...
    " .. update history
    zcl_dbbr_selscreen_history=>add_new_entry( iv_entity      = ls_copied_query-query_name
  ...
  METHOD delete_query.
    ...
    zcl_dbbr_selscreen_history=>delete_entity_from_history( iv_entity_id   = mv_entity_id
    ...
    notify_of_deleted_entity( iv_entity_id   = lv_entity_id
  ...
  METHOD load_entity_internal.
    ...
    IF ls_query_data-source IS INITIAL.
      ...
      load_standard_query( is_query_data = ls_query_data ).
    ELSE.
      mf_sql_query = abap_true.
      load_custom_query( is_query_data = ls_query_data ).
    ENDIF.

    zcl_dbbr_toolbar_util=>get_selscreen_table_tb( )->set_button_state(
        enabled = lf_enable_selfield_control
        fcode   = zif_dbbr_c_selscreen_functions=>control_sel_fields ).

    mv_entity_id = mv_query_name.

    mo_data->mr_s_entity_info->* = VALUE #( entity_id   = mv_query_name
  ...
  METHOD maintain_jumps.
    ...
    DATA(lr_jumplist_controller) = NEW zcl_dbbr_jumplist_controller( iv_query_id = mo_data->mr_s_query_info->query_id
  ...
  METHOD zif_dbbr_screen_util~get_deactivated_functions.
    ...
    IF     mv_query_name                    IS NOT INITIAL
       AND mo_data->mr_s_query_info->source IS NOT INITIAL.
...
*...... TODO: Activate count lines for custom queries
    ...
    ELSE.
      ...
      IF     mo_data->mr_s_query_info->created_by              = sy-uname
         AND mo_data->mr_s_global_data->settings-advanced_mode = abap_true.
  ...
  METHOD fill_primary_entity.
    ...
    IF is_primary_entity-type = zif_sat_c_entity_type=>cds_view.
      create_cds_fields( VALUE #( tabname          = is_primary_entity-tabname
  ...
  METHOD load_standard_query.
    ...
    <ls_join_definition>-primary_table.
    ...
    LOOP AT is_query_data-tables ASSIGNING FIELD-SYMBOL(<ls_table>).
      lr_tabfield_list->update_table_active_selection( iv_tabname_alias    = <ls_table>-tabname_alias
                                                       if_active_selection = <ls_table>-active_selection
                                                       iv_selection_order  = <ls_table>-selection_order ).
    ENDLOOP.

    lr_tabfield_list->clear_active_flag( if_clear_selection = abap_true
    ...
    IF mo_data->mr_s_query_info->formula IS NOT INITIAL.
      " ... validate formula
      TRY.
          DATA(lr_formula) = NEW zcl_dbbr_fe_validator( iv_formula   = mo_data->mr_s_query_info->formula
                                                        io_tabfields = lr_tabfield_list
          )->validate( ).

          zcl_dbbr_formula_helper=>update_tabflds_from_formula( ir_tabfields      = lr_tabfield_list
    ...
    IF is_query_data-has_filter_values = abap_true.
      ...
      IF lv_default_variant IS NOT INITIAL.
        ...
        zcl_dbbr_variant_loader=>create_variant_loader( iv_variant_id        = lv_default_variant
  ...
  METHOD fill_query_parameters.
    ...
    LOOP AT mt_parameters ASSIGNING FIELD-SYMBOL(<ls_param>).
      ...
      IF ls_tabfield-rollname IS NOT INITIAL OR ls_tabfield-datatype IS NOT INITIAL.
        ls_tabfield-has_custom_f4 = mo_data->mo_custom_f4_map->entry_exists( iv_tabname       = space
    ...
    DATA(ls_entity) = VALUE zdbbr_entity_info( active_selection = abap_true
                                               tabname          = zif_dbbr_c_global=>c_parameter_dummy_table
  ...
  METHOD edit_sql_query.
    ...
    IF lv_last_saved_query IS INITIAL.
      RETURN.
    ENDIF.
