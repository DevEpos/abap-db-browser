CLASS zcl_dbbr_join_selection_util DEFINITION
  ...
  PROTECTED SECTION.
    ...
    "! <p class="shorttext synchronized">Marks virtual join fields for later processing</p>
    METHODS mark_virt_join_selfields.

    "! <p class="shorttext synchronized">Fill virtual join field in the output table</p>
    ...
    "! <p class="shorttext synchronized">Update parameters of entities with the given parameter</p>
    METHODS update_table_parameters
      IMPORTING
        is_param TYPE zsat_table_parameter.

  PRIVATE SECTION.
    "! <p class="shorttext synchronized">Prefill cds view parameters</p>
    ...
    "! <p class="shorttext synchronized">Fill parameters for a certain entity in the join</p>
...
CLASS zcl_dbbr_join_selection_util IMPLEMENTATION.
  ...
  METHOD mark_virt_join_selfields.
    ...
    LOOP AT ms_join_def-tables ASSIGNING FIELD-SYMBOL(<ls_join_table>) WHERE is_virtual = abap_true.
      LOOP AT <ls_join_table>-field_conditions ASSIGNING FIELD-SYMBOL(<ls_join_field>).
        ...
        " update field
        DATA(lr_s_field) = mo_tabfields->get_field_ref( iv_tabname_alias = <ls_join_field>-ref_table
    ...
    mt_virtual_join_table_range = VALUE #( LET sign   = 'I'
                                               option = 'EQ' IN
  ...
  METHOD init.
    IF mt_param_values IS NOT INITIAL.
      ...
      prefill_parameters( EXPORTING io_tabfields = mo_tabfields
  ...
  METHOD create_from_clause.
    ...
    IF ms_join_def-tables IS INITIAL.
      IF     mv_entity_type = zif_sat_c_entity_type=>cds_view
         AND ( sy-saprl < 750 OR ms_technical_info-use_ddl_view_for_select = abap_true ).
  ...
  METHOD prefill_parameters.
    ...
    LOOP AT lt_tables ASSIGNING FIELD-SYMBOL(<ls_table>).
      ...
      fill_entity_params( EXPORTING iv_entity       = <ls_table>-tabname
  ...
  METHOD before_selection.
    ...
    IF     mf_join_is_active = abap_true
       AND line_exists( ms_join_def-tables[ is_virtual = abap_true ] ).
      mo_post_join_helper = zcl_dbbr_virtual_join_helper=>create( is_join_def   = ms_join_def
