"! <p class="shorttext synchronized">Loads Variants</p>
CLASS zcl_dbbr_variant_loader DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Creates variant loader for specific variant</p>
    ...
    "! <p class="shorttext synchronized">Creates variant loader for automatic variant</p>
    ...
    "! <p class="shorttext synchronized">Creates variant loader for default variant</p>
...
CLASS zcl_dbbr_variant_loader IMPLEMENTATION.
  ...
  METHOD create_variant_loader.
    DATA(lr_loader) = NEW zcl_dbbr_variant_loader( iv_variant_name      = iv_variant_name
  ...
  METHOD create_auto_variant_loader.
    DATA(lr_loader) = NEW zcl_dbbr_variant_loader( iv_entity_type       = iv_entity_type
                                                   iv_entity_id         = iv_entity_id
                                                   ir_t_multi_or        = ir_t_multi_or
                                                   ir_t_selfields       = ir_t_selfields
                                                   ir_t_selfields_multi = ir_t_selfields_multi
  ...
  METHOD zif_dbbr_variant_loader~load.
    DATA ls_tabfield TYPE zdbbr_tabfield_info_ui.
    DATA lr_s_tabfield TYPE REF TO zdbbr_tabfield_info_ui.
    ...
    IF ls_existing_variant IS INITIAL.
      IF mv_loader_type = c_loader_type-by_name_or_id.
        MESSAGE e030(zdbbr_info) WITH mv_variant_name INTO DATA(lv_dummy) ##NEEDED.
    ...
    SORT ls_existing_variant-variant_data BY tuple_id
                                             tabname
                                             fieldname
                                             counter.
    ...
    LOOP AT ls_existing_variant-variant_data ASSIGNING FIELD-SYMBOL(<ls_vard>) WHERE    data_type = zif_dbbr_c_global=>c_variant_datatypes-group_by
                                                                                     OR data_type = zif_dbbr_c_global=>c_variant_datatypes-aggregation
                                                                                     OR data_type = zif_dbbr_c_global=>c_variant_datatypes-totals.
    ...
    IF     lf_grouping_is_active = abap_false
       AND (    ls_existing_variant-has_output_fields = abap_true
             OR ls_existing_variant-has_sort_fields   = abap_true ).
      mr_tabfields->clear_active_flag( if_clear_output = ls_existing_variant-has_output_fields
    ...
    IF     lf_grouping_is_active       = abap_true
       AND ls_existing_variant-fields IS INITIAL.
      LOOP AT mr_t_selfields->* ASSIGNING FIELD-SYMBOL(<ls_grouped_field>) WHERE    group_by     = abap_true
                                                                                 OR aggregation <> space
                                                                                 OR totals       = abap_true.
        DATA(lv_index) = sy-tabix.
        TRY.
            lr_s_tabfield = mr_tabfields->get_field_ref( iv_tabname_alias = <ls_grouped_field>-tabname_alias
            ...
            IF    <ls_grouped_field>-aggregation <> space
               OR <ls_grouped_field>-totals       = abap_true.
    ...
    IF ls_existing_variant-has_output_fields = abap_true OR ls_existing_variant-has_sort_fields = abap_true.
      ...
      LOOP AT ls_existing_variant-fields ASSIGNING FIELD-SYMBOL(<ls_tabfield>) WHERE is_formula_field = abap_false.
        ...
        TRY.
            lr_s_tabfield = mr_tabfields->get_field_ref( iv_tabname_alias = <ls_tabfield>-tabname_alias
  ...
  METHOD fill_selscreen_with_tupledata.
    ...
    LOOP AT it_variant_data ASSIGNING FIELD-SYMBOL(<ls_vardata>)
      ...
      TRY.
          DATA(lr_field) = mr_tabfields->get_field_ref( iv_tabname_alias = <ls_vardata_group>-tabname_alias
      ...
      LOOP AT GROUP <ls_vardata_group> ASSIGNING FIELD-SYMBOL(<ls_vardata_group_entry>).
        IF lf_first_iteration = abap_true.
          DATA(ls_selfield_info) = VALUE zdbbr_selfield_info( tabname_alias = <ls_vardata_group_entry>-tabname
                                                              tabname       = lv_tabname
        ...
        ELSE.
          APPEND VALUE zdbbr_selfield_value( sign   = <ls_vardata_group_entry>-sign_val
                                             option = <ls_vardata_group_entry>-data_type
                                             low    = <ls_vardata_group_entry>-low_val
                                             high   = <ls_vardata_group_entry>-high_val )
  ...
  METHOD load_automatic_variant.
    rs_variant = zcl_dbbr_variant_factory=>get_automatic_variant( iv_entity_id   = mv_entity_id
                                                                  iv_entity_type = mv_entity_type ).
  ENDMETHOD.

  METHOD load_default_variant.
    rs_variant = zcl_dbbr_variant_factory=>get_default_variant( iv_entity_id   = mv_entity_id
                                                                iv_entity_type = mv_entity_type ).
  ENDMETHOD.

  METHOD load_variant.
    zcl_dbbr_variant_factory=>get_variant( EXPORTING iv_variant_name    = mv_variant_name
                                                     iv_variant_id      = mv_variant_id
                                                     iv_entity_id       = mv_entity_id
                                                     iv_entity_type     = mv_entity_type
                                                     if_load_completely = abap_true
                                           IMPORTING es_variant         = rs_variant ).
  ENDMETHOD.
ENDCLASS.
