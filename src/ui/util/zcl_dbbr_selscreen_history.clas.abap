"! <p class="shorttext synchronized">History Manager for Selection Screen</p>
CLASS zcl_dbbr_selscreen_history DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Navigation occurred</p>
    CLASS-EVENTS navigated
      EXPORTING
        VALUE(es_history_entry) TYPE zdbbr_selscreen_history
        VALUE(ev_current_index) TYPE i OPTIONAL.
    "! <p class="shorttext synchronized">History was modified</p>
    CLASS-EVENTS history_modified.

    "! <p class="shorttext synchronized">Clears navigation history</p>
    CLASS-METHODS clear_history.

    "! <p class="shorttext synchronized">Adds new history entry</p>
    ...
    "! <p class="shorttext synchronized">Deletes the given entity from the history</p>
    CLASS-METHODS delete_entity_from_history
      IMPORTING
        iv_entity_id   TYPE zsat_entity_id
        iv_entity_type TYPE zsat_entity_type.

    "! <p class="shorttext synchronized">Gets the current history index</p>
    CLASS-METHODS get_current_index
      RETURNING
        VALUE(result) TYPE i.

    "! <p class="shorttext synchronized">Retrieves all History entries</p>
    CLASS-METHODS get_history
      RETURNING
        VALUE(result) TYPE zdbbr_selscreen_history_t.

    "! <p class="shorttext synchronized">Checks if there is a next entry</p>
    CLASS-METHODS has_next
      RETURNING
        VALUE(result) TYPE abap_bool.

    "! <p class="shorttext synchronized">Checks if there is a previous entry</p>
    CLASS-METHODS has_previous
      RETURNING
        VALUE(result) TYPE abap_bool.

    "! <p class="shorttext synchronized">Get Previous history entry</p>
    CLASS-METHODS navigate_back.
    "! <p class="shorttext synchronized">Get next history entry</p>
    CLASS-METHODS navigate_forward.

    "! <p class="shorttext synchronized">Navigate to specific history entry</p>
    ...
    "! <p class="shorttext synchronized">Navigates to the current history entry</p>
    CLASS-METHODS navigate_to_current
      RETURNING
        VALUE(rf_navigated) TYPE abap_bool.

  PRIVATE SECTION.
    "! <p class="shorttext synchronized">List of history entries of Selection screen</p>
...
CLASS zcl_dbbr_selscreen_history IMPLEMENTATION.
  ...
  METHOD add_new_entry.
    ...
    IF st_history IS NOT INITIAL.
      ...
      IF     <ls_history>-entity_id   = iv_entity
         AND <ls_history>-entity_type = iv_type.
        RETURN.
      ENDIF.
    ENDIF.

    INSERT VALUE #( entity_id   = iv_entity
                    entity_type = iv_type
  ...
  METHOD has_next.
    result = abap_true.
    IF    sv_history_count = 0
       OR sv_current_index = 1.
      result = abap_false.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD has_previous.
    result = abap_true.
    IF    sv_history_count     = 1
       OR sv_current_index + 1 > sv_history_count.
      result = abap_false.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD navigate_to_current.
    ...
    rf_navigated = navigate_to( iv_index            = sv_current_index
