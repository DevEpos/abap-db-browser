"! <p class="shorttext synchronized">Creates Variant Data out of Selection Screen Information</p>
CLASS zcl_dbbr_variant_creator DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Create Variant data from selection screen information</p>
  ...
  PRIVATE SECTION.
    "! <p class="shorttext synchronized">Add special attribute to variant data</p>
    ...
    "! <p class="shorttext synchronized">Add normal value to variant data</p>
    ...
    "! <p class="shorttext synchronized">Add mutli or values to variant data</p>
    ...
    "! <p class="shorttext synchronized">Adds custom fields to variant</p>
...
CLASS zcl_dbbr_variant_creator IMPLEMENTATION.
  METHOD add_multi_or_variant_data.
    IF    is_selfield_info-low    IS NOT INITIAL
       OR is_selfield_info-high   IS NOT INITIAL
       OR is_selfield_info-option IS NOT INITIAL.
...
*&---------------------------------------------------------------------*
    cv_line_counter = cv_line_counter + 1.
...
*&---------------------------------------------------------------------*
    ...
    IF is_selfield-system_value_type IS NOT INITIAL.
      ...
      APPEND cs_layout_data TO ct_layout_data.
    ELSE.
      IF    is_selfield-low    IS NOT INITIAL
         OR is_selfield-high   IS NOT INITIAL
         OR is_selfield-option IS NOT INITIAL.
    ...
    IF is_selfield-push = abap_true.
      LOOP AT it_multi ASSIGNING FIELD-SYMBOL(<ls_select_multi>)
           WHERE     tabname_alias = is_selfield-tabname_alias
                 AND fieldname     = is_selfield-fieldname
                 AND ( low IS NOT INITIAL OR high IS NOT INITIAL ).
        cv_line_counter = cv_line_counter + 1.
        cs_layout_data-counter   = cv_line_counter.
        cs_layout_data-low_val   = <ls_select_multi>-low.
        cs_layout_data-high_val  = <ls_select_multi>-high.
  ...
  METHOD create_variant.
    ...
    rs_variant = VALUE zdbbr_variant_data( variant_id          = iv_variant_id
    ...
    LOOP AT it_selfields ASSIGNING FIELD-SYMBOL(<ls_selection_field>)
      ...
      " fill default values
      DATA(ls_vardata_entry) = VALUE zdbbr_vardata( tabname   = <ls_selection_field>-tabname_alias
                                                    fieldname = <ls_selection_field>-fieldname ).

      add_value_variant_data( EXPORTING is_selfield     = <ls_selection_field>
      ...
      IF <ls_selection_field>-group_by = abap_true.
        add_special_variant_data( EXPORTING iv_layout_data_type = zif_dbbr_c_global=>c_variant_datatypes-group_by
                                  CHANGING  cs_layout_data      = ls_vardata_entry
                                            ct_layout_data      = rs_variant-variant_data
                                            cv_line_counter     = lv_counter              ).
      ENDIF.
      ...
      IF <ls_selection_field>-aggregation <> space.
        add_special_variant_data( EXPORTING iv_layout_data_type = zif_dbbr_c_global=>c_variant_datatypes-aggregation
      ...
      IF <ls_selection_field>-totals = abap_true.
        add_special_variant_data( EXPORTING iv_layout_data_type = zif_dbbr_c_global=>c_variant_datatypes-totals
                                  CHANGING  cs_layout_data      = ls_vardata_entry
                                            ct_layout_data      = rs_variant-variant_data
                                            cv_line_counter     = lv_counter ).
      ENDIF.
    ...
    LOOP AT it_multi_or ASSIGNING FIELD-SYMBOL(<ls_multi_or_tuple>).
      ...
      ls_vardata_entry = VALUE zdbbr_vardata( tuple_id = <ls_multi_or_tuple>-pos ).
      ...
      LOOP AT <ls_multi_or_tuple>-values ASSIGNING FIELD-SYMBOL(<ls_multi_or_tuple_data>).
        ...
        add_multi_or_variant_data( EXPORTING is_selfield_info = <ls_multi_or_tuple_data>
                                   CHANGING  cs_layout_data   = ls_vardata_entry
                                             ct_layout_data   = rs_variant-variant_data ).
      ENDLOOP.
    ENDLOOP.
    ...
    IF is_global_data IS NOT INITIAL.
      add_custom_fields_to_variant( EXPORTING is_global_data = is_global_data
