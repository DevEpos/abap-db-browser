CLASS zcl_dbbr_cds_variant_starter DEFINITION
  PUBLIC
  INHERITING FROM zcl_dbbr_variant_starter FINAL
  CREATE PUBLIC.
  ...
  PROTECTED SECTION.
    METHODS fill_data_from_variant REDEFINITION.
...
CLASS zcl_dbbr_cds_variant_starter IMPLEMENTATION.
  ...
  METHOD zif_dbbr_variant_starter~initialize.
    ...
    IF ms_global_data-called_from_adt = abap_true.
      zcl_dbbr_usersettings_factory=>update_start_settings( iv_entity_id   = mv_cds_view
                                                            iv_entity_type = zif_sat_c_entity_type=>cds_view ).
    ENDIF.

    NEW zcl_dbbr_favmenu_factory( )->refresh_most_used( iv_entry     = mv_cds_view
  ...
  METHOD fill_data_from_variant.
    ...
    IF mv_variant_id = zif_dbbr_c_global=>c_dummy_variant.
      ...
      IF mo_cds_view->has_parameters( if_exclude_system_params = abap_true ).
        ...
        DATA(lo_param_popup) = NEW zcl_dbbr_cds_param_popup( io_tabfields     = mo_tabfield_list
        ...
        IF lt_param_values IS INITIAL.
          RAISE EXCEPTION TYPE zcx_dbbr_variant_error
            EXPORTING textid = zcx_dbbr_variant_error=>missing_params
                      msgv1  = |{ mo_cds_view->get_header( )-entityname_raw }|.
        ELSE.

          mt_selfields = VALUE #( FOR param IN lt_param_values
  ...
  METHOD fill_primary_entity.
    ...
    create_cds_fields( VALUE #( tabname       = mv_cds_view
  ...
  METHOD get_params_from_variant.
    ...
    IF ms_global_data-always_load_def_variant_first = abap_true.
      ls_variant = zcl_dbbr_variant_factory=>get_default_variant( iv_entity_id       = mv_cds_view
                                                                  iv_entity_type     = zif_sat_c_entity_type=>cds_view
    ...
    IF ms_global_data-auto_sel_filter_saving = abap_true.
      ls_variant = zcl_dbbr_variant_factory=>get_automatic_variant( iv_entity_id       = mv_cds_view
                                                                    iv_entity_type     = zif_sat_c_entity_type=>cds_view
                                                                    if_load_completely = abap_true ).
      rt_params = get_parameter_values_from_var( it_vardata = ls_variant-variant_data ).
    ENDIF.
  ...
  METHOD get_parameter_values_from_var.
    LOOP AT it_vardata ASSIGNING FIELD-SYMBOL(<ls_vardata>) WHERE tabname = zif_dbbr_c_global=>c_parameter_dummy_table.
      ...
      TRY.
          ...
          DATA(lr_field_ref) = mo_tabfield_list->get_field_ref( iv_tabname_alias = <ls_vardata>-tabname
                                                                iv_fieldname     = <ls_vardata>-fieldname ).
        CATCH cx_sy_itab_line_not_found.
          CONTINUE.
      ENDTRY.

      DATA(ls_param) = VALUE zif_sat_ty_global=>ty_s_cds_param_value( name = <ls_vardata>-fieldname ).
      ...
      rt_param_data = VALUE #( BASE rt_param_data
                               ( ls_param ) ).
  ...
  METHOD create_automatic_variant.
    DATA(ls_variant) = zcl_dbbr_variant_creator=>create_variant( iv_entity_id   = mv_cds_view
                                                                 iv_entity_type = zif_sat_c_entity_type=>cds_view
