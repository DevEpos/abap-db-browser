"! <p class="shorttext synchronized">Util for Handling CDS Selection Screen Logic</p>
CLASS zcl_dbbr_cds_selscreen_util DEFINITION
  PUBLIC
  INHERITING FROM zcl_dbbr_selscreen_util FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS zif_dbbr_screen_table_util~handle_pbo          REDEFINITION.
    METHODS zif_dbbr_screen_util~get_deactivated_functions REDEFINITION.
    METHODS zif_dbbr_screen_util~handle_pbo                REDEFINITION.
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">DD: Header for Structured Objects</p>
    ...
    "! <p class="shorttext synchronized">Choose sub entity of CDS View for selection screen</p>
    ...
    "! <p class="shorttext synchronized">Builds the custom default functions</p>
    METHODS build_default_custom_function.
    "! <p class="shorttext synchronized">Builds custom functions for cds query</p>
...
CLASS zcl_dbbr_cds_selscreen_util IMPLEMENTATION.
  ...
  METHOD load_entity_internal.
    ...
    IF mo_cds_view IS INITIAL.
      ...
      mo_data->mr_s_entity_info->* = VALUE #( entity_id   = space
    ...
    zcl_dbbr_screen_helper=>show_progress(
        iv_text     = |Selection Mask for CDS View { ls_cds_header-entityname_raw } is loading...|
        iv_progress = 1 ).
    ...
    mo_data->mr_s_entity_info->*         = VALUE #( entity_id   = mv_cds_view
                                                    entity_type = mv_entity_type ).
  ...
  METHOD fill_selection_mask.
    IF    mo_cds_view->has_parameters( )
       OR mo_data->mr_s_join_def->tables IS NOT INITIAL.
  ...
  METHOD fill_primary_entity.
    ...
    zcl_dbbr_cds_tabfield_util=>add_view_colums( io_custom_f4_map = mo_data->mo_custom_f4_map
  ...
  METHOD choose_cds_sub_entity.
    DATA(lr_choose_sub_entity_view) = NEW zcl_dbbr_cds_sub_entity_sel( ir_cds_view          = mo_cds_view
    ...
    ELSE.
      lr_choose_sub_entity_view->get_chosen_sub_entity( IMPORTING ev_enttiy_id   = DATA(lv_entity_id)
  ...
  METHOD zif_dbbr_screen_table_util~handle_pbo.
    ...
    DATA(lv_visibility) = COND i(
      ...
      WHEN NOT mo_cds_view->has_parameters( )
       AND NOT mo_data->is_join_active( )     THEN 1
      ELSE                                         0 ).
  ...
  METHOD zif_dbbr_screen_util~get_deactivated_functions.
    ...
    IF mo_cds_view IS BOUND.
      ...
      IF    lv_source_type = zif_sat_c_cds_view_type=>abstract_entity
         OR lv_source_type = zif_sat_c_cds_view_type=>extend.
    ...
    CASE cv_function.
      ...
      WHEN zif_dbbr_c_selscreen_functions=>show_ddls_source.
        TRY.
            ...
            zcl_uitb_abap_code_viewer=>show_code( iv_title = |DDL Source { mv_cds_view_name_raw }|
                                                  iv_code  = lv_source
      ...
      WHEN zif_dbbr_c_selscreen_functions=>open_cds_view_with_adt.
        TRY.
            zcl_sat_adt_util=>jump_adt( iv_obj_name = |{ mo_cds_view->get_header( )-ddlname }|
                                        iv_obj_type = 'DDLS' ).
          CATCH zcx_sat_adt_error INTO DATA(lx_adt_error).
            RAISE EXCEPTION TYPE zcx_sat_validation_exception
              EXPORTING previous = lx_adt_error.
        ENDTRY.
      ...
      WHEN zif_dbbr_c_selscreen_functions=>open_cds_query_in_qry_mon.
        ...
        IF lt_base_tables IS NOT INITIAL.
          ...
          IF lv_cube_view IS NOT INITIAL.
            zcl_sat_query_monitor_util=>open_in_query_monitor( iv_query_ddlname = mo_cds_view->get_header( )-ddlview ).
          ENDIF.
        ENDIF.

      WHEN zif_dbbr_c_selscreen_functions=>open_in_design_studio.
        TRY.
            NEW zcl_dbbr_design_studio_util( io_cds_view       = mo_cds_view
                                             io_selscreen_data = mo_data )->open_in_design_studio( ).
  ...
  METHOD build_default_custom_function.
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>open_cds_view_with_adt
                                  text  = |{ 'Open with ADT'(005) }| ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>go_to_ddic_view_of_cds
                                  text  = |{ 'Go to DDIC View'(006) }| ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>choose_cds_sub_entity
                                  text  = |{ 'Navigate to Sub Entity'(003) }| ).
    mo_custom_menu->add_separator( ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>show_ddls_source
                                  text  = |{ 'Show DDL Source'(002) }| ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>show_cds_dependency_tree
                                  text  = |{ 'Show Dependency Tree'(011) }| ).
  ENDMETHOD.

  METHOD build_cust_func_for_cds_qry.
    mo_custom_menu->add_separator( ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>open_cds_query_in_aox
                                  text  = |{ 'Open with SAP Analysis for Office'(012) }| ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>open_cds_query_in_qry_mon
                                  text  = |{ 'Open with Query Monitor (RSRT)'(013) }| ).
    mo_custom_menu->add_function( fcode = zif_dbbr_c_selscreen_functions=>open_in_design_studio
