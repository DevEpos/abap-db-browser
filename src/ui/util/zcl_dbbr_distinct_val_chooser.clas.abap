CLASS zcl_dbbr_distinct_val_chooser DEFINITION
  ...
  PRIVATE SECTION.
    ...
    DATA mt_distinct_values TYPE STANDARD TABLE OF ty_distinct WITH EMPTY KEY.
...
CLASS zcl_dbbr_distinct_val_chooser IMPLEMENTATION.
  METHOD constructor.
    ...
    FIELD-SYMBOLS <lt_data_copy> TYPE table.

    ASSIGN ir_t_data->* TO <lt_data>.
    CREATE DATA mr_data LIKE <lt_data>.

    DATA(lr_extended_list) = zcl_uitb_data_list=>create_for_table_ref( ir_t_data = mr_data ).

    IF NOT lr_extended_list->has_component( 'LINE_INDEX' ).
      lr_extended_list->extend( it_comp_extend = VALUE #( ( component = 'LINE_INDEX' type = 'SYST-TABIX' ) ) ).
  ...
  METHOD do_on_first_screen_call.
    ...
    mr_alv = zcl_uitb_alv=>create_alv( ir_data      = REF #( mt_distinct_values )
  ...
  METHOD group_by_field.
    ...
    LOOP AT <lt_table> ASSIGNING FIELD-SYMBOL(<ls_row>).
      ...
      IF    <ls_distinct_value>            IS NOT ASSIGNED
         OR <ls_distinct_value>-fieldvalue <> <lv_field_value>.
      ...
      ELSE. " aggregate entry
        <ls_distinct_value>-count = <ls_distinct_value>-count + 1.
      ENDIF.
