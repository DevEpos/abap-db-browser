  ...
  METHOD create_default_variant_starter.
    CASE iv_entity_type.
      ...
      WHEN zif_sat_c_entity_type=>cds_view.
        result = NEW zcl_dbbr_cds_variant_starter( iv_cds_view   = CONV #( iv_variant_entity_id )
                                                   iv_variant_id = zif_dbbr_c_global=>c_dummy_variant ).
      WHEN zif_sat_c_entity_type=>query.
        result = NEW zcl_dbbr_query_var_starter( iv_query_id   = iv_variant_entity_id
                                                 iv_variant_id = zif_dbbr_c_global=>c_dummy_variant ).

      WHEN zif_sat_c_entity_type=>table.
        result = NEW zcl_dbbr_table_var_starter( iv_tabname    = CONV #( iv_variant_entity_id )
                                                 iv_variant_id = zif_dbbr_c_global=>c_dummy_variant ).
    ENDCASE.
  ENDMETHOD.
