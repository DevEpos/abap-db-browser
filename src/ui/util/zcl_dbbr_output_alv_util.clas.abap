...
CLASS zcl_dbbr_output_alv_util DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Hides empty columns</p>
    ...
    "! <p class="shorttext synchronized">Builds selection criteria from db filters</p>
    ...
    "! <p class="shorttext synchronized">Delete filters from selected columns</p>
    METHODS del_filter_from_selected_cols
      RETURNING
        VALUE(rf_filter_removed) TYPE abap_bool.

    "! <p class="shorttext synchronized">Group data by selected columns</p>
    METHODS execute_column_grouping.

    "! <p class="shorttext synchronized">Execute a quick filter for the selected cells</p>
    METHODS execute_quick_filter
      IMPORTING
        if_exclusion_mode TYPE abap_bool OPTIONAL.

    "! <p class="shorttext synchronized">Retrieve the count of filtered entries</p>
    METHODS get_filtered_count
      RETURNING
        VALUE(result) TYPE sy-tabix.

    "! <p class="shorttext synchronized">Retrieve all active filters</p>
    METHODS get_filters
      RETURNING
        VALUE(result) TYPE lvc_t_filt.

    "! <p class="shorttext synchronized">Get Filter from Selection Criteria</p>
    ...
    "! <p class="shorttext synchronized">Have the DB Filters changed?</p>
    METHODS have_db_filters_changed
      RETURNING
        VALUE(result) TYPE abap_bool.

    "! <p class="shorttext synchronized">Have the UI Filters changed?</p>
    METHODS have_ui_filters_changed
      RETURNING
        VALUE(result) TYPE abap_bool.

    "! <p class="shorttext synchronized">Checks if column grouping is active</p>
    METHODS is_column_grouping_active
      RETURNING
        VALUE(result) TYPE abap_bool.

    "! <p class="shorttext synchronized">Remove Grouping from Columns</p>
    METHODS remove_column_grouping.
    "! <p class="shorttext synchronized">Update the filters from ALV Grid</p>
    METHODS update_filters.

    "! <p class="shorttext synchronized">Sets the db filter</p>
    METHODS set_db_filter
      IMPORTING
        it_filter TYPE lvc_t_filt.

    "! <p class="shorttext synchronized">Updates the table reference</p>
  ...
  PROTECTED SECTION.
    ...
    "! <p class="shorttext synchronized">ALV control: Table of filter conditions</p>
    ...
    "! <p class="shorttext synchronized">Check fi the db filters changed</p>
    METHODS check_db_filter_changed.

    "! <p class="shorttext synchronized">Check if the given filter tables differ</p>
    ...
    "! <p class="shorttext synchronized">Check if the ui filters changed</p>
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS c_string_inttype TYPE string VALUE 'Cg' ##NO_TEXT.

    DATA mf_db_filters_changed TYPE abap_bool.
    DATA mf_ui_filters_changed TYPE abap_bool.

    "! <p class="shorttext synchronized">Check if two rows differ in their values</p>
    METHODS rows_differ
      IMPORTING
    ...
    "! <p class="shorttext synchronized">Handles String/LCHR columns</p>
    "!
    "! @parameter cs_filter | <p class="shorttext synchronized">The current ALV Filter value</p>
...
CLASS zcl_dbbr_output_alv_util IMPLEMENTATION.
  ...
  METHOD check_db_filter_changed.
    mf_db_filters_changed = check_filter_changed( CHANGING ct_filter_new = ms_new_filters-db_filter
  ...
  METHOD check_filter_changed.
    ...
    SORT ct_filter_old BY fieldname
                          low
                          high
                          sign
                          option.
    SORT ct_filter_new BY fieldname
                          low
                          high
                          sign
                          option.
    ...
    IF lines( ct_filter_new ) = lines( ct_filter_old ).
      ...
      DO lines( ct_filter_new ) TIMES.
        ...
        IF    lr_filter_old->fieldname <> lr_filter_new->fieldname
           OR lr_filter_old->low       <> lr_filter_new->low
           OR lr_filter_old->high      <> lr_filter_new->high
           OR lr_filter_old->option    <> lr_filter_new->option
           OR lr_filter_old->sign      <> lr_filter_new->sign.
  ...
  METHOD check_ui_filter_changed.
    mf_ui_filters_changed = check_filter_changed( CHANGING ct_filter_new = ms_new_filters-ui_filter
  ...
  METHOD del_filter_from_selected_cols.
    ...
    IF lt_selected_cols IS INITIAL.
      RETURN.
    ENDIF.

    lt_fields_range = VALUE #( FOR col IN lt_selected_cols
    ...
    IF rf_filter_removed = abap_false.
      RETURN.
    ENDIF.
  ...
  METHOD execute_column_grouping.
    ...
    FIELD-SYMBOLS <lv_field_value_unique> TYPE any.
    FIELD-SYMBOLS <lv_field_value> TYPE any.
    FIELD-SYMBOLS <ls_unique_row> TYPE any.

    DATA lt_sort_key TYPE abap_sortorder_tab.
    DATA lr_t_temp TYPE REF TO data.
    DATA lv_group_count TYPE sy-tabix.
    DATA lt_alv_sort_criteria TYPE lvc_t_sort.
    DATA lt_field_compare TYPE tt_compare_field.
    ...
    DATA(lt_col_selopt) = VALUE zif_sat_ty_global=>ty_t_selopt( FOR col IN lt_columns
                                                                ( sign   = 'I'
                                                                  option = 'EQ'
                                                                  low    = col-fieldname ) ).
    ...
    LOOP AT lt_fieldcat ASSIGNING FIELD-SYMBOL(<ls_field>).
      ...
      ELSE. " this is a sorting column
        ...
        IF     lf_is_numeric = abap_true
           AND (    <ls_field>-domname = zif_dbbr_c_global=>c_domain_names-timestamp
                 OR <ls_field>-domname = zif_dbbr_c_global=>c_domain_names-timestamp_long ).
          lf_is_numeric = abap_false.
        ENDIF.

        IF lf_is_numeric = abap_false.
          lt_sort_key = VALUE #( BASE lt_sort_key
    ...
    LOOP AT <lt_table> ASSIGNING FIELD-SYMBOL(<ls_row>).
      ...
      IF    <ls_unique_row> IS NOT ASSIGNED
         OR rows_differ( is_left_row   = <ls_row>
      ...
      ELSE. " aggregate entry
        ...
        LOOP AT lt_field_compare ASSIGNING <ls_group_field> WHERE is_numeric = abap_true.
          ...
          TRY.
              <lv_field_value_unique> = <lv_field_value_unique> + <lv_field_value>.
  ...
  METHOD execute_quick_filter.
    ...
    lt_selected_cells_internal = VALUE #( FOR cell IN lt_cells
    ...
    LOOP AT lt_selected_cells_internal ASSIGNING FIELD-SYMBOL(<ls_cell>).
      ...
      APPEND VALUE lvc_s_filt( fieldname = <ls_cell>-fieldname
  ...
  METHOD get_filter_from_criteria.
    ...
    LOOP AT it_criteria ASSIGNING FIELD-SYMBOL(<ls_selfield>)
      ...
      TRY.
          DATA(lr_selection_field) = mo_tabfields->get_field_ref( iv_tabname_alias = <ls_selfield>-tabname_alias
      ...
      zcl_sat_where_clause_builder=>get_option( EXPORTING iv_sign   = ls_filter-sign
                                                          iv_option = ls_filter-option
                                                          iv_high   = ls_filter-high
                                                CHANGING  cv_option = ls_filter-option
                                                          cv_low    = ls_filter-low ).
      rt_filter = VALUE #( BASE rt_filter
                           ( ls_filter ) ).
      ...
      LOOP AT it_criteria_multi ASSIGNING FIELD-SYMBOL(<ls_selfield_multi>)
           WHERE     fieldname = <ls_selfield>-fieldname
                 AND tabname   = <ls_selfield>-tabname
                 AND (    low    IS NOT INITIAL
                       OR high   IS NOT INITIAL
                       OR option IS NOT INITIAL ).
    ...
    IF mf_use_live_filter = abap_true AND mf_col_grouping_active = abap_false.
      LOOP AT lt_filter ASSIGNING FIELD-SYMBOL(<ls_filter>)
        ...
        TRY.
            ...
            IF    lr_field->is_text_field         = abap_true
               OR lr_field->is_formula_field      = abap_true
               OR lr_field->is_virtual_join_field = abap_true
               OR lr_field->is_virtual_element    = abap_true
               OR lr_field->inttype               = cl_abap_typedescr=>typekind_string
               OR lr_field->datatype              = 'LCHR'.
        ...
        <lt_filters> = VALUE #( BASE <lt_filters> FOR filter IN GROUP <ls_filter_group>
                                ( filter ) ).
      ENDLOOP.
  ...
  METHOD hide_empty_columns.
    ...
    DATA lf_hide_col TYPE abap_bool.
    DATA lv_type TYPE char1.

    FIELD-SYMBOLS <lt_data> TYPE table.

    lt_cols_to_hide = VALUE #( FOR field IN ct_fieldcat WHERE ( no_out = abap_false AND tech = abap_false )
                               ( sign = 'I' option = 'EQ' low = field-fieldname ) ).
