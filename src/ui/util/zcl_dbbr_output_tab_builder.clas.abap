"! <p class="shorttext synchronized">Creation of output structure</p>
CLASS zcl_dbbr_output_tab_builder DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Create dynamic table for data output</p>
    "!
    ...
    "! <p class="shorttext synchronized">Create/enhance table components for output table</p>
    "!
...
CLASS zcl_dbbr_output_tab_builder IMPLEMENTATION.
  METHOD add_text_comp_to_dyntab.
    DATA(ls_ddtext_text_field_def) = VALUE zdbbr_abap_comp_type( name        = ir_current_field->alv_fieldname
    ...
    ELSEIF lv_textfield_count > 1.
      ...
      ct_comp_type_tab = VALUE #( BASE ct_comp_type_tab
                                  ( ls_ddtext_text_field_def ) ).
    ELSE.
  ...
  METHOD create_dyn_comp_tab.
    ...
    WHILE ir_tabfields->has_more_lines( ).
      ...
      DATA(ls_comp) = VALUE zdbbr_abap_comp_type( name        = lr_current_field->alv_fieldname
      ...
      IF lr_current_field->is_text_field = abap_false.
        ...
        IF is_tech_info-use_reduced_memory = abap_true.
          ...
          IF    lr_current_field->output_active           = abap_true
             OR lr_current_field->has_active_text_field   = abap_true
             OR lr_current_field->is_calculation_field    = abap_true
             OR lr_current_field->is_key                  = abap_true
             OR lr_current_field->needed_for_virtual_join = abap_true
             OR lr_current_field->needed_for_virtual_elem = abap_true.

            rt_comp_tab = VALUE #( BASE rt_comp_tab
                                   ( ls_comp ) ).
          ENDIF.
      ...
      ELSE.
        ...
        IF     lr_current_field->is_text_field = abap_true
           AND lr_current_field->output_active = abap_true.
    ...
    rt_comp_tab = VALUE #( BASE rt_comp_tab
                           ( name        = zif_dbbr_c_special_out_columns=>line_index
                             ...
                             type_name   = COND #( WHEN if_active_grouping = abap_false
                                                   THEN 'SYST-TABIX'
                                                   ELSE 'ZDBBR_NO_OF_LINES' ) )
                           " cell color
  ...
  METHOD enhance_output_table_comps.
    ...
    zcl_uitb_rtti_util=>extend_table_by_components(
      EXPORTING it_data             = <lt_data>
                it_component_append = VALUE #( ( component = zif_dbbr_c_special_out_columns=>line_index
                                                 type      = 'SYST-TABIX' )
      ...
      IMPORTING er_line_type        = eo_line_type
