"! <p class="shorttext synchronized">Helper for virtual joins</p>
CLASS zcl_dbbr_virtual_join_helper DEFINITION
  PUBLIC
  CREATE PRIVATE.

  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Creates new instance of virtual join helper</p>
    "!
    ...
    "! <p class="shorttext synchronized">Fills the cache tables with values</p>
    "!
    ...
    "! <p class="shorttext synchronized">Checks if post SQL Join exists</p>
    "!
    METHODS post_join_exists
      RETURNING
        VALUE(result) TYPE abap_bool.

    "! <p class="shorttext synchronized">Process the the virtual joins for each line</p>
    "!
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Builds where clause for all entries cache select</p>
    "!
    ...
    "! <p class="shorttext synchronized">Builds where clause for cache select</p>
    "!
    ...
    "! <p class="shorttext synchronized">Builds where condition out of join and current output line</p>
    "!
    ...
    "! <p class="shorttext synchronized">Creates all needed cache tables</p>
    "!
    METHODS create_cache_tables.

    "! <p class="shorttext synchronized">Fills cache with FOR ALL ENTRIES Select</p>
    "!
    ...
    "! <p class="shorttext synchronized">Fills cache with normal SELECT due to needed conversion</p>
    "!
    ...
    "! <p class="shorttext synchronized">Fills a single cache table with values</p>
    "!
    ...
    "! <p class="shorttext synchronized">Fills all the virtual output fields for a single line</p>
    "!
    "!
    ...
    "! <p class="shorttext synchronized">Get correct value for condition for WHERE Clause</p>
    "!
    ...
    "! <p class="shorttext synchronized">Parse filter conditions for virtual join table</p>
    "!
    ...
    "! <p class="shorttext synchronized">Parse field conditions for virtual join table</p>
    "!
    ...
    "! <p class="shorttext synchronized">Processes a single line in the output table</p>
    "!
...
CLASS zcl_dbbr_virtual_join_helper IMPLEMENTATION.
  METHOD build_for_all_where_clause.
    ...
    LOOP AT is_post_join-field_cond ASSIGNING FIELD-SYMBOL(<ls_field_cond>).
      ...
      DATA(lv_length_suffix) = COND string(
        WHEN lv_offset IS NOT INITIAL AND lv_length IS NOT INITIAL THEN |+{ lv_offset }({ lv_length })|
        WHEN lv_length IS NOT INITIAL                              THEN |({ lv_length })| ).
  ...
  METHOD build_where_for_cache_query.
    ...
    LOOP AT is_virtual_join-field_cond ASSIGNING FIELD-SYMBOL(<ls_field_cond>).
      ...
      DATA(lv_value) = cl_abap_dyn_prg=>quote( get_value_for_where( iv_value      = <lv_ref_value>
...
*    ).
  ...
  METHOD constructor.
    ...
    LOOP AT is_join_def-tables ASSIGNING FIELD-SYMBOL(<ls_join_table>) WHERE is_virtual = abap_true.
      DATA(ls_post_join) = VALUE zdbbr_post_join_data( join_table = <ls_join_table>-add_table
      ...
      LOOP AT <ls_join_table>-field_conditions ASSIGNING FIELD-SYMBOL(<ls_join_field>).
        ...
        TRY.
            DATA(lr_ref_field) = mr_fields_all->get_field_ref( iv_tabname_alias = <ls_join_field>-ref_table
        ...
        IF ls_join_field-field_inttype <> ls_join_field-ref_field_inttype.
          ...
          ls_join_field-field_type_ref       = CAST #( cl_abap_typedescr=>describe_by_name(
                                                           |{ <ls_join_table>-add_table }-{ ls_join_field-field }| ) ).
        ENDIF.

        ls_post_join-field_cond = VALUE #( BASE ls_post_join-field_cond
                                           ( ls_join_field  ) ).
      ENDLOOP.
      ...
      LOOP AT <ls_join_table>-filter_conditions ASSIGNING FIELD-SYMBOL(<ls_filter_cond>).
        ...
        ELSE.
          TRY.
              lr_field = mr_fields_all->get_field_ref( iv_tabname_alias = <ls_filter_cond>-tabname
        ...
        ls_post_join-filter_cond = VALUE #( BASE ls_post_join-filter_cond
                                            ( ls_filter_cond ) ).
      ENDLOOP.
      ...
      mt_post_join_data = VALUE #( BASE mt_post_join_data
                                   ( ls_post_join ) ).
    ENDLOOP.
  ...
  METHOD create_cache_tables.
    ...
    LOOP AT mt_post_join_data ASSIGNING FIELD-SYMBOL(<ls_post_join>).
      ...
      lt_fields = VALUE #( FOR <ls_key_field> IN <ls_post_join>-field_cond
                           ( fieldname = <ls_key_field>-field
                             tabname   = <ls_post_join>-join_table
                             keyflag   = abap_true ) ).

      " get output fields for join table
      lt_fields = VALUE zsat_dfies_itab( BASE lt_fields
                                         FOR <ls_field> IN mt_conditional_fields
                                         WHERE (     output_active = abap_true
                                                 AND tabname       = <ls_post_join>-join_table )
      ...
      <ls_post_join>-data_cache   = zcl_dbbr_ddic_util=>build_dynamic_sorted_table(
                                        it_fields     = lt_fields
...
*          lt_where  TYPE TABLE OF string
    ...
    lt_select = VALUE #( FOR <ls_field> IN cs_post_join-cache_fields
                         ( |{ <ls_field>-fieldname }| ) ).
  ...
  METHOD fill_cache_single_select.
    ...
    DATA(lr_cache_table_temp) = zcl_dbbr_ddic_util=>build_dynamic_std_table( it_fields = cs_post_join-cache_fields ).
    ...
    LOOP AT it_table ASSIGNING FIELD-SYMBOL(<ls_data>).
      ...
      DATA(lt_where) = zcl_sat_join_helper=>build_where_for_conditions(
                           VALUE #( ( LINES OF
                                      parst_virtjtab_field_cond( is_line       = <ls_data>
  ...
  METHOD fill_output_fields.
    ...
    FIELD-SYMBOLS <lt_new_lines> TYPE STANDARD TABLE.
    ...
    DATA(lt_where) = build_where_for_cache_query( is_virtual_join = is_virtual_join
                                                  is_line         = cs_line ).
    ...
    LOOP AT it_cache ASSIGNING FIELD-SYMBOL(<ls_cache_line>) WHERE (lt_where).
      ...
      LOOP AT <lt_existing> ASSIGNING FIELD-SYMBOL(<ls_new_line>).
        LOOP AT mt_conditional_fields ASSIGNING FIELD-SYMBOL(<ls_cond_field>) WHERE tabname = is_virtual_join-join_table.
          ...
          IF     <lv_output_field_value> IS ASSIGNED
             AND <lv_cache_value>        IS ASSIGNED.
        ...
        <lt_new_lines> = VALUE #( BASE <lt_new_lines>
                                  ( <ls_new_line> ) ).
      ENDLOOP.
    ENDLOOP.
  ...
  METHOD get_value_for_where.
    ...
    IF is_field_cond-is_conversion_needed = abap_true.
      ...
      IF    is_field_cond-field_inttype = cl_abap_typedescr=>typekind_char
         OR is_field_cond-field_inttype = cl_abap_typedescr=>typekind_clike
         OR is_field_cond-field_inttype = cl_abap_typedescr=>typekind_string.
        <lv_converted> = condense( <lv_converted> ).
      ENDIF.
  ...
  METHOD parse_virtjtab_filter_cond.
    ...
    DATA lf_parenthesis_opened TYPE abap_bool.
    DATA lv_parenthesis_open TYPE string.
    DATA lv_parenthesis_closed TYPE string.
    ...
    LOOP AT it_filter_cond INTO DATA(ls_filter_cond) WHERE is_for_add_table = abap_true.
      ...
      IF    ls_filter_cond-operator = zif_sat_c_operator=>like
         OR ls_filter_cond-operator = zif_sat_c_operator=>not_like.
        ls_filter_cond-value = replace( val  = ls_filter_cond-value
                                        sub  = '*'
                                        with = '%'
                                        occ  = 0 ).

        IF ls_filter_cond-value2 IS NOT INITIAL.
          ls_filter_cond-value2 = replace( val  = ls_filter_cond-value2
                                           sub  = '*'
                                           with = '%'
                                           occ  = 0 ).
        ENDIF.
      ...
      DATA(lv_value1) = COND #(
        WHEN ls_filter_cond-value_type = zif_sat_c_join_cond_val_type=>system_value_input
        THEN |@{ ls_filter_cond-value }|
        ELSE cl_abap_dyn_prg=>quote( ls_filter_cond-value ) ).

      IF ls_filter_cond-operator = 'BETWEEN'.
        lv_value2 = COND #(
          WHEN ls_filter_cond-value_type = zif_sat_c_join_cond_val_type=>system_value_input
          THEN |@{ ls_filter_cond-value2 }|
          ELSE cl_abap_dyn_prg=>quote( ls_filter_cond-value2 ) ).
        lv_value2 = | AND { lv_value2 }|.
      ENDIF.

      rt_conditions = VALUE #( BASE rt_conditions
                               ( open_bracket    = lv_parenthesis_open
                                 join_operator   = COND #( WHEN lv_index = 1
                                                           THEN mv_and_string
                                                           ELSE lv_previous_join_cond )
                                 value           = |{ ls_filter_cond-fieldname }| &&
                                                   | { ls_filter_cond-operator } | &&
                                                   |{ lv_value1 }{ lv_value2 }|
                                 closing_bracket = lv_parenthesis_closed ) ).

      lv_previous_join_cond = COND #( WHEN ls_filter_cond-and_or = zif_sat_c_selection_condition=>or
                                      THEN mv_or_string
                                      ELSE mv_and_string ).
  ...
  METHOD process_table.
    ...
    zcl_dbbr_screen_helper=>show_progress( iv_progress = 50
    ...
    LOOP AT <lt_data> ASSIGNING FIELD-SYMBOL(<ls_data>).
      process_line( CHANGING cs_line        = <ls_data>
                             cr_t_new_lines = lr_t_temp ).
      DELETE <lt_data>.
    ENDLOOP.
