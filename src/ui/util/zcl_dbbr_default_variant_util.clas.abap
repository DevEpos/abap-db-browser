"! <p class="shorttext synchronized">Utility for Handling Default Entity Variant</p>
CLASS zcl_dbbr_default_variant_util DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Loads defaultVariant</p>
    ...
    "! <p class="shorttext synchronized">Deletes default Variant</p>
    CLASS-METHODS delete_default_variant
      IMPORTING
        io_selscreen_util TYPE REF TO zcl_dbbr_selscreen_util.

    "! <p class="shorttext synchronized">Creates default variant</p>
...
CLASS zcl_dbbr_default_variant_util IMPLEMENTATION.
  METHOD load_default_variant.
    io_selscreen_util->get_entity_information( IMPORTING ev_type       = DATA(lv_entity_type)
  ...
  METHOD delete_default_variant.
    io_selscreen_util->get_entity_information( IMPORTING ev_type       = DATA(lv_entity_type)
                                                         ev_entity_raw = DATA(lv_entity_name)
                                                         ev_entity_id  = DATA(lv_entity_id) ).
    DATA(lf_deleted) = zcl_dbbr_variant_factory=>delete_default_variant( iv_entity_id   = lv_entity_id
                                                                         iv_entity_type = lv_entity_type ).
  ...
  METHOD create_default_variant.
    ...
    IF     io_data->get_mode( ) = zif_dbbr_c_selscreen_mode=>table
       AND io_data->is_join_active( ).
      MESSAGE e044(zdbbr_exception) DISPLAY LIKE 'I'.
      RETURN.
    ENDIF.

    io_selscreen_util->get_entity_information( IMPORTING ev_entity_raw = DATA(lv_entity_name)
                                                         ev_entity_id  = DATA(lv_entity_id) ).
