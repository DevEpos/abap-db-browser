"! <p class="shorttext synchronized">Utility class for Variants of an Entity</p>
CLASS zcl_dbbr_variant_util DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Choose variant for given entity</p>
...
CLASS zcl_dbbr_variant_util IMPLEMENTATION.
  METHOD select_and_load_variant.
    DATA(lo_variant_f4) = NEW zcl_dbbr_variant_f4_view( iv_entity_id   = iv_entity_id
                                                        iv_entity_name = iv_entity_name
                                                        iv_entity_type = iv_entity_type ).

    DATA(lv_variant_id) = lo_variant_f4->choose_variant( ).
    IF lv_variant_id IS INITIAL.
      RETURN.
    ENDIF.
