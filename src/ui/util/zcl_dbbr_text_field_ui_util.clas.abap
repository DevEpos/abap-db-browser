"! <p class="shorttext synchronized">Text Field Util for UI</p>
CLASS zcl_dbbr_text_field_ui_util DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Adds text field columns for selected columns</p>
    METHODS add_text_field_columns.

    "! <p class="shorttext synchronized">Manages text field columns</p>
    METHODS manage_text_field_columns.

    "! <p class="shorttext synchronized">Adds 'Add Text Field' function to context menu</p>
    "!
    "! @parameter it_selected_cols | <p class="shorttext synchronized">List of selected columns</p>
    "! @parameter it_fieldcat      | <p class="shorttext synchronized">ALV Field Catalog</p>
    "! @parameter ct_menu_entries  | <p class="shorttext synchronized">List of context menu entries</p>
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Updates text fields in ALV</p>
    "!
    "! @parameter it_fields | <p class="shorttext synchronized"></p>
...
CLASS zcl_dbbr_text_field_ui_util IMPLEMENTATION.
  METHOD add_text_field_columns.
    ...
    LOOP AT lt_cols ASSIGNING FIELD-SYMBOL(<ls_col>).
      DATA(lr_field) = mo_selection_util->mo_tabfields->get_field_ref_by_alv_name(
                           iv_alv_fieldname = <ls_col>-fieldname ).
      CHECK lr_field->has_text_field = abap_true.

      TRY.
          DATA(lr_text_field) = lo_tabfields->get_field_ref( iv_tabname_alias = lr_field->tabname_alias
      ...
      IF sy-subrc <> 0.
        ...
        lt_new_fields = VALUE #( BASE lt_new_fields
                                 ( lr_text_field ) ).
  ...
  METHOD manage_text_field_columns.
    ...
    DATA(lo_text_field_col_selector) = NEW zcl_dbbr_text_field_selector( io_tabfields = mo_selection_util->mo_tabfields
                                                                         it_fieldcat  = lt_fieldcat ).

    DATA(lt_text_field_status) = lo_text_field_col_selector->get_text_fields( ).
    IF lt_text_field_status IS INITIAL.
      RETURN.
    ENDIF.
    ...
    LOOP AT lt_text_field_status ASSIGNING FIELD-SYMBOL(<ls_text_field>) WHERE visible = abap_true.
      <ls_text_field>-new_field = xsdbool( NOT line_exists( lt_fieldcat[
                                                                fieldname = <ls_text_field>-alv_fieldname ] ) ).
    ENDLOOP.
...
*      CLEAR mo_selection_util->mt_add_texts.
      zcl_dbbr_addtext_helper=>prepare_text_fields( EXPORTING ir_fields    = lo_tabfields
    ...
    LOOP AT it_fields ASSIGNING FIELD-SYMBOL(<ls_field>).
      ...
      ELSE.
        lt_fieldcat[ fieldname = <ls_field>-alv_fieldname
    ...
    IF lf_reset_alv_table = abap_true.
      ...
      mo_selection_util->get_alv_util( )->update_result_table_ref( ir_t_data = mo_selection_util->mr_t_data ).
