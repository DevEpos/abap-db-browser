"! <p class="shorttext synchronized">Selection Util for SQL Query</p>
CLASS zcl_dbbr_sql_query_selctn_util DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS execute_selection                              REDEFINITION.
    METHODS init                                           REDEFINITION.
    METHODS refresh_selection                              REDEFINITION.
    METHODS zif_dbbr_screen_util~get_deactivated_functions REDEFINITION.
    METHODS build_simple_alv_title                         REDEFINITION.

  PROTECTED SECTION.
    METHODS create_dynamic_table REDEFINITION.
    METHODS create_field_catalog REDEFINITION.
    METHODS is_f4_saving_allowed REDEFINITION.
    METHODS select_data          REDEFINITION.
    METHODS get_sql_query        REDEFINITION.
    METHODS before_selection     REDEFINITION.
    METHODS after_selection      REDEFINITION.
    METHODS has_result           REDEFINITION.
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Parse/Set parameters for query</p>
...
CLASS zcl_dbbr_sql_query_selctn_util IMPLEMENTATION.
  ...
  METHOD refresh_selection.
    ...
    zcl_dbbr_sql_query_exec=>execute_query( EXPORTING io_query          = mo_query
                                                      iv_row_count      = ms_technical_info-max_lines
                                                      if_count_only     = mf_count_lines
                                            IMPORTING et_data_info      = mt_query_result_col
                                                      ev_execution_time = mv_execution_time_str
                                                      er_data           = mr_query_result ).

    IF mv_query_history_id IS NOT INITIAL.
      zcl_dbbr_sql_query_factory=>update_execution_time( iv_query_id  = mv_query_history_id
                                                         iv_exec_time = CONV #( replace( val  = mv_execution_time_str
                                                                                         sub  = ' ms'
                                                                                         with = '' ) ) ).
    ENDIF.
  ...
  METHOD build_simple_alv_title.
    result = COND #(
      WHEN mv_entity_id IS NOT INITIAL
      THEN |{ 'Result of'(001) } { mv_entity_id }|
  ...
  METHOD init.
    ...
    ELSE.
      ls_query = lo_query_f->get_query( iv_query_name      = mv_entity_id
  ...
  METHOD set_parameters_in_query.
    LOOP AT mt_param_values ASSIGNING FIELD-SYMBOL(<ls_param>)
      ...
      IF ls_query_param-is_range = abap_true.
        mo_query->set_parameter_value( iv_name        = ls_query_param-name
  ...
  METHOD create_dynamic_table.
    ...
    zcl_dbbr_output_tab_builder=>enhance_output_table_comps( EXPORTING ir_table      = mr_query_result
                                                             IMPORTING eo_line_type  = mo_query_result_line_type
  ...
  METHOD create_field_catalog.
    ...
    LOOP AT lt_dfies ASSIGNING FIELD-SYMBOL(<ls_dfies>).
      DATA(lf_tech) = COND #(
        WHEN <ls_dfies>-fieldname = zif_dbbr_c_special_out_columns=>hide_flag
          OR <ls_dfies>-fieldname = zif_dbbr_c_special_out_columns=>line_index
        THEN abap_true ).
      DATA(ls_field) = VALUE lvc_s_fcat( fieldname  = <ls_dfies>-fieldname
      ...
      IF     ls_field-scrtext_s IS INITIAL
         AND ls_field-scrtext_m IS INITIAL
         AND ls_field-scrtext_l IS INITIAL.
  ...
  METHOD get_sql_query.
    ...
    rv_query = COND #(
        WHEN if_for_console = abap_true
        THEN mo_query->ms_data-source
        ELSE mo_query->ms_data-select_source ).
  ENDMETHOD.
