CLASS zcl_dbbr_cds_navigator DEFINITION
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Determine Parameter String</p>
    "!
    "! @parameter it_params        | <p class="shorttext synchronized">List of CDS parameters</p>
    "! @parameter iv_cds_view_name | <p class="shorttext synchronized">CDS view name</p>
    "! @parameter io_tabfields     | <p class="shorttext synchronized">Table fields</p>
    ...
    "! <p class="shorttext synchronized">Request parameter value input</p>
    "!
    "! @parameter it_params        | <p class="shorttext synchronized">List of parameters</p>
    "! @parameter iv_cds_view_name | <p class="shorttext synchronized">CDS view name</p>
    "! @parameter io_tabfields     | <p class="shorttext synchronized">Table fields</p>
    "! @parameter rt_param_values  | <p class="shorttext synchronized">List of parameter/value</p>
...
CLASS zcl_dbbr_cds_navigator IMPLEMENTATION.
  ...
  METHOD create_output_fields.
    ...
    CASE ms_association-kind.
      ...
      WHEN zif_sat_c_cds_assoc_type=>table OR
        ...
        zcl_dbbr_tabfield_builder=>create_tabfields( iv_tablename     = ms_association-ref_cds_view
      ...
      WHEN OTHERS.
        ...
        zcl_dbbr_cds_tabfield_util=>add_view_colums( ir_tabfield_list = mr_tabfields
  ...
  METHOD fill_selection_fields.
    ...
    DATA lt_or_tab TYPE zif_sat_ty_global=>ty_t_or_seltab_sql.
    DATA lt_cond_tab TYPE zif_sat_ty_global=>ty_t_seltab_sql.
    ...
    LOOP AT mt_source_index ASSIGNING FIELD-SYMBOL(<lv_index>).
      ...
      IF lt_cond_tab IS NOT INITIAL AND lines( lt_key_fields ) = lines( lt_cond_tab ).
        lt_or_tab = VALUE #( BASE lt_or_tab
                             ( values = lt_cond_tab ) ).
      ENDIF.
    ENDLOOP.
    ...
    LOOP AT lr_t_target_fields->* ASSIGNING FIELD-SYMBOL(<ls_target_key_field>) WHERE datatype <> 'CLNT'.
      EXIT.
    ENDLOOP.
    IF <ls_target_key_field> IS NOT ASSIGNED.
      RETURN.
    ENDIF.
    ...
    lt_cond_tab = VALUE #(
        ( sign         = 'I'
          ...
          sqlfieldname = |\\{ ms_association-raw_name }{ mv_params }-{ <ls_target_key_field>-fieldname_raw }| ) ).
    DATA(lt_not_null_cond) = VALUE zif_sat_ty_global=>ty_t_or_seltab_sql( ( values = lt_cond_tab ) ).

    lt_and_tab = VALUE #( ( lt_not_null_cond ) ).
  ...
  METHOD zif_dbbr_table_navigator~navigate.
    ...
    IF mv_nav_count = 7.
      RAISE EXCEPTION TYPE zcx_dbbr_association_nav_error
        EXPORTING textid = zcx_dbbr_association_nav_error=>max_navigation_level_reached.
    ENDIF.
    ...
    " ... start new transaction for association select
  ...
  METHOD request_param_values.
    ...
    " ... create parameters
    zcl_dbbr_cds_tabfield_util=>add_parameters( ir_tabfield_list = io_tabfields
                                                io_custom_f4_map = lo_custom_f4_map
                                                it_parameters    = it_params ).

    DATA(lo_param_popup) = NEW zcl_dbbr_cds_param_popup( io_tabfields     = io_tabfields
    ...
    IF rt_param_values IS INITIAL.
      RAISE EXCEPTION TYPE zcx_dbbr_association_nav_error
        EXPORTING textid = zcx_dbbr_association_nav_error=>assoc_param_not_available.
    ENDIF.
  ENDMETHOD.

  METHOD determine_param_string.
    mv_params = zcl_dbbr_cds_param_util=>build_param_string( iv_param_indentation    = strlen( iv_cds_view_name )
