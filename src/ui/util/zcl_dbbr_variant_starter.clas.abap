"! <p class="shorttext synchronized">Starting DB Browser with variant</p>
CLASS zcl_dbbr_variant_starter DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! @parameter IV_VARIANT_ID | <p class="shorttext synchronized"></p>
    METHODS constructor
      IMPORTING
  ...
  PROTECTED SECTION.
    ...
    "! <p class="shorttext synchronized">Create table field</p>
    "!
    ...
    "! <p class="shorttext synchronized">Fills data from variant</p>
    "!
    METHODS fill_data_from_variant
      RAISING
        zcx_dbbr_variant_error.

    "! <p class="shorttext synchronized">Fill selection screen with tuple data</p>
    "!
    METHODS fill_selscreen_with_tupledata
      IMPORTING
        it_variant_data TYPE zdbbr_vardata_itab.

    "! <p class="shorttext synchronized">Fill selection screen with normal variant data</p>
    "!
    METHODS fill_selscreen_with_vardata
      IMPORTING
        it_variant_data TYPE zdbbr_vardata_itab.

    "! <p class="shorttext synchronized">Fill selection screen table</p>
    "!
    METHODS fill_primary_entity
      ABSTRACT.

    "! <p class="shorttext synchronized">Retrieve table field references for execution</p>
    "!
    ...
    "! <p class="shorttext synchronized">Loads the variant</p>
    "!
    METHODS load_variant.
    "! <p class="shorttext synchronized">Shows progress text during execution</p>
    "!
...
CLASS zcl_dbbr_variant_starter IMPLEMENTATION.
  ...
  METHOD create_table_field.
    " TODO: parameter IF_FIRST_NON_KEY_FIELD is never used (ABAP cleaner)
...
*... default sign is inclusive
    ...
    IF lr_new_field IS NOT INITIAL.
      ...
      IF lr_addtext_bl->text_exists( is_data_element_info = is_field ).
        ...
        lr_addtext_bl->add_text_fields_to_list( ir_tabfields    = mo_tabfield_list
  ...
  METHOD create_cds_fields.
    TRY.
        ...
        " ...... create parameters
        zcl_dbbr_cds_tabfield_util=>add_parameters( ir_tabfield_list = mo_tabfield_list
                                                    it_parameters    = lo_cds_view->get_parameters( ) ).
        " ...... create table fields for cds view
        " TODO: variable is assigned but never used (ABAP cleaner)
        DATA(ls_header) = lo_cds_view->get_header( ).

        rs_entity_info = zcl_dbbr_cds_tabfield_util=>add_view_colums( ir_tabfield_list = mo_tabfield_list
  ...
  METHOD create_table_fields.
    ...
    zcl_dbbr_addtext_bl=>get_instance( )->determine_text_fields( iv_entity      = is_entity_info-tabname
    ...
    LOOP AT lt_dfies ASSIGNING FIELD-SYMBOL(<ls_data_element_field>) WHERE datatype <> 'CLNT'.
      create_table_field( is_field  = <ls_data_element_field>
                          is_entity = is_entity_info ).
    ENDLOOP.

    mo_tabfield_list->add_table( VALUE zdbbr_entity_info( active_selection  = is_entity_info-active_selection
  ...
  METHOD fill_data_from_variant.
    ...
    SORT ms_variant-variant_data BY tuple_id
                                    tabname
                                    fieldname
                                    counter.
    ...
    " TODO: variable is assigned but never used (ABAP cleaner)
    LOOP AT ms_variant-variant_data ASSIGNING FIELD-SYMBOL(<ls_vard>) WHERE    data_type = zif_dbbr_c_global=>c_variant_datatypes-group_by
                                                                            OR data_type = zif_dbbr_c_global=>c_variant_datatypes-aggregation.
    ...
    LOOP AT ms_variant-variant_data ASSIGNING FIELD-SYMBOL(<ls_vardata>)
      ...
      IF <ls_vardata_tuple>-tuple IS NOT INITIAL.
        fill_selscreen_with_tupledata( VALUE #( FOR data IN GROUP <ls_vardata_tuple>
                                                ( data ) ) ).
      ELSE.
        fill_selscreen_with_vardata( VALUE #( FOR data IN GROUP <ls_vardata_tuple>
                                              ( data ) ) ).
      ENDIF.
    ENDLOOP.

    IF     lf_grouping_is_active = abap_false
       AND (    ms_variant-has_output_fields = abap_true
             OR ms_variant-has_sort_fields   = abap_true ).
      mo_tabfield_list->clear_active_flag( if_clear_output = ms_variant-has_output_fields
    ...
    IF ms_variant-has_output_fields = abap_true OR ms_variant-has_sort_fields = abap_true.
      ...
      LOOP AT ms_variant-fields ASSIGNING FIELD-SYMBOL(<ls_tabfield>).
        ...
        TRY.
            DATA(lr_field_stru) = mo_tabfield_list->get_field_ref( iv_tabname_alias = <ls_tabfield>-tabname
  ...
  METHOD fill_selscreen_with_tupledata.
    DATA(ls_multi_or) = VALUE zdbbr_or_seltab( pos = it_variant_data[ 1 ]-tuple_id ).
    ...
    LOOP AT it_variant_data ASSIGNING FIELD-SYMBOL(<ls_vardata>)
      ...
      LOOP AT GROUP <ls_vardata_group> ASSIGNING FIELD-SYMBOL(<ls_vardata_group_entry>).
        IF lf_first_iteration = abap_true.
          DATA(ls_selfield_info) = VALUE zdbbr_selfield_info( tabname   = <ls_vardata_group_entry>-tabname
        ...
        ELSE.
          ls_selfield_info-multi_values = VALUE #( BASE ls_selfield_info-multi_values
                                                   ( sign   = <ls_vardata_group_entry>-sign_val
                                                     option = <ls_vardata_group_entry>-data_type
                                                     low    = <ls_vardata_group_entry>-low_val
                                                     high   = <ls_vardata_group_entry>-high_val ) ).
      ...
      ls_multi_or-values = VALUE #( BASE ls_multi_or-values
                                    ( ls_selfield_info ) ).
    ENDLOOP.

    mt_selfields_or = VALUE #( BASE mt_selfields_or
                               ( ls_multi_or ) ).
  ENDMETHOD.
  ...
  METHOD fill_selscreen_with_vardata.
    LOOP AT it_variant_data ASSIGNING FIELD-SYMBOL(<ls_vardata>)
      ...
      DATA(lt_vardata_group) = VALUE zdbbr_vardata_itab( FOR data IN GROUP <ls_vardata_group>
                                                         ( data ) ).
      ...
      IF sy-subrc <> 0.
        ...
        TRY.
            DATA(lr_field_ref) = mo_tabfield_list->get_field_ref( iv_tabname_alias = lv_tabname_alias
      ...
      LOOP AT lt_vardata_group ASSIGNING FIELD-SYMBOL(<ls_vardata_group_entry>).
        CASE <ls_vardata_group_entry>-data_type.
          ...
          WHEN zif_dbbr_c_global=>c_variant_datatypes-totals.
            <ls_selfield>-totals = abap_true.

          WHEN zif_dbbr_c_global=>c_variant_datatypes-aggregation.
            <ls_selfield>-aggregation = <ls_vardata_group_entry>-low_val.

          WHEN zif_dbbr_c_global=>c_variant_datatypes-group_by.
          ...
          WHEN OTHERS. " normal selection field
            IF lf_first_selvalue = abap_true.
              ...
              IF <ls_vardata_group_entry>-system_value_type IS NOT INITIAL.
                zcl_sat_system_helper=>get_system_value(
                  EXPORTING iv_system_value_type = <ls_vardata_group_entry>-system_value_type
  ...
  METHOD load_variant.
    ...
    ELSE.
      zcl_dbbr_variant_factory=>get_variant( EXPORTING iv_variant_id = mv_variant_id
