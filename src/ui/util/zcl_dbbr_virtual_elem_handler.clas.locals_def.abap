    ...
    BEGIN OF sadl_mpd_exposure,
      ...
      BEGIN OF meth,
        BEGIN OF get_exposure,
          name TYPE string VALUE 'GET_EXPOSURE_ENTITY_ID',
          BEGIN OF param,
...
CLASS lcl_sadl_mdp_exposure DEFINITION.
  ...
  PUBLIC SECTION.
    CLASS-METHODS get_exposure_load_id
      IMPORTING
    ...
    CLASS-METHODS create_method_param_data
      IMPORTING
...
CLASS lcl_sadl_mdp_factory DEFINITION.
  ...
  PUBLIC SECTION.
    ...
    METHODS get_mdp_for_id
      RETURNING
        VALUE(rr_mdp) TYPE REF TO data.

    METHODS get_entity_load_by_id
      RETURNING
        VALUE(rr_entity_load) TYPE REF TO data.

  PRIVATE SECTION.
    DATA mr_load_id TYPE REF TO data.

    METHODS create_metadata_provider_ref
      RETURNING
        VALUE(rr_mdp) TYPE REF TO data.
ENDCLASS.
