CLASS zcl_dbbr_query_var_starter DEFINITION
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Handle join entities</p>
    "!
    METHODS handle_join_tables.
    "! <p class="shorttext synchronized">Load the query from the database</p>
    "!
    METHODS load_query.
    "! <p class="shorttext synchronized">Fill necessary information for execution</p>
    "!
...
CLASS zcl_dbbr_query_var_starter IMPLEMENTATION.
  ...
  METHOD handle_join_tables.
    ...
    LOOP AT ms_join_def-tables ASSIGNING FIELD-SYMBOL(<ls_join_table_info>).
      DATA(ls_join_entity) = VALUE zdbbr_entity_info(
          ...
          selection_order    = VALUE #( mt_table_list[
                                            tabname_alias = <ls_join_table_info>-add_table_alias ]-selection_order OPTIONAL ) ).
  ...
  METHOD load_variant.
    IF mv_variant_id = zif_dbbr_c_global=>c_dummy_variant.
      ...
      DATA(lv_default_variant_id) = zcl_dbbr_variant_factory=>find_default_query_variant(
                                        iv_query_id = ms_query-query_id ).
  ...
  METHOD fill_secondary_data.
    ...
    mo_tabfield_list->clear_active_flag( if_clear_selection = abap_true
    ...
    IF ms_query-formula IS NOT INITIAL.
      TRY.
          mo_formula = NEW zcl_dbbr_fe_validator( iv_formula   = ms_query-formula
                                                  io_tabfields = mo_tabfield_list
          )->validate( ).

          zcl_dbbr_formula_helper=>update_tabflds_from_formula( ir_tabfields      = mo_tabfield_list
  ...
  METHOD load_query.
    ...
    ms_query = COND #( WHEN mv_variant_id = zif_dbbr_c_global=>c_dummy_variant
                       THEN mr_query_f->get_query( iv_query_name = CONV #( mv_query_id ) )
                       ELSE mr_query_f->get_query_by_id( CONV #( mv_query_id ) ) ).
  ...
  METHOD zif_dbbr_variant_starter~execute_variant.
    ...
    get_tabfields( IMPORTING er_tabfields     = DATA(lr_tabfields)
    ...
    DATA(lr_controller) = zcl_dbbr_selection_controller=>create_controller(
        VALUE #(
            ...
            exclude_function = VALUE #( ( zif_dbbr_c_selection_functions=>leave_screen_with_layout )
  ...
  METHOD zif_dbbr_variant_starter~initialize.
    ...
    IF     mv_variant_id                  = zif_dbbr_c_global=>c_dummy_variant
       AND ms_global_data-called_from_adt = abap_false.
      RETURN.
    ENDIF.

    IF ms_global_data-called_from_adt = abap_true.
      zcl_dbbr_usersettings_factory=>update_start_settings( iv_entity_id   = ms_query-query_name
                                                            iv_entity_type = zif_sat_c_entity_type=>query ).
    ENDIF.

    NEW zcl_dbbr_favmenu_factory( )->refresh_most_used( iv_entry     = ms_query-query_name
