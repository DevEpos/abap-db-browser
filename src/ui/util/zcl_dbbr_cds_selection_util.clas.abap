"! <p class="shorttext synchronized">Utitlity methods for CDS Views for Selection Controller</p>
CLASS zcl_dbbr_cds_selection_util DEFINITION
  PUBLIC
  INHERITING FROM zcl_dbbr_selection_util FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    METHODS build_simple_alv_title                         REDEFINITION.
    METHODS fill_header                                    REDEFINITION.
    METHODS get_entity_name                                REDEFINITION.
    METHODS handle_alv_ctx_menu_request                    REDEFINITION.
    METHODS init                                           REDEFINITION.
    METHODS zif_dbbr_screen_util~free_resources            REDEFINITION.
  ...
  PROTECTED SECTION.
    METHODS create_from_clause  REDEFINITION.
    METHODS read_entity_infos   REDEFINITION.
    METHODS ignore_empty_result REDEFINITION.
    METHODS before_selection    REDEFINITION.
    METHODS after_selection     REDEFINITION.
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Change CDS parameter values</p>
    METHODS change_parameters.
    "! <p class="shorttext synchronized">Choose Association for navigation</p>
    METHODS choose_association.

    "! <p class="shorttext synchronized">Performs Association navigation</p>
    "! @parameter IS_ASSOC | <p class="shorttext synchronized"></p>
    METHODS navigate
      IMPORTING
        is_assoc TYPE zsat_cds_association.

    "! <p class="shorttext synchronized">Shows the Source Code of the CDS</p>
    METHODS show_cds_source.

    "! <p class="shorttext synchronized">Get virtual element handler instance</p>
    "!
    "! @parameter ro_virtual_elem_handler | <p class="shorttext synchronized">Instance of virtual element handler</p>
    METHODS get_virtual_elem_handler
      RETURNING
        VALUE(ro_virtual_elem_handler) TYPE REF TO zcl_dbbr_virtual_elem_handler.

    "! <p class="shorttext synchronized">Mark fields that are needed for virtual element calculation</p>
    METHODS mark_virtual_elem_requested
      RAISING
        zcx_dbbr_application_exc.

    "! <p class="shorttext synchronized">Event handler for when association gets chosen</p>
...
CLASS zcl_dbbr_cds_selection_util IMPLEMENTATION.
  ...
  METHOD show_cds_source.
    TRY.
        ...
        zcl_uitb_abap_code_viewer=>show_code( iv_title = |DDL Source { mo_cds_view->get_header( )-entityname_raw }|
                                              iv_code  = lv_source
  ...
  METHOD change_parameters.
    ...
    DATA(lo_param_popup) = NEW zcl_dbbr_cds_param_popup(
        ...
        it_param_values  = VALUE #( FOR param IN mt_param_values WHERE ( is_parameter = abap_true )
                                    ( name = param-fieldname value = param-low ) ) ).
    lo_param_popup->show( ).
    ...
    IF lt_new_param_values IS NOT INITIAL.
      LOOP AT lt_new_param_values ASSIGNING FIELD-SYMBOL(<ls_new_param>).
        ASSIGN mt_param_values[ fieldname    = <ls_new_param>-name
                                is_parameter = abap_true ] TO FIELD-SYMBOL(<ls_old_param>).
        IF     sy-subrc            = 0
           AND <ls_old_param>-low <> <ls_new_param>-value.
  ...
  METHOD choose_association.
    ...
    lr_association_chooser = NEW #( ir_cds_view          = mo_cds_view
  ...
  METHOD create_from_clause.
    ...
    ELSEIF     ms_technical_info-use_ddl_view_for_select  = abap_true
           AND ls_header-source_type <> zif_sat_c_cds_view_type=>table_function.
  ...
  METHOD fill_header.
    cr_dd_doc->add_table( EXPORTING no_of_columns = 3
                                    border        = '0'
                                    width         = '60%'
    ...
    " ...add Table content
    lo_table->span_columns( col_start_span = lo_col1
                            no_of_cols     = 2 ).
    lo_table->new_row( ).
    lo_col1->add_text( text         = 'CDS View'
    ...
    IF mo_cds_view->has_associations( ).
      ...
      lo_table->span_columns( col_start_span = lo_col1
                              no_of_cols     = 2 ).
      lo_col1->add_text( text         = 'Associations'
                         sap_emphasis = cl_dd_document=>strong ).
      lo_table->new_row( ).
  ...
  METHOD handle_alv_ctx_menu_request.
    ...
    super->handle_alv_ctx_menu_request( EXPORTING if_selected_cols  = if_selected_cols
    ...
    IF     if_selected_cols = abap_true
       AND if_selected_rows = abap_false.
      RETURN.
    ENDIF.

    IF mo_cds_view->has_associations( ).
      lt_entries = VALUE #( ( fcode = zif_dbbr_c_selection_functions=>navigate_association
  ...
  METHOD ignore_empty_result.
    ...
    IF     mr_s_global_data->called_from_adt = abap_true
  ...
  METHOD navigate.
    ...
    TRY.
      ...
      CATCH zcx_dbbr_association_nav_error INTO DATA(lx_assoc_nav).
        lx_assoc_nav->show_message( iv_message_type = 'I'
                                    iv_display_type = 'E' ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD zif_dbbr_screen_util~handle_pbo.
    IF if_first_call = abap_true.
      IF     ms_technical_info-assoc_sel_mode           = zif_dbbr_c_assoc_select_mode=>docked
         AND ms_technical_info-show_assoc_brws_at_start = abap_true
*... don't show association browser if no association exist
         AND mo_cds_view->has_associations( ).
        choose_association( ).
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD after_selection.
    IF mf_handle_virtual_elem = abap_true.
      zcl_dbbr_screen_helper=>show_progress( iv_text     = |{ 'Calculating Virtual Fields'(001) }|
                                             iv_progress = 50 ).

      TRY.
          get_virtual_elem_handler( )->calculate_elements( EXPORTING iv_entity_name = mo_cds_view->mv_view_name
                                                           CHANGING  ct_data        = mr_t_data ).
  ...
  METHOD before_selection.
    IF     ms_technical_info-calculate_virtual_element = abap_true
       AND mo_tabfields->has_virtual_element_fields( if_consider_output_only = ms_technical_info-use_reduced_memory ).
  ...
  METHOD mark_virtual_elem_requested.
    ...
    TRY.
        get_virtual_elem_handler( )->adjust_requested( iv_entity_name = mo_cds_view->mv_view_name
                                                       it_fields      = lt_fields ).
      CATCH zcx_dbbr_application_exc INTO DATA(lo_exception).
        IF ms_technical_info-ignore_error_virt_elem_calc = abap_false.
          RAISE EXCEPTION lo_exception.
