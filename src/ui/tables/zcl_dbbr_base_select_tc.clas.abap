"! <p class="shorttext synchronized">Base table controller for low/high like input table controls</p>
CLASS zcl_dbbr_base_select_tc DEFINITION
  ...
  PROTECTED SECTION.
    ...
    "! <p class="shorttext synchronized">Check interval validity of selection field</p>
    "!
    "! @parameter iv_save_low  | <p class="shorttext synchronized"></p>
    "! @parameter iv_save_high | <p class="shorttext synchronized"></p>
    METHODS check_interval_validity
      IMPORTING
        iv_save_low  TYPE zdbbr_selfield-low
        iv_save_high TYPE zdbbr_selfield-high.

    "! <p class="shorttext synchronized">Convert selfield low/high to internal format</p>
    "!
    "! @parameter if_no_uppercase_conversion | <p class="shorttext synchronized"></p>
    METHODS conv_selfields_to_internal
      IMPORTING
        if_no_uppercase_conversion TYPE abap_bool.

    "! <p class="shorttext synchronized">Fills the sign in the selection field</p>
    "!
    METHODS fill_selopt_sign.

    "! <p class="shorttext synchronized">Retrieve current selscreen util instance</p>
    "!
    "! @parameter rr_util | <p class="shorttext synchronized"></p>
    METHODS get_util
      RETURNING
        VALUE(rr_util) TYPE REF TO zcl_dbbr_selscreen_util.

    "! <p class="shorttext synchronized">Checks if custom timestamp conversion should be done</p>
    "!
    "! @parameter rf_custom_timst_conv | <p class="shorttext synchronized"></p>
    METHODS is_custom_timest_conv
      RETURNING
        VALUE(rf_custom_timst_conv) TYPE abap_bool.

    "! <p class="shorttext synchronized">Check if uppercase conversion should be prevented</p>
    "!
    "! @parameter rf_no_uppercase_conv | <p class="shorttext synchronized"></p>
    METHODS is_uppercase_conv_disabled
      RETURNING
        VALUE(rf_no_uppercase_conv) TYPE abap_bool.

    "! <p class="shorttext synchronized">Perform pre conversion checks</p>
    "!
    "! @parameter is_selfield          | <p class="shorttext synchronized"></p>
    "! @parameter if_no_uppercase_conv | <p class="shorttext synchronized"></p>
    "! @parameter cv_value             | <p class="shorttext synchronized"></p>
...
CLASS zcl_dbbr_base_select_tc IMPLEMENTATION.
  METHOD is_custom_timest_conv.
    rf_custom_timst_conv = xsdbool(
           (    mr_selfield_line->domname = zif_dbbr_c_global=>c_domain_names-timestamp
             OR mr_selfield_line->domname = zif_dbbr_c_global=>c_domain_names-timestamp_long )
       AND get_util( )->mo_data->mr_s_global_data->settings-disable_date_to_times_conv = abap_false ).
  ENDMETHOD.
  ...
  METHOD check_interval_validity.
    ...
    IF     mr_selfield_line->low   > mr_selfield_line->high
       AND mr_selfield_line->high <> space
       AND (    mr_selfield_line->datatype = 'CHAR'
             OR mr_selfield_line->datatype = 'DATS'
             OR mr_selfield_line->datatype = 'LANG'
             OR mr_selfield_line->datatype = 'CUKY'
             OR mr_selfield_line->datatype = 'CLNT'
             OR mr_selfield_line->datatype = 'NUMC'
             OR mr_selfield_line->datatype = 'TIMS' ).
  ...
  METHOD conv_selfields_to_internal.
    ...
    IF if_no_uppercase_conversion = abap_true.
      RETURN.
    ENDIF.
  ...
  METHOD is_uppercase_conv_disabled.
    IF     mr_selfield_line->convexit = space
       AND (    mr_selfield_line->inttype = cl_abap_typedescr=>typekind_char
             OR mr_selfield_line->inttype = cl_abap_typedescr=>typekind_string )
       AND mr_selfield_line->lowercase = abap_true.
  ...
  METHOD precheck_selfield_value.
    ...
    " .. all other fields
    IF     is_selfield-lowercase <> abap_true
       AND if_no_uppercase_conv   = abap_false.
      TRANSLATE cv_value TO UPPER CASE.
    ENDIF.

    " .. was sy-datum entered?
    IF     (    is_selfield-datatype = 'DATS'
             OR     (    is_selfield-domname = zif_dbbr_c_global=>c_domain_names-timestamp
                      OR is_selfield-domname = zif_dbbr_c_global=>c_domain_names-timestamp_long )
                AND get_util( )->mo_data->mr_s_global_data->settings-disable_date_to_times_conv = abap_false )
       AND (    cv_value = 'SY-DATUM' OR cv_value = 'TODAY' ).
      cv_value = |{ sy-datum DATE = USER }|.
    ENDIF.

    IF     is_selfield-datatype  = 'CHAR'
       AND is_selfield-intlen   >= 12
       AND ( cv_value = 'SY-UNAME' OR cv_value = 'ME' ).
      cv_value = sy-uname.
    ENDIF.
