"! <p class="shorttext synchronized">Table controller for multiple or input</p>
CLASS zcl_dbbr_multi_or_table DEFINITION
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Delete input of current line</p>
    "!
    "! @parameter CS_SELFIELD | <p class="shorttext synchronized"></p>
    METHODS delete_line
      CHANGING
        cs_selfield TYPE zdbbr_selfield.

    METHODS query_for_search_criteria.
    "! <p class="shorttext synchronized">Update the input status of the fields</p>
    "!
...
CLASS zcl_dbbr_multi_or_table IMPLEMENTATION.
  ...
  METHOD delete_line.
    ...
    DELETE mr_multi_or_multi->* WHERE     tabname   = cs_selfield-tabname
                                      AND fieldname = cs_selfield-fieldname.
  ENDMETHOD.
  ...
  METHOD query_for_search_criteria.
    DATA(lo_popup) = zcl_uitb_pgv_factory=>create_single_field_popup( iv_title = 'Search'
                                                                      is_field = VALUE #( tabname   = 'dd03l'
                                                                                          fieldname = 'fieldname' )
      )->show( ).
  ...
  METHOD search.
    ...
    DATA(lv_index) = lr_table_searcher->search( iv_search_value  = |{ mv_search_value }|
...
*&---------------------------------------------------------------------*
    ...
    LOOP AT SCREEN.
      IF    screen-name = 'GS_MULTI_OR-LOW'
         OR screen-name = 'GS_MULTI_OR-HIGH'.
