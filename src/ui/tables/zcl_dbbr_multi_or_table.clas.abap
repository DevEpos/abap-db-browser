...
*&---------------------------------------------------------------------*
*& Description: Validation of entered values in single line
*&---------------------------------------------------------------------*
...
***    IF mr_selfield_line->low <> space.
***      " check if input length is 45 or more
***      zcl_dbbr_input_validator=>check_and_crop_input( CHANGING cv_value = mr_selfield_line->low ).
***
***      " as the screen field is upper case sensitive I have to convert
***      " all other fields
***      IF mr_selfield_line->lowercase <> abap_true.
***        TRANSLATE mr_selfield_line->low TO UPPER CASE. "#EC TRANSLANG
***      ENDIF.
***
***      """ was sy-datum entered?
***      IF mr_selfield_line->datatype = 'DATS' AND mr_selfield_line->low = 'SY-DATUM'.
***        mr_selfield_line->low = |{ sy-datum DATE = USER }|.
***      ENDIF.
***
***    ENDIF.
***    IF mr_selfield_line->high <> space.
***      " check if input length is 45 or more
***      zcl_dbbr_input_validator=>check_and_crop_input( CHANGING cv_value = mr_selfield_line->high ).
***
***      " as the screen field is upper case sensitive I have to convert
***      " all other fields
***      IF mr_selfield_line->lowercase <> abap_true.
***        TRANSLATE mr_selfield_line->high TO UPPER CASE. "#EC TRANSLANG
***      ENDIF.
***
***      """ was sy-datum entered?
***      IF mr_selfield_line->datatype = 'DATS' AND mr_selfield_line->high = 'SY-DATUM'.
***        mr_selfield_line->high = |{ sy-datum DATE = USER }|.
***      ENDIF.
***    ENDIF.
***
...
***      mr_selfield_line->low  = lv_save_low.
***      mr_selfield_line->high = lv_save_high.
***      MESSAGE e650(db).
***      RETURN.
***    ENDIF.
***
