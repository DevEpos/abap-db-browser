...
CLASS zcl_dbbr_jumplist_param_table IMPLEMENTATION.
  METHOD constructor.
    ...
    LOOP AT CAST zdbbr_jumpparam_data_ui_itab( mr_table_data )->* ASSIGNING <ls_jump_param>.
      IF <ls_jump_param>-param_field <> space.
        DATA(lr_param_field) = mr_query_fields->get_field_ref( iv_tabname_alias = <ls_jump_param>-param_table
  ...
  METHOD validate.
    ...
    IF         cv_function_code  = 'ENTER'
       AND     <lt_table>       IS NOT INITIAL
       AND NOT line_exists( <lt_table>[ active = abap_true ] ).
      CLEAR cv_function_code.
