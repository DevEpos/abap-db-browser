CLASS zcl_dbbr_jumplist_table DEFINITION
  PUBLIC
  INHERITING FROM zcl_uitb_generic_table FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS zif_uitb_table~add_line                 REDEFINITION.
    METHODS zif_uitb_table~update_screen_attributes REDEFINITION.
    METHODS zif_uitb_table~pbo                      REDEFINITION.
    METHODS zif_uitb_table~get_current_line_value   REDEFINITION.
    METHODS zif_uitb_table~update_fields            REDEFINITION.
...
*&---------------------------------------------------------------------*
  ...
  METHOD jump_crit_f4.
    ...
    DATA(lr_join_field_f4) = NEW zcl_dbbr_tabfield_tree_f4( iv_screen_title     = 'Value help for query Field'
  ...
  METHOD jump_target_f4.
    ...
    zcl_dbbr_f4_helper=>call_built_in_f4( EXPORTING iv_repid         = zif_dbbr_c_report_id=>jump_list_manager
  ...
  METHOD set_jump_destinations.
    ...
    LOOP AT CAST zdbbr_jumpdest_data_ui_itab( mr_table_data )->* ASSIGNING <ls_jump_field>.
      IF mf_join_is_active = abap_true.
        <ls_jump_field>-jump_field = mr_query_tabfields->get_field( iv_tabname   = <ls_jump_field>-jump_source_table
      ...
      IF <ls_jump_field>-crit_field IS NOT INITIAL.
        IF mf_join_is_active = abap_true.
          <ls_jump_field>-criterion = mr_query_tabfields->get_field( iv_tabname   = <ls_jump_field>-crit_table
  ...
  METHOD validate.
    ...
    LOOP AT <lt_table> ASSIGNING FIELD-SYMBOL(<ls_jump_field>).
      ...
      IF    <ls_jump_field>-jump_field  IS INITIAL
         OR <ls_jump_field>-jump_target IS INITIAL.
      ...
      TRY.
          DATA(ls_jump_field) = mr_query_tabfields->get_field_by_sql_name(
                                    iv_sql_fieldname = <ls_jump_field>-jump_field ).
      ...
      IF <ls_jump_field>-criterion IS NOT INITIAL.
        TRY.
            DATA(ls_crit_field) = mr_query_tabfields->get_field_by_sql_name(
                                      iv_sql_fieldname = <ls_jump_field>-criterion ).
  ...
  METHOD zif_uitb_table~update_screen_attributes.
    ...
    DATA lv_param_details_infot TYPE string.
    DATA lv_param_details_text TYPE string VALUE 'Param.'.
    ...
    IF <ls_jump_field>-jump_field IS NOT INITIAL.
      ...
      zcl_dbbr_icon_handler=>create_icon( EXPORTING iv_icon_name = lv_param_details_icon
                                                    iv_text      = lv_param_details_text
                                                    iv_info      = lv_param_details_infot
                                          IMPORTING ev_push      = mr_ui_param_details_button->* ).
      ...
      IF <ls_jump_field>-has_error = abap_false AND <ls_jump_field>-is_copied = abap_false.
        LOOP AT SCREEN INTO ls_screen.
          IF    ls_screen-name = mc_table_fields-jump_field
             OR ls_screen-name = mc_table_fields-criterion
             OR ls_screen-name = mc_table_fields-criterion_operation
             OR ls_screen-name = mc_table_fields-criterion_value.
            ls_screen-input = 0.
