...
CLASS zcl_dbbr_selscreen_hist_view DEFINITION
  ...
  PRIVATE SECTION.
    ...
    DATA mt_history TYPE STANDARD TABLE OF ty_history WITH EMPTY KEY.
    ...
    "! <p class="shorttext synchronized">Update state of navigation functions in toolbar</p>
    METHODS update_function_state.
    "! <p class="shorttext synchronized">Initialization</p>
    METHODS initialize.
    "! <p class="shorttext synchronized">Fills the history table from the current navigation history</p>
    METHODS fill_history_table.

    "! <p class="shorttext synchronized">Event handler for when history was modified</p>
    METHODS on_history_modified
      FOR EVENT history_modified OF zcl_dbbr_selscreen_history.

    "! <p class="shorttext synchronized">Event handler for history navigation</p>
    ...
    "! <p class="shorttext synchronized">Event handler for alv toolbar command</p>
    ...
    "! <p class="shorttext synchronized">Event Handler for link click on alv cell</p>
...
CLASS zcl_dbbr_selscreen_hist_view IMPLEMENTATION.
  METHOD class_constructor.
    st_yellow = VALUE #( ( color = zcl_uitb_alv_color_mapper=>string_to_struc( zif_uitb_c_alv_colors=>yellow ) ) ).
  ENDMETHOD.
  ...
  METHOD initialize.
    ...
    mr_dock = NEW cl_gui_docking_container( side      = cl_gui_docking_container=>dock_at_bottom
                                            extension = 120 ).

    fill_history_table( ).

    mr_alv = zcl_uitb_alv=>create_alv( ir_data                 = REF #( mt_history )
    ...
    lr_functions->add_function( iv_name             = c_navback_function
                                iv_icon             = |{ icon_arrow_left }|
                                iv_tooltip          = CONV #( 'Previous Object'(001) )
                                if_start_of_toolbar = abap_true ).
    lr_functions->add_function( iv_name             = c_navforward_function
    ...
    lr_functions->add_function( iv_name             = c_clear_function
                                iv_icon             = |{ icon_delete }|
                                iv_tooltip          = |{ 'Clear History'(004) }|
                                if_start_of_toolbar = abap_true ).
    lr_functions->add_function( iv_type = zcl_uitb_alv_functions=>separator ).
    lr_functions->add_function( iv_name             = 'CLOSE'
                                iv_icon             = |{ icon_close }|
    ...
    SET HANDLER on_user_command FOR mr_alv->get_events( ).
  ...
  METHOD on_history_navigation.
    ...
    mr_alv->get_selections( )->set_current_cell( VALUE #( row    = ev_current_index
                                                          column = 'ENTITY_ID' ) ).
  ...
  METHOD update_function_state.
    DATA(lr_functions) = mr_alv->get_functions( ).
    lr_functions->set_function( iv_name   = c_navback_function
                                if_enable = zcl_dbbr_selscreen_history=>has_previous( ) ).
    lr_functions->set_function( iv_name   = c_navforward_function
  ...
  METHOD zif_uitb_disposable~dispose.
    ...
    SET HANDLER on_history_modified ACTIVATION abap_false.
    SET HANDLER on_history_navigation ACTIVATION abap_false.
  ...
  METHOD zif_uitb_view~show.
    ...
    SET HANDLER on_history_modified.
