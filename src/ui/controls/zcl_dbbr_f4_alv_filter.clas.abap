"! <p class="shorttext synchronized">ALV Filter for F4 Value help</p>
CLASS zcl_dbbr_f4_alv_filter DEFINITION
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Creates the ALV for the filtering</p>
    METHODS create_alv.
ENDCLASS.
...
CLASS zcl_dbbr_f4_alv_filter IMPLEMENTATION.
  METHOD constructor.
    ...
    LOOP AT it_selfield ASSIGNING FIELD-SYMBOL(<ls_selfield>).
      mt_filter = VALUE #( ( fieldname  = <ls_selfield>-fieldname
  ...
  METHOD create_alv.
    ...
    mo_alv = zcl_uitb_alv=>create_alv( ir_data      = REF #( mt_filter )
