...
CLASS zcl_dbbr_object_navigator DEFINITION
  ...
  PUBLIC SECTION.
    ...
    ALIASES free        FOR zif_uitb_view~free.
    ALIASES has_focus   FOR zif_uitb_gui_control~has_focus.
    ALIASES hide        FOR zif_uitb_view~hide.
    ALIASES is_visible  FOR zif_uitb_view~is_visible.
    ALIASES search      FOR zif_uitb_content_searcher~search.
    ALIASES search_next FOR zif_uitb_content_searcher~search_next.
    ...
    "! <p class="shorttext synchronized">Updates the content view of the navigator</p>
    METHODS update_view
      IMPORTING
        iv_function TYPE ui_func.

  PRIVATE SECTION.
    ALIASES mf_visible FOR zif_uitb_view~mf_visible.
    ...
    "! <p class="shorttext synchronized">Retrieve the currently active view in the navigator</p>
    METHODS get_active_control
      RETURNING
        VALUE(rr_control) TYPE REF TO zif_uitb_gui_control.

    "! <p class="shorttext synchronized">Handler for Toolbar Button event</p>
    METHODS on_toolbar_button
      FOR EVENT function_selected OF cl_gui_toolbar
      IMPORTING
        fcode.

    "! <p class="shorttext synchronized">Handler for goto next object nav. view</p>
    "!
    METHODS on_goto_next_objnav_view
      FOR EVENT goto_next_view_in_objnav OF zcl_dbbr_selscr_nav_events.

    "! <p class="shorttext synchronized">Show Favorites view in navigator</p>
    METHODS show_favorites
      IMPORTING
        ir_container TYPE REF TO cl_gui_container.

    "! <p class="shorttext synchronized">Show history view</p>
    "!
    "! @parameter ir_container | <p class="shorttext synchronized"></p>
    METHODS show_history
      IMPORTING
        ir_container TYPE REF TO cl_gui_container.

    "! <p class="shorttext synchronized">Show Repository Browser (DB,CDS View) in navigator</p>
...
CLASS zcl_dbbr_object_navigator IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    mt_view_info = VALUE #( ( cell_id  = c_first_view_id
                              function = c_view-history
                              caption  = VALUE #( text = 'History'(003)
                                                  icon = zif_dbbr_c_icon=>history ) )
                            ( cell_id  = 3
                              function = c_view-favorites
                              caption  = VALUE #( text = 'Favorites'(001)
                                                  icon = icon_system_favorites ) )
                            ( cell_id  = 4
                              function = c_view-object_browser
                              caption  = VALUE #( text = 'Object Browser'(002)
                                                  icon = icon_tree ) ) ).
  ...
  METHOD create_dock.
    ...
    DATA(lr_global_data) = CAST zdbbr_global_data(
                              zcl_uitb_data_cache=>get_instance( zif_dbbr_c_report_id=>main
                              )->get_data_ref( zif_dbbr_main_report_var_ids=>c_s_data ) ).

    mo_dock = NEW cl_gui_docking_container(
                      side      = COND #( WHEN lr_global_data->tech_settings-dock_obj_nav_on_right = abap_false
                                          THEN cl_gui_docking_container=>dock_at_left
  ...
  METHOD create_splitter.
    ...
    mo_splitter = NEW cl_gui_splitter_container( parent  = mo_dock
                                                 rows    = lv_max_views + 1
                                                 columns = 1 ).

    mo_splitter->set_row_mode( cl_gui_splitter_container=>mode_absolute ).
    mo_splitter->set_border( EXPORTING  border = abap_false
                             EXCEPTIONS OTHERS = 0 ).

    DO lv_max_views + 1 TIMES.
      mo_splitter->set_row_sash( id    = sy-index
      ...
      IF sy-index > 1.
        mo_splitter->set_row_height( id     = sy-index
                                     height = 0 ).
      ENDIF.
    ENDDO.

    " .. Set height of toolbar panel
    mo_splitter->set_row_height( id     = 1
  ...
  METHOD create_toolbar.
    ...
    DATA(lo_container) = mo_splitter->get_container( row    = 1
                                                     column = 1 ).
    mo_mode_toolbar = NEW #( parent       = lo_container
    ...
    SET HANDLER on_toolbar_button FOR mo_mode_toolbar.

    LOOP AT mt_view_info ASSIGNING FIELD-SYMBOL(<ls_view_info>).
      mo_mode_toolbar->add_button( fcode      = <ls_view_info>-function
  ...
  METHOD show_favorites.
    ...
    mo_favorites_tree = NEW #( io_parent      = ir_container
                               io_parent_view = me ).
    TRY.
        mo_favorites_tree->show(
            if_global = xsdbool( lo_s_global_data->fav_user_mode = zif_dbbr_c_global=>c_fav_user_modes-global ) ).
      CATCH zcx_uitb_tree_error INTO DATA(lx_tree_error). " TODO: variable is assigned but never used (ABAP cleaner)
  ...
  METHOD show_history.
    ...
    mo_history = NEW zcl_dbbr_object_history_tree( io_parent_container = ir_container
                                                   io_parent_view      = me ).
  ENDMETHOD.
  ...
  METHOD show_repo_browser.
    ...
    mo_object_browser = NEW zcl_dbbr_object_browser_tree( io_parent      = ir_container
                                                          io_parent_view = me ).
  ENDMETHOD.
  ...
  METHOD update_view.
    ...
    DATA(lo_container) = mo_splitter->get_container( row    = lr_new_view->cell_id
                                                     column = 1 ).
    ...
    adapt_view_and_toolbar( iv_active_view_id = lv_active_view_id
  ...
  METHOD zif_uitb_view~show.
    IF mo_splitter IS INITIAL.
      ...
      DATA(lr_global_data) = CAST zdbbr_global_data( zcl_uitb_data_cache=>get_instance( zif_dbbr_c_report_id=>main )->get_data_ref(
                                                         zif_dbbr_main_report_var_ids=>c_s_data ) ).
  ...
  METHOD zif_uitb_gui_composite_view~execute_command.
    ...
    CASE io_command->mv_function.
      ...
      WHEN c_command_id-show_object_list.
        IF io_command->mr_params IS NOT BOUND.
          RETURN.
        ENDIF.
        ...
        update_view( c_view-object_browser ).
        zcl_dbbr_selscr_nav_events=>raise_display_object_list( iv_entity_id   = lr_entity_info->entity_id
