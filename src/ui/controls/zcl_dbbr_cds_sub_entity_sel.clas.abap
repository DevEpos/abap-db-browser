CLASS zcl_dbbr_cds_sub_entity_sel DEFINITION
  ...
  PRIVATE SECTION.
    ...
    DATA mt_associations TYPE STANDARD TABLE OF ty_association WITH EMPTY KEY.
...
CLASS zcl_dbbr_cds_sub_entity_sel IMPLEMENTATION.
  METHOD change_alv_fields.
    ...
    IF mf_all = abap_true.
      lr_cols->set_column_position( iv_columnname = 'CARDINALITY_TEXT'
                                    iv_position   = 1 ).
      lr_cols->set_column_position( iv_columnname = 'RAW_NAME'
                                    iv_position   = 2 ).
      lr_cols->set_column_position( iv_columnname = 'KIND_DESCRIPTION'
                                    iv_position   = 3 ).
      lr_cols->set_column_position( iv_columnname = 'REF_CDS_VIEW_RAW'
                                    iv_position   = 4 ).
      lr_cols->set_column_position( iv_columnname = 'DDTEXT'
                                    iv_position   = 5 ).
    ENDIF.
    lr_cols->set_optimized( ).

    mr_assoc_alv->get_functions( )->set_function( iv_name    = 'ALLFIELDS'
                                                  if_checked = mf_all ).
  ENDMETHOD.
  ...
  METHOD choose_association.
    ...
    on_link_click( ev_row    = ls_cell-row
                   ev_column = space ).
  ENDMETHOD.
  ...
  METHOD constructor.
    ...
    IF mf_as_dock = abap_false.
      ...
      SET HANDLER on_before_output FOR mr_tmplt_prog.
      SET HANDLER on_user_command FOR mr_tmplt_prog.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD create_association_alv.
    ...
    TRY.
        mr_assoc_alv = zcl_uitb_alv=>create_alv( ir_data                 = REF #( mt_associations )
                                                 iv_description_language = 'E'
                                                 ir_container            = get_container( )
                                                 if_editable             = abap_false ).

        SET HANDLER on_link_click FOR mr_assoc_alv->get_events( ).
        ...
        lr_cols->set_column_position( iv_columnname = 'CARDINALITY_TEXT'
                                      iv_position   = 1 ).
        lr_col->set_descriptions( iv_short = 'Card.'
                                  iv_long  = 'Cardinality' ).
        ...
        IF mf_as_dock = abap_true.
          mr_assoc_alv->get_functions( )->add_function( iv_name    = 'ALLFIELDS'
                                                        iv_text    = 'All Fields'
                                                        iv_tooltip = 'Toggle All Fields' ).
          mr_assoc_alv->get_functions( )->add_function( iv_name    = 'CLOSE'
                                                        iv_icon    = |{ icon_close }|
  ...
  METHOD create_tree.
    ...
    mr_cds_subentity_tree = NEW #( ir_parent           = get_container( )
                                   is_hierarchy_header = VALUE treemhhdr( heading = 'Entity Name'(003)
                                                                          width   = 60 ) ).

    SET HANDLER on_node_double_click FOR mr_cds_subentity_tree->get_events( ).

    " TODO: variable is assigned but never used (ABAP cleaner)
    DATA(lr_toolbar) = mr_cds_subentity_tree->get_toolbar( ).

    TRY.
        mr_cds_subentity_tree->get_columns( )->add_column( iv_colname     = c_type_col
                                                           iv_width       = 40
                                                           iv_header_text = 'Type'(004) ).
        mr_cds_subentity_tree->get_columns( )->add_column( iv_colname     = c_description_col
                                                           iv_width       = 60
  ...
  METHOD fill_association_alv.
    ...
    mr_assoc_alv->get_selections( )->set_current_cell( VALUE #( row    = 1
                                                                column = 'RAW_NAME' ) ).
  ...
  METHOD fill_status_functions.
    ...
    mr_tmplt_prog->add_function( iv_function_id = zif_uitb_template_prog=>c_func_f2
  ...
  METHOD fill_tree.
    ...
    LOOP AT lt_base_tables ASSIGNING FIELD-SYMBOL(<ls_base_table>).
      ...
      lr_nodes->add_node( iv_node_key          = CONV #( <ls_base_table>-entityname )
    ...
    LOOP AT lt_associations ASSIGNING FIELD-SYMBOL(<ls_assoc>).
      lv_icon = SWITCH #(
        ...
        WHEN zif_sat_c_entity_type=>cds_view THEN zif_dbbr_c_icon=>cds_view
        WHEN zif_sat_c_entity_type=>table    THEN zif_dbbr_c_icon=>database_table ).
      lr_nodes->add_node( iv_node_key          = CONV #( <ls_assoc>-name )
  ...
  METHOD get_container.
    result = COND #(
      WHEN mf_as_dock = abap_true
      THEN mr_dock
      ELSE mr_tmplt_prog->get_container( ) ).
  ENDMETHOD.
