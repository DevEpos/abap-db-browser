...
*"----------------------------------------------------------------------
  ...
  IF     gs_data-settings-last_entity_id   IS NOT INITIAL
     AND gs_data-settings-last_entity_type  = zif_sat_c_entity_type=>query.
  ...
  IF ls_context-parameters IS NOT INITIAL.
    ...
    IF     lt_params          IS NOT INITIAL
       AND lines( lt_params )  > 1
       AND line_exists( lt_params[ param_id = zif_sat_c_adt_start_params=>adt_call ] ).

      DATA(lv_skip_selscreen_value) = VALUE #( lt_params[
                                                   param_id = zif_sat_c_adt_start_params=>skip_selscreen ]-param_value DEFAULT abap_false ).
      TRY.
