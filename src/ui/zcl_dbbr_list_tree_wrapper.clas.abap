...
CLASS zcl_dbbr_list_tree_wrapper IMPLEMENTATION.
  METHOD add_node.
    ...
    mr_list_tree_model->add_node( node_key          = ir_node->get_node_key( )
...
*                                  drag_drop_id      = " "
    ...
    IF if_recursive = abap_true.
      ...
      LOOP AT ir_node->get_children( ) ASSIGNING FIELD-SYMBOL(<lr_child_node>).
        add_node( ir_node              = <lr_child_node>
  ...
  METHOD get_node_internal.
    ...
    mr_list_tree_model->node_get_user_object( EXPORTING node_key    = iv_node_key
    ...
    rr_node = NEW zcl_dbbr_tree_node_wrapper( iv_node_key        = iv_node_key
                                              it_items           = lt_items
  ...
  METHOD get_selected_node.
    ...
    ELSE.
      ...
      IF lv_node IS NOT INITIAL.
        rr_node = get_node( iv_node_key  = lv_node
                            if_recursive = if_recursive ).
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD get_selected_nodes.
    ...
    IF lt_nodes IS NOT INITIAL.
      rt_nodes = VALUE #( FOR <lv_node_key> IN lt_nodes
