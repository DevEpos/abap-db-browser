"! <p class="shorttext synchronized">Class for calling dialogs</p>
CLASS zcl_dbbr_dialogs DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Show user settings dialog for DB Browser</p>
    ...
    "! <p class="shorttext synchronized">Shows the multi input selection screen for a table field</p>
...
CLASS zcl_dbbr_dialogs IMPLEMENTATION.
  METHOD show_multi_select.
    CHECK    cs_selfield-is_parameter = abap_false
          OR (     cs_selfield-is_parameter   = abap_true
               AND cs_selfield-is_range_param = abap_true ).

    DATA(lo_multi_select_table) = NEW zcl_dbbr_multi_select_table( ).
    lo_multi_select_table->init_table( is_template = cs_selfield
                                       it_multi    = ct_selfield_multi ).
    " create the controller
    DATA(lo_multi_select_controller) = NEW zcl_dbbr_multi_select_ctlr( io_custom_f4_map   = io_custom_f4_map
    ...
    IF ef_changed = abap_false.
      RETURN.
    ENDIF.

    lo_multi_select_controller->transfer_data( CHANGING cs_selfield       = cs_selfield
