...
CLASS zcl_dbbr_built_in_f4_sc IMPLEMENTATION.
  METHOD constructor.
    ...
    mt_function_exclude = VALUE #( BASE mt_function_exclude
                                   ( c_test_f4_func ) ).

    DATA(lr_data_cache) = zcl_uitb_data_cache=>get_instance( zif_dbbr_c_report_id=>main ).
    mr_ui_built_in_f4 = CAST zdbbr_build_in_f4_ui_data( lr_data_cache->get_data_ref(
                                                            zif_dbbr_main_report_var_ids=>c_s_built_in_f4 ) ).
...
*&---------------------------------------------------------------------*
    ...
    CALL FUNCTION 'F4IF_DETERMINE_SEARCHHELP'
      EXPORTING  tabname           = mr_ui_built_in_f4->search_table
  ...
  METHOD save_search_help.
    ...
    IF NOT built_in_search_help_exists( ).
      zcx_sat_validation_exception=>raise_with_text(
          |Theres is no Value Help for the Field { mr_ui_built_in_f4->search_field }| &&
    ...
    IF mf_delete_existing = abap_true.
      zcl_dbbr_custom_f4_factory=>delete_f4_assignments( iv_tabname   = mr_ui_built_in_f4->tabname
                                                         iv_fieldname = mr_ui_built_in_f4->fieldname ).
    ENDIF.

    IF zcl_dbbr_custom_f4_factory=>exists_built_in_f4( EXPORTING iv_tabname   = mr_ui_built_in_f4->search_table
    ...
    ELSE.
      ...
      DATA(ls_built_in_type_info) = VALUE zdbbr_built_in_data_type( datatype = ls_search_table_field-datatype
                                                                    inttype  = ls_search_table_field-inttype
                                                                    leng     = ls_search_table_field-outputlen ).
      ms_f4_def = VALUE zdbbr_f4_data( description              = mr_ui_built_in_f4->description
                                       created_by               = sy-uname
...
*&---------------------------------------------------------------------*
    ...
    IF mf_tabfield_provided = abap_false.
      RETURN.
    ENDIF.

    " --- check if there is already a search help for this table/fieldname
    IF NOT zcl_dbbr_custom_f4_factory=>exists_assignment_for_tabfield( iv_tabname   = mr_ui_built_in_f4->tabname
                                                                       iv_fieldname = mr_ui_built_in_f4->fieldname ).
      RETURN.
    ENDIF.
  ...
  METHOD zif_uitb_screen_controller~call_screen.
    zcl_uitb_screen_util=>call_screen( iv_screen_id    = zif_dbbr_screen_ids=>c_define_built_in_f4
