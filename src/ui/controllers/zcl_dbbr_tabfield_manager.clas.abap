CLASS zcl_dbbr_tabfield_manager DEFINITION
  ...
  PRIVATE SECTION.
    ...
    TYPES: BEGIN OF ty_table_overview.
    ...
    DATA mt_tableoverview TYPE STANDARD TABLE OF ty_table_overview.
...
CLASS zcl_dbbr_tabfield_manager IMPLEMENTATION.
  METHOD constructor.
    ...
    mr_filter_fields_tab = CAST zdbbr_button( lr_data_cache->get_data_ref(
                                                  zif_dbbr_main_report_var_ids=>c_bt_filter_fields ) ).
    ...
    IF     mv_mode             = zif_dbbr_c_global=>c_field_chooser_modes-selection
       AND is_join_def-tables IS NOT INITIAL.
      ...
      " ... create table overview list before extraction
    ...
    mo_output_field_tree = COND #(
       WHEN mv_mode = zif_dbbr_c_global=>c_field_chooser_modes-selection
       THEN NEW zcl_dbbr_field_output_tree( io_fields            = mo_fields
       ...
       ELSE NEW zcl_dbbr_fld_outp_tree_out( io_fields            = mo_fields
                                            io_original_fields   = mo_original_fields
                                            if_field_aggregation = if_field_aggregation
                                            if_single_table_mode = mf_single_table_mode
                                            iv_mode              = mv_mode
    ...
    mo_select_field_tree = NEW #( io_fields            = mo_fields
                                  iv_mode              = mv_mode
                                  if_single_table_mode = mf_single_table_mode
                                  iv_current_table     = mv_current_table
                                  iv_entity_type       = mv_entity_type ).
    ...
    SET HANDLER on_output_tree_updated FOR mo_output_field_tree.
    SET HANDLER on_select_tree_updated FOR mo_select_field_tree.
  ENDMETHOD.
  ...
  METHOD create_alv_grid.
    ...
    " set event handlers
    SET HANDLER on_table_hotspot_click FOR mr_dd_alv_list.
    SET HANDLER on_table_data_changed_finished FOR mr_dd_alv_list.
  ...
  METHOD create_table_overview.
    ...
    mr_overview_dock = NEW cl_gui_docking_container( side  = cl_gui_docking_container=>dock_at_top
                                                     ratio = 25 ).
  ...
  METHOD display_alv_grid.
    ...
    mr_dd_alv_list->set_table_for_first_display( EXPORTING is_layout       = ls_layout
  ...
  METHOD on_table_data_changed_finished.
    ...
    IF lr_s_entity_info->tabname_alias = mv_current_table.
      ...
      mr_dd_alv_list->refresh_table_display( is_stable = VALUE #( row = abap_true
                                                                  col = abap_true ) ).
    ELSE.
  ...
  METHOD on_table_hotspot_click.
    " get current line
    IF NOT line_exists( mt_tableoverview[ e_row_id-index ] ).
      RETURN.
    ENDIF.
...
*        ENDIF.
    ...
    DATA(lv_mode_text) = COND string( WHEN mv_mode = zif_dbbr_c_global=>c_field_chooser_modes-output
                                      THEN 'Output'
                                      ELSE 'Selection' ).
