"! <p class="shorttext synchronized">Dependency Tree for CDS View</p>
CLASS zcl_dbbr_cds_dependency_tree DEFINITION
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Add new node to the tree</p>
    METHODS add_node
      IMPORTING
        is_node        TYPE cl_ddls_dependency_visitor=>ty_s_dependency_graph_node
        iv_parent_node TYPE tm_nodekey OPTIONAL.

    "! <p class="shorttext synchronized">Create tree model</p>
    METHODS create_tree
      IMPORTING
        io_container TYPE REF TO cl_gui_container.

    "! <p class="shorttext synchronized">Fetches CDS dependencies</p>
    METHODS fetch_dependencies
      RETURNING
        VALUE(rf_dependencies_read) TYPE abap_bool.

    "! <p class="shorttext synchronized">Retrieve Text for constant</p>
    ...
    "! <p class="shorttext synchronized">Get text for object type</p>
    ...
    "! <p class="shorttext synchronized">Get icon for node type</p>
    ...
    "! <p class="shorttext synchronized">Trigger Show CDS Source Code</p>
    METHODS do_show_cds_source
      IMPORTING
        ir_params TYPE REF TO data OPTIONAL.

    "! <p class="shorttext synchronized">Show CDS Source Code</p>
    METHODS show_cds_source
      IMPORTING
        iv_cds_name TYPE zsat_entity_id.

    "! <p class="shorttext synchronized">Trigger Show Content of DB Entity</p>
    METHODS do_show_content
      IMPORTING
        if_new_task TYPE abap_bool   OPTIONAL
        ir_params   TYPE REF TO data OPTIONAL.

    "! <p class="shorttext synchronized">Show Content of DB Entity</p>
    ...
    "! <p class="shorttext synchronized">Creates code viewer control</p>
    ...
    "! <p class="shorttext synchronized">Creates context menu for options</p>
    ...
    "! <p class="shorttext synchronized">Switch between tree and alv mode</p>
    METHODS toggle_usage_alv.
    METHODS toggle_dependency_tree.

    "! <p class="shorttext synchronized">Trigger ADT jump</p>
    METHODS do_open_with_adt
      IMPORTING
        ir_params TYPE REF TO data.

    "! <p class="shorttext synchronized">Jump to ADT with selected entity</p>
    METHODS open_with_adt
      IMPORTING
        iv_entity_id   TYPE zsat_entity_id
        iv_entity_type TYPE zsat_entity_type.

    "! <p class="shorttext synchronized">Open DB Browser in new window for entity</p>
    METHODS open_db_browser_new_window
      IMPORTING
        iv_entity_id   TYPE zsat_entity_id
        iv_entity_type TYPE zsat_entity_type.

    "! <p class="shorttext synchronized">Trigger Open DB Browser in new window for entity</p>
...
CLASS zcl_dbbr_cds_dependency_tree IMPLEMENTATION.
  ...
  METHOD add_node.
    ...
    IF    is_node-type = zcl_sat_cds_dep_analyzer=>c_node_type-cds_view
       OR is_node-type = zcl_sat_cds_dep_analyzer=>c_node_type-cds_table_function
       OR is_node-type = zcl_sat_cds_dep_analyzer=>c_node_type-cds_db_view.
    ...
    IF is_node-children IS BOUND.
      LOOP AT CAST cl_ddls_dependency_visitor=>ty_t_dependency_graph_nodes( is_node-children )->* ASSIGNING FIELD-SYMBOL(<ls_child_node>).
        add_node( is_node        = <ls_child_node>
  ...
  METHOD do_before_dynpro_output.
    ...
    DATA(lt_fkey_map) = VALUE zif_uitb_ty_gui_screen=>ty_t_fkey_map(
                                  ( fkey            = zif_uitb_c_gui_screen=>c_functions-ctrl_f1
                                    ...
                                    text            = |{ 'Set Focus to Tree' }| )
                                  ( fkey            = zif_uitb_c_gui_screen=>c_functions-ctrl_f4
                                    mapped_function = c_functions-expand_all
                                    text            = |{ 'Expand all nodes' }| )
                                  ( fkey            = zif_uitb_c_gui_screen=>c_functions-ctrl_shift_f4
                                    mapped_function = c_functions-collapse_all
                                    text            = |{ 'Collapse all nodes' }| )
                                  ( fkey            = zif_uitb_c_gui_screen=>c_functions-f5
                                    mapped_function = c_functions-exec_with_dbbrs
                                    text            = |{ 'Show DB Content'(046) }| )
                                  ( fkey            = zif_uitb_c_gui_screen=>c_functions-f6
                                    mapped_function = c_functions-exec_with_dbbrs_new_window
                                    text            = |{ 'Show DB Content in new Task'(048) }| )
                                  ( fkey            = zif_uitb_c_gui_screen=>c_functions-f7
                                    mapped_function = c_functions-show_ddl_source
                                    text            = |{ 'Show CDS Source Code'(044) }| )
                                  ( fkey            = zif_uitb_c_gui_screen=>c_functions-f9
                                    mapped_function = c_functions-show_metrics
                                    text            = |{ 'Show Complexity Metrics'(032) }| ) ).

    IF     mo_splitter IS BOUND
       AND mo_splitter->is_element_visible( 2 ).

      lt_fkey_map = VALUE #( BASE lt_fkey_map
                             ( fkey            = zif_uitb_c_gui_screen=>c_functions-ctrl_f5
                               mapped_function = c_functions-maximize_code_view
                               text            = |{ 'Maximize Code Viewer'(050) }| )
                             ( fkey            = zif_uitb_c_gui_screen=>c_functions-ctrl_shift_f5
                               mapped_function = c_functions-close_code_view
                               text            = |{ 'Close Code Viewer'(051) }| ) ).
    ENDIF.
  ...
  METHOD create_code_viewer.
    create_control_toolbar( EXPORTING io_parent  = io_container
                                      it_button  = VALUE #( ( function  = c_functions-close_code_view
                                                              icon      = icon_close
    ...
    ENDIF.
    mo_code_view = NEW zcl_uitb_abap_code_view( io_parent_container = lo_container
                                                iv_theme            = mv_theme ).
  ENDMETHOD.

  METHOD create_tree.
    " .. Create switch for to toggle between tree and ALV on the left side
    mo_switch = NEW zcl_uitb_gui_switch_container( io_parent = io_container ).

    create_control_toolbar( EXPORTING io_parent  = mo_switch->add_child( iv_id = 'TREE' )
                                      it_button  = VALUE #( ( function  = c_functions-expand_all
                                                            ...
                                                            ( function  = c_functions-options_menu
                                                              ...
                                                              icon      = icon_settings ) )
                            IMPORTING eo_toolbar = DATA(mo_toolbar)
                                      eo_client  = DATA(lo_left_container) ).

    modify_options_ctx_menu( ).

    mo_toolbar->set_static_ctxmenu( EXPORTING  fcode   = c_functions-options_menu
                                               ctxmenu = mo_options_menu
                                    EXCEPTIONS OTHERS  = 1 ).

    mo_tree = NEW zcl_uitb_column_tree_model( ir_parent           = lo_left_container
                                              is_hierarchy_header = VALUE #( heading = 'Entity Name'(035)
                                                                             width   = 110 )
                                              if_auto_node_key    = abap_true
                                              iv_selection_mode   = cl_tree_model=>node_sel_mode_multiple ).

    SET HANDLER on_node_context_menu_request FOR mo_tree->get_events( ).
    SET HANDLER on_node_context_menu_select FOR mo_tree->get_events( ).
    SET HANDLER on_node_double_click FOR mo_tree->get_events( ).

    " .. Create further columns
    DATA(lo_columns) = mo_tree->get_columns( ).
    lo_columns->add_hierarchy_column( c_hierarchy_node2 ).
    lo_columns->add_column( iv_colname     = c_relation_col
                            iv_width       = 20
                            iv_header_text = |{ 'SQL Relation'(034) }| ).
    lo_columns->add_column( iv_colname     = c_sql_name_col
                            iv_width       = 40
                            iv_header_text = |{ 'SQL Name'(001) }| ).
    lo_columns->add_column( iv_colname     = c_object_type_col
                            iv_width       = 30
                            iv_header_text = |{ 'Object Type'(036) }| ).

    zcl_uitb_screen_util=>show_progress( iv_progress = 50
                                         iv_text     = |{ 'Building Dependency Tree...'(003) }| ).
    add_node( is_node = ms_dependencies ).
  ...
  METHOD adjust_dbl_click_menu.
    ...
    ro_menu->add_function( fcode   = |{ c_dbl_click_action-function_prefix }{ c_dbl_click_action-open_in_db_browser }|
                           text    = |{ 'Open with DB Browser' }|
                           checked = xsdbool( mv_dbl_click_action = c_dbl_click_action-open_in_db_browser ) ).
    ro_menu->add_function( fcode   = |{ c_dbl_click_action-function_prefix }{ c_dbl_click_action-adt_navigation }|
                           text    = |{ TEXT-042 }|
    ...
    ro_menu->add_function( fcode   = |{ c_dbl_click_action-function_prefix }{ c_dbl_click_action-exec_with_dbbrs }|
                           text    = |{ TEXT-040 }|
  ...
  METHOD modify_options_ctx_menu.
    ...
    mo_options_menu->add_submenu( menu = lo_double_click_setting_menu
  ...
  METHOD fetch_dependencies.
    ...
    zcl_uitb_screen_util=>show_progress( iv_progress = 1
                                         iv_text     = |{ 'Analyzing CDS Dependencies...'(004) }| ).
  ...
  METHOD fill_entity_texts.
    zcl_uitb_screen_util=>show_progress( iv_progress = 75
                                         iv_text     = |{ 'Loading Descriptions...'(037) }| ).

    SELECT entity, description FROM zsat_i_databaseentity
  ...
  METHOD load_cds_source_into_view.
    mo_code_view->show_document( iv_code = iv_code ).
  ...
  METHOD on_node_context_menu_select.
    ...
    CASE ev_fcode.
      ...
      WHEN c_functions-open_with_adt.
        open_with_adt( iv_entity_id   = <ls_node_map>-entity_id
      ...
      WHEN c_functions-exec_with_dbbrs.
        show_content( if_new_task    = abap_false
                      iv_entity_id   = <ls_node_map>-entity_id
                      iv_entity_type = <ls_node_map>-entity_type ).
  ...
  METHOD on_node_double_click.
    ...
    CASE mv_dbl_click_action.
      ...
      WHEN c_dbl_click_action-open_in_db_browser.
        ...
        zcl_dbbr_selscr_nav_events=>raise_entity_chosen( iv_entity_id   = lr_node_map->entity_id
      ...
      WHEN c_dbl_click_action-adt_navigation.
        open_with_adt( iv_entity_id   = lr_node_map->entity_id
                       iv_entity_type = lr_node_map->entity_type ).
    ENDCASE.

    IF lv_fcode IS NOT INITIAL.
      on_node_context_menu_select( ev_fcode    = lv_fcode
  ...
  METHOD do_show_cds_source.
    ...
    ELSE.
      ...
      IF sy-subrc <> 0.
        RETURN.
      ENDIF.
      on_node_context_menu_select( ev_fcode    = c_functions-show_ddl_source
  ...
  METHOD show_cds_source.
    TRY.
        ...
        load_cds_source_into_view( iv_code = lv_source ).
  ...
  METHOD show_complexity_metrics.
    ...
    zcl_uitb_popup_help_viewer=>create( iv_title        = |{ 'Complexity Metrics for'(018) } { mv_view_name }|
                                        ...
                                        ir_html_content = NEW zcl_uitb_html_content(
                                          )->add_heading( iv_text = |{ 'Used Data Sources'(019) }|
                                          )->start_table(
                                          )->start_table_row(
                                          )->add_table_body_cell( iv_text = |{ 'Database tables:'(020) }|
                                          )->add_table_body_cell( iv_text = |{ <ls_metrics>-numtabs NUMBER = USER }|
                                          )->end_table_row( )->start_table_row(
                                          )->add_table_body_cell( iv_text = |{ 'Database views:'(021) }|
                                          )->add_table_body_cell( iv_text = |{ <ls_metrics>-numviews NUMBER = USER }|
                                          )->end_table_row( )->start_table_row(
                                          )->add_table_body_cell( iv_text = |{ 'CDS views with parameters'(022) }|
                                          )->add_table_body_cell(
                                              iv_text = |{ <ls_metrics>-numviewswithparamter NUMBER = USER }|
                                          )->end_table_row( )->start_table_row(
                                          )->add_table_body_cell( iv_text = |{ 'CDS table functions'(023) }|
                                          )->add_table_body_cell(
                                              iv_text = |{ <ls_metrics>-numtablefunctions NUMBER = USER }|
                                          )->end_table_row( )->end_table(
                                          )->add_heading( iv_text = |{ 'SQL Operations'(024) }|
                                          )->start_table( )->start_table_row(
                                          )->add_table_body_cell( iv_text = |{ 'JOIN Operations:'(025) }|
                                          )->add_table_body_cell( iv_text = |{ <ls_metrics>-numjoins NUMBER = USER }|
                                          )->end_table_row( )->start_table_row(
                                          )->add_table_body_cell( iv_text = |{ 'UNION Operations:'(026) }|
                                          )->add_table_body_cell( iv_text = |{ <ls_metrics>-numunions NUMBER = USER }|
                                          )->end_table_row( )->start_table_row(
                                          )->add_table_body_cell( iv_text = |{ 'GROUP BY clauses'(027) }|
                                          )->add_table_body_cell( iv_text = |{ <ls_metrics>-numgroupby NUMBER = USER }|
                                          )->end_table_row( )->end_table(
                                          )->add_heading(
                                              iv_text = |{ 'Performance Related Function Calls and Operations'(028) }|
                                          )->start_table( )->start_table_row(
                                          )->add_table_body_cell( iv_text = |{ 'Function calls:'(029) }|
                                          )->add_table_body_cell(
                                              iv_text = |{ <ls_metrics>-numfunctioncalls NUMBER = USER }|
                                          )->end_table_row( )->start_table_row(
                                          )->add_table_body_cell( iv_text = |{ 'Explicit type CAST operations'(030) }|
                                          )->add_table_body_cell(
                                              iv_text = |{ <ls_metrics>-numexplicitcasts NUMBER = USER }|
                                          )->end_table_row( )->start_table_row(
                                          )->add_table_body_cell( iv_text = |{ 'CASE expressions:'(031) }|
                                          )->add_table_body_cell( iv_text = |{ <ls_metrics>-numcase NUMBER = USER }|
                                          )->end_table_row( )->end_table( )
    )->show( ).
  ENDMETHOD.
  ...
  METHOD do_show_content.
    IF ir_params IS BOUND.
      ASSIGN CAST lty_s_command_info( ir_params )->* TO FIELD-SYMBOL(<ls_command_info>).
      IF sy-subrc = 0.
        show_content( if_new_task    = if_new_task
      ...
      WHEN OTHERS.
        IF io_command->mv_function CP |{ c_dbl_click_action-function_prefix }*|.
          DATA(lv_new_action) = CONV zdbbr_action(
            substring( val = io_command->mv_function
                       off = strlen( c_dbl_click_action-function_prefix ) ) ).
          update_settings( iv_dbl_click_action = lv_new_action ).
        ENDIF.
  ...
  METHOD toggle_usage_alv.
    ...
    IF mo_usage_alv IS INITIAL.
      zcl_uitb_screen_util=>show_progress( iv_progress = 1
                                           iv_text     = |{ 'Analyzing Usages' }| ).
      mo_usage_alv = NEW #( io_dependency_tree = me
                            io_parent          = mo_switch->add_child( iv_id = 'ALV' )
                            iv_cds_view_name   = mv_view_name ).
    ENDIF.

    mo_switch->set_child_visible( iv_id      = 'TREE'
                                  if_visible = abap_false ).
    mo_switch->set_child_visible( iv_id      = 'ALV'
                                  if_visible = abap_true ).
  ENDMETHOD.
  ...
  METHOD open_with_adt.
    TRY.
        zcl_sat_adt_util=>jump_adt( iv_obj_name = |{ iv_entity_id }|
                                    iv_obj_type = SWITCH #(
                                    iv_entity_type
  ...
  METHOD do_open_with_adt.
    IF ir_params IS BOUND.
      ASSIGN CAST lty_s_command_info( ir_params )->* TO FIELD-SYMBOL(<ls_command_info>).
      IF sy-subrc = 0.
        open_with_adt( iv_entity_id   = <ls_command_info>-entity_id
                       iv_entity_type = <ls_command_info>-entity_type ).
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD do_open_in_new_window.
    IF ir_params IS BOUND.
      ASSIGN CAST lty_s_command_info( ir_params )->* TO FIELD-SYMBOL(<ls_command_info>).
      IF sy-subrc = 0.
        open_db_browser_new_window( iv_entity_id   = <ls_command_info>-entity_id
                                    iv_entity_type = <ls_command_info>-entity_type ).
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD get_metrics_calculator.
    IF gs_metrics_settings-settings_loaded = abap_false.
      ...
      gs_metrics_settings-has_bitmask_param = xsdbool( line_exists( lo_metrics_coll_typeref->methods[
