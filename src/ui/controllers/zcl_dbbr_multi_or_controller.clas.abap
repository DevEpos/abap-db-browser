...
CLASS zcl_dbbr_multi_or_controller IMPLEMENTATION.
  METHOD call_selfield_value_help.
    ...
    IF    ls_selfield-low  <> ls_selfield_save-low
       OR ls_selfield-high <> ls_selfield_save-high.
      mr_ui_multi_or_field->* = ls_selfield.
    ENDIF.
  ENDMETHOD.

  METHOD constructor.
    " TODO: parameter IT_MULTI_SELFIELD_ALL is never used (ABAP cleaner)
    ...
    mr_ui_selfields = CAST zdbbr_selfield_itab( lr_data_cache->get_data_ref(
                                                    zif_dbbr_main_report_var_ids=>c_t_selection_fields ) ).
    mr_ui_multi_or_fields = CAST zdbbr_selfield_itab( lr_data_cache->get_data_ref(
                                                          zif_dbbr_main_report_var_ids=>c_t_multi_or ) ).
    mr_ui_multi_or_tuple_no = CAST zdbbr_seq_number( lr_data_cache->get_data_ref(
                                                         zif_dbbr_main_report_var_ids=>c_v_or_tuple_number ) ).
    ...
    " delete parameters
    DELETE mr_ui_multi_or_fields->* WHERE tabname = zif_dbbr_c_global=>c_parameter_dummy_table.
...
*&---------------------------------------------------------------------*
    ...
    LOOP AT mr_ui_multi_or_fields->* ASSIGNING FIELD-SYMBOL(<ls_multi_or_selfield>).
      ...
      IF     <ls_selfield_line>-low    = space
         AND <ls_selfield_line>-high   = space
         AND <ls_selfield_line>-option = space.
        CONTINUE.
      ENDIF.
      ...
      LOOP AT mt_multi_selfield_all ASSIGNING FIELD-SYMBOL(<ls_selfield_multi>) WHERE     tabname   = <ls_multi_or_selfield>-tabname
                                                                                      AND fieldname = <ls_multi_or_selfield>-fieldname.
  ...
  METHOD go_to_next_tuple.
    ...
    mr_ui_multi_or_tuple_no->* = mr_ui_multi_or_tuple_no->* + 1.
  ...
  METHOD go_to_previous_criteria.
    ...
    WHILE lv_current_line > 0.
      ...
      IF    lr_current_entry->low    IS NOT INITIAL
         OR lr_current_entry->high   IS NOT INITIAL
         OR lr_current_entry->option IS NOT INITIAL.
  ...
  METHOD show_multi_selection_dialog.
    ...
    zcl_dbbr_dialogs=>show_multi_select( EXPORTING io_custom_f4_map  = mr_custom_f4_map
                                         CHANGING  cs_selfield       = <ls_selfield>
  ...
  METHOD store_current_tuple_data.
    ...
    LOOP AT mr_ui_multi_or_fields->* ASSIGNING FIELD-SYMBOL(<ls_multi_or>)
      ...
      IF     <ls_multi_or>-datatype  = 'DATS'
         AND <ls_multi_or>-option   <> space
         AND <ls_multi_or>-low       = space
         AND <ls_multi_or>-high      = space.
        " fill date with initial date
        <ls_multi_or_line>-low = VALUE sy-datum( ).
      ENDIF.

      " get multi selection values
      <ls_multi_or_line>-multi_values = VALUE #( FOR multi_value IN mt_multi_selection
                                                 WHERE (     tabname   = <ls_multi_or>-tabname
                                                         AND fieldname = <ls_multi_or>-fieldname )
  ...
  METHOD zif_uitb_screen_controller~handle_user_command.
    ...
    CASE lv_function_code.
      WHEN zif_dbbr_c_global=>c_function_codes-cancel OR
           ...
           zif_dbbr_c_global=>c_function_codes-leave_screen OR
      ...
      WHEN mc_functions-select_option_choice.
        ...
        zcl_uitb_cursor=>get_cursor( if_reset = abap_false )->set_line(
            mr_multi_or_table->zif_uitb_table~get_current_loop_line( ) ).
  ...
  METHOD zif_uitb_screen_controller~pbo.
    ...
    IF mf_search_successful = abap_true.
      zcl_uitb_cursor=>set_cursor( iv_field = 'GS_MULTI_OR-LOW'
                                   iv_line  = 1 ).
