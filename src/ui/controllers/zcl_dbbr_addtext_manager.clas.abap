CLASS zcl_dbbr_addtext_manager DEFINITION
  ...
  PROTECTED SECTION.
    ...
    METHODS get_selection_mode    REDEFINITION.
    METHODS get_popup_dimensions  REDEFINITION.
    METHODS adjust_columns        REDEFINITION.
    METHODS on_user_command       REDEFINITION.
    METHODS do_after_alv_creation REDEFINITION.
    METHODS get_table_reference   REDEFINITION.

  PRIVATE SECTION.
    DATA mt_addtext_data TYPE zdbbr_addtext_ui_itab.
    DATA mt_table_selopt TYPE zif_sat_ty_global=>ty_t_selopt.
    DATA mt_tabinfo TYPE zdbbr_tabinfo_itab.
...
*&---------------------------------------------------------------------*
  ...
  METHOD select_data.
    ...
    DATA(lr_addtext_f) = NEW zcl_dbbr_addtext_factory( ).
    lr_addtext_f->find_add_texts_for_tablist( EXPORTING it_tabname_selopt = mt_table_selopt
