CLASS zcl_dbbr_tabfield_tree_f4 DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Sets the preferred entity which is to be expanded</p>
...
CLASS zcl_dbbr_tabfield_tree_f4 IMPLEMENTATION.
  ...
  METHOD set_preferred_entity.
    ms_preferred_entity = VALUE #( entity_alias    = iv_entity_alias
  ...
  METHOD create_tree.
    mo_tree = NEW zcl_uitb_column_tree_model( ir_parent           = io_container
                                              is_hierarchy_header = VALUE treemhhdr( heading = 'Table / Field' ) ).
    ...
    SET HANDLER on_node_double_click FOR lr_events.
    SET HANDLER on_node_key_press FOR lr_events.
  ...
  METHOD expand_preferred_node.
    TRY.
        DATA(lr_s_preferred_node) = REF #( mt_node_map[ alias     = ms_preferred_entity-entity_alias
