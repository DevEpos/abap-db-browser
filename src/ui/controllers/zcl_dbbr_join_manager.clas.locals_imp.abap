...
CLASS lcl_join_field IMPLEMENTATION.
  ...
  METHOD update_field.
    mf_changed = xsdbool(
          ms_field-field           <> value-field
       OR ms_field-off_length      <> value-off_length
       OR ms_field-off_offset      <> value-off_offset
       OR ms_field-operator        <> value-operator
       OR ms_field-ref_table       <> value-ref_table
       OR ms_field-ref_table_alias <> value-ref_table_alias
       OR ms_field-ref_field       <> value-ref_field ).
...
CLASS lcl_join_filter IMPLEMENTATION.
  ...
  METHOD update_filter.
    mf_changed = xsdbool(
          ms_filter-fieldname     <> value-fieldname
       OR ms_filter-operator      <> value-operator
       OR ms_filter-tabname       <> value-tabname
       OR ms_filter-tabname_alias <> value-tabname_alias
       OR ms_filter-value         <> value-value
       OR ms_filter-value2        <> value-value2
       OR ms_filter-value_type    <> value-value_type ).
    rf_changed = mf_changed.
    ms_filter = value.
  ENDMETHOD.
ENDCLASS.
...
CLASS lcl_join_table IMPLEMENTATION.
  ...
  METHOD set_tab_info.
    mf_changed = xsdbool(
         ms_table_info-is_virtual      <> value-is_virtual
      OR ms_table_info-join_type       <> value-join_type
      OR ms_table_info-add_table_alias <> value-add_table_alias ).
    rf_changed = mf_changed.
    ms_table_info = value.
  ENDMETHOD.
      ...
      IF lv_relat_index = 0.
        mt_filter_cond = VALUE #( BASE mt_filter_cond
                                  ( node_key   = io_filter->mv_node_key
                                    filter_ref = io_filter ) ).
      ELSE.
        INSERT VALUE #( node_key   = io_filter->mv_node_key
                        filter_ref = io_filter )
  ...
  METHOD add_field.
    ...
    ELSE.
      ...
      INSERT VALUE #( node_key  = io_field->mv_node_key
                      field_ref = io_field )
  ...
  METHOD to_structure.
    ...
    LOOP AT mt_field_cond ASSIGNING FIELD-SYMBOL(<ls_field_info>).
      rs_table-field_conditions = VALUE #( BASE rs_table-field_conditions
...
CLASS lcl_join IMPLEMENTATION.
  ...
  METHOD to_structure.
    ...
    LOOP AT mt_tables ASSIGNING FIELD-SYMBOL(<ls_table>).
      rs_join_def-tables = VALUE #( BASE rs_join_def-tables
    ...
    LOOP AT mt_tables ASSIGNING FIELD-SYMBOL(<ls_table>).
      rt_nodes_to_update = VALUE #( BASE rt_nodes_to_update
                                    ( LINES OF CORRESPONDING #(
                                         <ls_table>-tab_ref->update_alias( iv_old_alias = mv_primary_entity_alias
  ...
  METHOD update_table_alias.
    LOOP AT mt_tables ASSIGNING FIELD-SYMBOL(<ls_join_table>).
      rt_nodes_to_update = VALUE #( BASE rt_nodes_to_update
                                    ( LINES OF CORRESPONDING #(
                                         <ls_join_table>-tab_ref->update_alias( iv_old_alias = iv_old_alias
  ...
  METHOD get_possible_entities_for_f4.
    rt_entities = VALUE #( ( entity_id     = mv_primary_entity
