...
CLASS zcl_dbbr_eb_settings_view IMPLEMENTATION.
  ...
  METHOD save_settings.
    zcl_dbbr_usersettings_factory=>set_entity_browser_settings(
        VALUE #( link_mode       = ms_settings_ref-link_mode->*
  ...
  METHOD zif_uitb_screen_controller~call_screen.
    ...
    zcl_uitb_screen_util=>call_screen( iv_screen_id    = get_screen_id( )
                                       ...
                                       it_object_map   = VALUE #( ( variable_name = c_r_controller
                                                                    global_ref    = me ) )
                                       iv_start_column = 10
                                       iv_start_line   = 2 ).
  ENDMETHOD.
  ...
  METHOD zif_uitb_screen_controller~pbo.
    ...
    IF mf_first_call = abap_true.
      lt_list = VALUE #( ( key = zif_dbbr_c_object_browser_mode=>cds_view       text = 'Find CDS Views'(001) )
      ...
      CALL FUNCTION 'VRM_SET_VALUES'
        EXPORTING id     = CONV vrm_id( c_p_entry_search_function )
                  values = lt_list.
      CLEAR mf_first_call.
    ENDIF.
    set_status( ).
