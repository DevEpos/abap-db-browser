...
CLASS zcl_dbbr_edit_join_table_ctrl DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Created new join table</p>
    EVENTS created
      EXPORTING
    ...
    "! <p class="shorttext synchronized">Retrieves the updated join table</p>
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Join table</p>
    ...
    "! <p class="shorttext synchronized">Sends update value via event and clears screen values</p>
    "!
    ...
    "! <p class="shorttext synchronized">Saves the validated Join Entity</p>
    "!
    METHODS save_entity
      IMPORTING
...
*......... clear save flag for next saving
  ...
  METHOD validate.
    ...
    TRY.
      ...
      CATCH zcx_sat_data_read_error INTO DATA(lx_read_error).
        RAISE EXCEPTION TYPE zcx_sat_validation_exception
          EXPORTING previous       = lx_read_error
    ...
    IF mr_alias->* IS INITIAL.
      mr_alias->* = zcl_dbbr_entity_alias_util=>create_entity_alias( iv_entity = to_upper( mr_join_table->* ) ).
    ENDIF.
    ...
    TRY.
      ...
      CATCH zcx_sat_validation_exception INTO DATA(lx_valid_error).
        ...
        zcx_sat_validation_exception=>raise_with_text( iv_text      = lx_valid_error->get_text( )
                                                       iv_parameter = c_p_alias ).
    ENDTRY.

    IF     mr_join_type->*       = zif_sat_c_join_types=>right_outer_join
       AND mr_is_virtual_join->* = abap_true.
      zcx_sat_validation_exception=>raise_with_text( iv_text      = |Right Outer Join is not possible with Virtual Join|
  ...
  METHOD zif_uitb_screen_controller~call_screen.
    zcl_uitb_screen_util=>call_screen( iv_screen_id    = get_screen_id( )
                                       ...
                                       it_object_map   = VALUE #( ( variable_name = c_r_edit_join_table_controller
                                                                    global_ref    = me ) )
                                       iv_start_column = 10
                                       iv_start_line   = 2 ).
  ENDMETHOD.
  ...
  METHOD save_entity.
    ms_join_table = VALUE #( add_table       = mr_join_table->*
