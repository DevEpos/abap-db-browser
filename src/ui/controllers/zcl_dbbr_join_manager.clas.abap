...
CLASS zcl_dbbr_join_manager DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        ...
        !ir_s_join_ref    TYPE REF TO zdbbr_join_def .
  ...
  PRIVATE SECTION.
    ...
    DATA mr_join TYPE REF TO lcl_join .
    "! <p class="shorttext synchronized" lang="en">Tree for storing join definition</p>
    DATA mr_join_tree TYPE REF TO zcl_uitb_column_tree_model .
    CONSTANTS:
      BEGIN OF c_columns,
    ...
    DATA mr_s_join_def TYPE REF TO zdbbr_join_def .
    DATA mr_splitter TYPE REF TO cl_gui_splitter_container .
    DATA mr_header_document TYPE REF TO cl_dd_document .
    ...
    DATA:
      mt_node_map TYPE STANDARD TABLE OF lty_node_map WITH KEY node_key .
    "! <p class="shorttext synchronized" lang="en">Tree Model: Node Key</p>
    DATA ms_new_cond_relat_nodemap TYPE lty_node_map .
    DATA mv_new_cond_relationship TYPE i .
    "! <p class="shorttext synchronized" lang="en">Node of a column tree model</p>
    DATA mr_relat_node TYPE REF TO zcl_uitb_ctm_node .
    ...
    METHODS create_filter_condition_node
      ...
      RETURNING
        VALUE(rv_node_key)   TYPE tm_nodekey .
    "! <p class="shorttext synchronized" lang="en">Create header document to hold some information</p>
    ...
    "! <p class="shorttext synchronized" lang="en">Create GUI Status functions for Join Manager</p>
    METHODS create_status_functions .
    "! <p class="shorttext synchronized" lang="en">Creates toolbar for Tree</p>
    ...
    METHODS delete_all_conditions
      IMPORTING
        ...
        !ir_join_table TYPE REF TO lcl_join_table
        !is_node_map   TYPE lty_node_map
      RAISING
        zcx_uitb_tree_error .
    ...
    METHODS fill_value_item_content
      IMPORTING
        !ir_node             TYPE REF TO zcl_uitb_ctm_node
        !ir_filter_condition TYPE REF TO lcl_join_filter .
    "! <p class="shorttext synchronized" lang="en">Get next free node key</p>
    METHODS get_next_key
      RETURNING
        VALUE(result) TYPE tm_nodekey .
    "! <p class="shorttext synchronized" lang="en">Handler for PBO of program</p>
    METHODS on_before_output
          FOR EVENT before_output OF zif_uitb_view_callback
      IMPORTING
          !er_callback .
    "! <p class="shorttext synchronized" lang="en">Handler for Button press in tree</p>
    METHODS on_button_click
          FOR EVENT button_click OF zcl_uitb_ctm_events
      IMPORTING
          !ev_node_key
          !ev_item_name .
    ...
    METHODS on_delete_request_from_join
      ...
      IMPORTING
          !ev_node_key .
    "! <p class="shorttext synchronized" lang="en">Handler for when screen should be left</p>
    METHODS on_exit
    ...
    METHODS on_node_keypress
      ...
      IMPORTING
          !ev_node_key
          !ev_key .
    "! <p class="shorttext synchronized" lang="en">Handler for Toolbar function click</p>
    METHODS on_toolbar_function
          FOR EVENT function_selected OF zif_uitb_toolbar_events
      IMPORTING
          !ev_fcode .
    "! <p class="shorttext synchronized" lang="en">Handler for user command of program</p>
...
CLASS zcl_dbbr_join_manager IMPLEMENTATION.
  ...
  METHOD add_and_condition.
    ...
    TRY .
*...... Get selected node
        DATA(lt_nodes) = mr_join_tree->get_selections( )->get_selected_nodes( ).
        CHECK lines( lt_nodes ) = 1.
        DATA(lr_selected_node) = lt_nodes[ 1 ].

*...... Retrieve information about node from map
        ASSIGN mt_node_map[ node_key = lr_selected_node->mv_node_key ] TO FIELD-SYMBOL(<ls_node>).
        CHECK sy-subrc = 0.
...
*.......... get join table reference to determine possible virtual join mode for condition view
            DATA(lr_join_table) = mr_join->get_table( <ls_node>-tabname ).
            lv_source_table = <ls_node>-tabname.
            DATA(lf_virtual_join) = lr_join_table->get_tab_info( )-is_virtual.
...
*...... Get target tables for condition view
        DATA(lt_tables_range) = mr_join->get_possible_tables_for_f4( iv_table = <ls_node>-tabname ).
        IF lf_exclude_current_table = abap_true.
          DELETE lt_tables_range WHERE low = <ls_node>-tabname.
        ENDIF.

        ms_new_cond_relat_nodemap = <ls_node>.
        mr_relat_node = mr_join_tree->get_nodes( )->get_node( <ls_node>-node_key ).

*...... Create new AND condition
        DATA(lr_edit_join_cond_view) = NEW zcl_dbbr_edit_join_cond_view(
          iv_mode              = lv_condition_view_mode
          it_target_table_list = lt_tables_range
...
*...... Set the node type to 'AND' for successive create calls from view
        ...
        SET HANDLER:
           on_created_filter_condition FOR lr_edit_join_cond_view,
           on_created_field_condition FOR lr_edit_join_cond_view.

        lr_edit_join_cond_view->show( ).

        CHECK lr_edit_join_cond_view->was_not_cancelled( ).

*...... Retrieve the updated information from the view
        lr_edit_join_cond_view->get_updated_condition(
          IMPORTING
...
*...... create the appropriate node
  ...
  METHOD add_new_join_table.
    DATA(lr_edit_join_table_controller) = NEW zcl_dbbr_edit_join_table_ctrl( ).

    SET HANDLER: on_created_join_table FOR lr_edit_join_table_controller.
    lr_edit_join_table_controller->zif_uitb_screen_controller~call_screen( ).

    CHECK lr_edit_join_table_controller->zif_uitb_screen_controller~was_not_cancelled( ).

    create_join_table_node( lr_edit_join_table_controller->get_updated_join_table( ) ).
  ENDMETHOD.
...
*...... Get target tables for condition view
        ...
        ms_new_cond_relat_nodemap = <ls_node>.
        mr_relat_node = lr_selected_node.
...
*...... fetch the node to get the information about the join field condition
        DATA(lr_node) = mr_join_tree->get_nodes( )->get_node( iv_node_key ).
        DATA(lr_join_field) = CAST lcl_join_field( lr_node->get_user_object( ) ).
        DATA(ls_join_field) = lr_join_field->get_field( ).

*...... get join table as well, to get virtual join information
        DATA(lr_join_table) = mr_join->get_table( lr_join_field->mv_table ).

*...... build target table list
        DATA(lt_tables_range) = mr_join->get_possible_tables_for_f4( iv_table = lr_join_field->mv_table ).
        DELETE lt_tables_range WHERE low = lr_join_field->mv_table.

*...... change the join field information
        DATA(lr_edit_join_cond) = NEW zcl_dbbr_edit_join_cond_view(
            if_is_new            = abap_false
            if_allow_offset      = lr_join_table->get_tab_info( )-is_virtual
            iv_source_table      = lr_join_field->mv_table
            is_field_condition   = ls_join_field
            it_target_table_list = lt_tables_range
        ).
        lr_edit_join_cond->show( ).

        CHECK lr_edit_join_cond->was_not_cancelled( ).
        lr_edit_join_cond->get_updated_condition( IMPORTING es_field_condition  = ls_join_field ).


*...... update the join field information
        lr_node->get_item( c_columns-field )->set_text( |{ ls_join_field-field }| ).
        lr_node->get_item( c_columns-comparator1 )->set_text( |{ ls_join_field-operator }| ).
        lr_node->get_item( c_columns-value )->set_text( |{ ls_join_field-ref_table }-{ ls_join_field-ref_field }| ).

        CHECK lr_join_field->update_field( ls_join_field ).
        MESSAGE |Field Condition was updated| TYPE 'S'.
...
*...... fetch the node to get the information about the join filter condition
        DATA(lr_node) = mr_join_tree->get_nodes( )->get_node( iv_node_key ).
        DATA(lr_join_filter) = CAST lcl_join_filter( lr_node->get_user_object( ) ).
        DATA(ls_join_filter) = lr_join_filter->get_filter( ).

*...... build target table list
        DATA(lt_tables_range) = mr_join->get_possible_tables_for_f4( iv_table = lr_join_filter->mv_table ).
...
*...... change the join field information
        ...
        CHECK lr_edit_join_cond->was_not_cancelled( ).
        lr_edit_join_cond->get_updated_condition( IMPORTING es_filter_condition = ls_join_filter ).

        CHECK lr_join_filter->update_filter( ls_join_filter ).
        fill_value_item_content(
          ir_node             = lr_node
          ir_filter_condition = lr_join_filter
        ).

*...... update the join field information
        lr_node->get_item( c_columns-field )->set_text( |{ ls_join_filter-tabname }-{ ls_join_filter-fieldname }| ).
        lr_node->get_item( c_columns-comparator1 )->set_text( |{ ls_join_filter-operator }| ).
        lr_node->get_item( c_columns-value )->set_text( |{ ls_join_filter-value }| ).
        lr_node->get_item( c_columns-value2 )->set_text( |{ ls_join_filter-value2 }| ).
        IF ls_join_filter-operator = zif_dbbr_c_operator=>between.
          lr_node->get_item( c_columns-comparator2 )->set_text( |and| ).
        ELSE.
          lr_node->get_item( c_columns-comparator2 )->set_text( || ).
        ENDIF.
...
*...... 1) fetch the node to get the information about the join table
        DATA(lr_node) = mr_join_tree->get_nodes( )->get_node( iv_node_key ).
        DATA(lr_join_table) = CAST lcl_join_table( lr_node->get_user_object( ) ).

*...... 2) change the join table
        DATA(ls_current_tab_info) = lr_join_table->get_tab_info( ).
        DATA(lr_edit_join_table_ctrl) = NEW zcl_dbbr_edit_join_table_ctrl(
            is_join_table = ls_current_tab_info
            if_is_new     = abap_false
        ).
        lr_edit_join_table_ctrl->zif_uitb_screen_controller~call_screen( ).

        CHECK lr_edit_join_table_ctrl->zif_uitb_screen_controller~was_not_cancelled( ).
        DATA(ls_updated_tab_info) = lr_edit_join_table_ctrl->get_updated_join_table( ).

        CHECK lr_join_table->set_tab_info( ls_updated_tab_info ).
...
*........ clear all offset information from associated field conditions
          lr_join_table->clear_offset_from_fields( ).
        ENDIF.

*...... update the join type
        lr_node->get_item( c_columns-value )->set_text(
  ...
  METHOD change_node.
    TRY.
        ...
        DATA(lt_selected_nodes) = mr_join_tree->get_selections( )->get_selected_nodes( ).
...
*...... perform appropriate edit action according to node type
  ...
  METHOD class_constructor.
    DATA(lr_join_type_descr) = CAST cl_abap_elemdescr(
      cl_abap_typedescr=>describe_by_data( VALUE zdbbr_jointype( ) )
    ).

    st_join_type_fixvals = lr_join_type_descr->get_ddic_fixed_values(
        p_langu = 'E'
    ).
  ENDMETHOD.
  ...
  METHOD constructor.
    ...
    mr_s_join_def = ir_s_join_ref.
    mv_title = |DB Browser - Joins for { mv_primary_table }|.
    mr_template_prog = zcl_uitb_templt_prog_callback=>create_template_program(
      mv_title
    ).
    ...
    create_status_functions( ).

  ENDMETHOD.


  METHOD create_columns.
    DATA(lr_cols) = mr_join_tree->get_columns( ).
    lr_cols->add_column(
  ...
  METHOD create_field_condition_node.
    DATA(lr_new_field_cond) = NEW lcl_join_field(
    ...
    IF ms_new_cond_relat_nodemap-node_type = c_node_type-table_fields.
      IF mr_relat_node IS BOUND AND NOT mr_relat_node->has_children( ).
        CLEAR lv_and_text.
      ENDIF.
    ENDIF.

    TRY.
        DATA(lr_new_field_cond_node) = mr_join_tree->get_nodes( )->add_node(
            iv_node_key          = lr_new_field_cond->mv_node_key
            iv_relationship      = mv_new_cond_relationship
            iv_relative_node_key = ms_new_cond_relat_nodemap-node_key
            if_folder            = abap_false
            ir_user_object       = lr_new_field_cond
            ...
            it_item_table        = VALUE #(
              ...
              ( item_name = c_columns-value
                class     = cl_column_tree_model=>item_class_text
                text      = |{ is_field_cond-ref_table }-{ is_field_cond-ref_field }| )
            )
        ).

*...... Build the node map entry for the new table
        DATA(ls_field_node_map) = VALUE lty_node_map(
          node_key  = lr_new_field_cond->mv_node_key
...
*...... expand all nodes of new table node
        IF mv_new_cond_relationship = cl_tree_model=>relat_last_child.
          mr_join_tree->get_nodes( )->expand_node(
...
*...... add the new condition to the join table
        DATA(lr_join_table) = mr_join->get_table( iv_table = ms_new_cond_relat_nodemap-tabname ).
        lr_join_table->add_field(
            ir_field          = lr_new_field_cond
            ir_field_node     = lr_new_field_cond_node
        ).
...
*...... update node map entry and relation ship for next new entry
        IF if_from_event = abap_true.
          ...
          mv_new_cond_relationship = cl_tree_model=>relat_next_sibling.
          mr_relat_node = lr_new_field_cond_node.
        ELSE.
        ...
        rv_node_key = lr_new_field_cond->mv_node_key.

        IF mf_init_mode = abap_false.
          MESSAGE |Field Condition was created| TYPE 'S'.
          mr_join_tree->get_selections( )->select_nodes( VALUE #( ( rv_node_key ) ) ).
        ENDIF.
      CATCH zcx_uitb_tree_error INTO DATA(lx_tree_error).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD create_filter_condition_node.
    DATA(lr_new_filter_cond) = NEW lcl_join_filter(
...
*.. set AND/OR according to requested node type
    ...
    lr_new_filter_cond->set_and_or( lv_and_or ).

    IF mr_relat_node IS BOUND AND
*       mv_new_cond_relationship <> cl_tree_model=>relat_last_child AND
       mr_relat_node->is_folder( ) AND
       NOT mr_relat_node->has_children( ).
      CLEAR lv_and_or.
    ENDIF.

    TRY.
        DATA(lr_new_filter_cond_node) = mr_join_tree->get_nodes( )->add_node(
            iv_node_key          = lr_new_filter_cond->mv_node_key
            iv_relationship      = mv_new_cond_relationship
            iv_relative_node_key = ms_new_cond_relat_nodemap-node_key
            if_folder            = abap_false
            ir_user_object       = lr_new_filter_cond
            iv_image             = |{ icon_io_predefined_value }|
            it_item_table        = VALUE treemcitab(
               ...
               ( item_name = c_columns-field
                 class     = cl_column_tree_model=>item_class_text
                 text      = |{ is_filter_condition-tabname }-{ is_filter_condition-fieldname }| )
               ( item_name = c_columns-comparator1
                 class     = cl_column_tree_model=>item_class_text
...
*...... update or group for filter ( if necessary )
        update_or_group_of_filter(
          ir_filter_node = lr_new_filter_cond_node
          iv_node_type   = iv_node_type
        ).
        fill_value_item_content( ir_node             = lr_new_filter_cond_node
                                 ir_filter_condition = lr_new_filter_cond      ).

*...... Build the node map entry for the new filter condition
        DATA(ls_field_node_map) = VALUE lty_node_map(
          node_key  = lr_new_filter_cond->mv_node_key
          tabname   = ms_new_cond_relat_nodemap-tabname
...
*        determine_predecessor_
        lr_join_table->add_filter(
            ir_filter         = lr_new_filter_cond
            ir_filter_node    = lr_new_filter_cond_node
*            iv_relationship   = mv_new_cond_relationship
*            iv_relat_node_key = ms_new_cond_relat_nodemap-node_key
        ).
...
*...... update node map entry and relation ship for next new entry
        IF if_from_event = abap_true.
          ms_new_cond_relat_nodemap = ls_field_node_map.
          mv_new_cond_relationship = cl_tree_model=>relat_next_sibling.
          mr_relat_node = lr_new_filter_cond_node.
        ELSE.
          CLEAR: mv_new_cond_relationship,
                 mr_relat_node,
                 ms_new_cond_relat_nodemap.
        ...
        rv_node_key = lr_new_filter_cond->mv_node_key.
  ...
  METHOD create_header.
    CHECK mr_header_document IS INITIAL.

    DATA(lr_container) = mr_splitter->get_container( column = 1 row = 1 ).

    mr_header_document = NEW cl_dd_document( ).

    mr_header_document->initialize_document( background_color = cl_dd_area=>col_background_level2 ).

    mr_header_document->add_table(
    ...
    lr_table->new_row( ).
*.. Read description for primary tables
    DATA(ls_prim_tab_info) = zcl_dbbr_dictionary_helper=>get_table_info( mv_primary_table ).

    lr_col1->add_text( text = |{ mv_primary_table } - { ls_prim_tab_info-ddtext }| ).

    mr_header_document->merge_document( ).

    mr_header_document->display_document( parent = lr_container reuse_control = abap_true ).
  ENDMETHOD.
...
*... check if this node already exists
    IF mr_join->has_table( is_join_table-add_table ).
      MESSAGE |Join Table { is_join_table-add_table } has already been added| TYPE 'I' DISPLAY LIKE 'E'.
      RETURN.
    ELSEIF is_join_table-add_table = mv_primary_table.
      MESSAGE |Table { is_join_table-add_table } is the primary table and cannot be added as a join table| TYPE 'I' DISPLAY LIKE 'E'.
      RETURN.
    ENDIF.

*... Create new table node for entered join table
    DATA(ls_tab_info) = zcl_dbbr_dictionary_helper=>get_table_info( |{ is_join_table-add_table }| ).

*... Create new join table entry
    DATA(lr_new_join_table) = NEW lcl_join_table(
        is_join_table = is_join_table
        iv_node_key   = get_next_key( )
    ).

    SET HANDLER on_delete_request_from_join FOR lr_new_join_table.

    mr_join->add_table( lr_new_join_table ).

    TRY.
        mr_join_tree->get_nodes( )->add_node(
            iv_node_key          = lr_new_join_table->mv_node_key
            if_folder            = abap_true
            ir_user_object       = lr_new_join_table
            iv_image             = |{ icon_database_table }|
            iv_expanded_image    = |{ icon_database_table }|
            it_item_table        = VALUE #(
              ( item_name = zcl_uitb_column_tree_model=>c_hierarchy_column
                class     = cl_column_tree_model=>item_class_text
                text      = |{ is_join_table-add_table }| )
              ( item_name = c_columns-field
                class     = cl_column_tree_model=>item_class_text
...
*...... Build the node map entry for the new table
        mt_node_map = VALUE #( BASE mt_node_map
          ( node_key  = lr_new_join_table->mv_node_key
...
*...... Add sub nodes for field conditions
        ...
        mr_join_tree->get_nodes( )->add_node(
            ...
            iv_relative_node_key = lr_new_join_table->mv_node_key
            if_folder            = abap_true
...
*...... expand all nodes of new table node
        ...
        rv_node_key = lr_new_join_table->mv_node_key.
  ...
  METHOD create_or_group_node.
    DATA(lv_and_text) = and.
    TRY .
        DATA(lf_first_child) = mr_relat_node->is_first_child( ).
...
*...... unecessary as the OR Group node will become the first child
        ...
        DATA(lr_or_group_node) = mr_join_tree->get_nodes( )->add_node(
...
*...... Create node map entry for this node
        DATA(ls_or_group_nm) = VALUE lty_node_map(
...
*...... the table filter has to be moved under the new or group
        IF ms_new_cond_relat_nodemap-node_type = c_node_type-table_filter.
          IF lf_first_child = abap_false.
            mr_relat_node->get_item( mr_join_tree->c_hierarchy_column )->set_text( '' ).
          ENDIF.

          mr_relat_node->move_node_to(
            iv_relative_node = rv_node_key
            iv_relationship  = cl_tree_model=>relat_first_child
          ).
          update_or_group_of_filter(
              ir_filter_node      = mr_relat_node
...
*........ changed in the user object and the node map entry
          ASSIGN mt_node_map[ node_key = mr_relat_node->mv_node_key ] TO FIELD-SYMBOL(<ls_and_filter>).
          <ls_and_filter>-node_type = c_node_type-table_filter_or.
          CAST lcl_join_filter( mr_relat_node->get_user_object( ) )->set_and_or( or ).
          mr_relat_node->get_item( mr_join_tree->c_hierarchy_column )->set_text( '' ).
        ENDIF.
...
*........ Update relative node information
        ...
        mv_new_cond_relationship = cl_tree_model=>relat_last_child.
        mr_relat_node = lr_or_group_node.
      CATCH zcx_uitb_tree_error.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD create_status_functions.
    mr_template_prog->add_function(
        iv_function_id = zif_uitb_template_prog=>c_func_f8
        iv_text        = |{ 'Update Join Definition'(009) }|
        iv_icon        = |{ icon_okay }|
        iv_icon_text   = |{ 'Transfer'(010) }|
    ).

    mr_template_prog->add_function(
        iv_function_id = zif_uitb_template_prog=>c_func_shift_f8
        iv_text        = |{ 'Show SQL FROM'(017) }|
        iv_icon        = |{ icon_display_text }|
        iv_icon_text   = |{ 'Show SQL'(019) }|
    ).

    mr_template_prog->add_function(
        iv_function_id = zif_uitb_template_prog=>c_func_f9
        iv_text        = |{ 'Help' }|
  ...
  METHOD create_toolbar.
    DATA(lr_toolbar) = mr_join_tree->get_toolbar( ).

    SET HANDLER:
      on_toolbar_function FOR lr_toolbar.

    lr_toolbar->add_button(
        iv_fcode     = c_functions-expand_all_nodes
        iv_icon      = icon_expand_all
        iv_quickinfo = 'Expand all nodes'(007)
    ).
    lr_toolbar->add_button(
        iv_fcode     = c_functions-collapse_all_nodes
        iv_icon      = icon_collapse_all
        iv_quickinfo = 'Collapse all'(008)
    ).
    lr_toolbar->add_separator( ).
    lr_toolbar->add_button(
        iv_fcode     = c_functions-add_table
        iv_icon      = icon_create
        iv_text      = 'Join Table'(013)
    ).
    lr_toolbar->add_button(
        iv_fcode     = c_functions-delete_table
        iv_icon      = icon_delete
        iv_text      = 'Join Table'(013)
    ).
    lr_toolbar->add_button(
        iv_fcode     = c_functions-delete_all_tables
        iv_icon      = icon_delete
        iv_text      = 'All Join Tables'(015)
    ).
    lr_toolbar->add_separator( ).
    lr_toolbar->add_button(
        iv_fcode     = c_functions-add_and_condition
        iv_icon      = icon_insert_relation
        iv_text      = 'AND Condition'
    ).
    lr_toolbar->add_button(
        iv_fcode     = c_functions-add_or_condition
        iv_icon      = icon_tree
        iv_text      = 'OR Condition'
    ).
    lr_toolbar->add_button(
        iv_fcode     = c_functions-edit_condition
        iv_icon      = icon_change
        iv_text      = 'Condition'
    ).
    lr_toolbar->add_button(
        iv_fcode     = c_functions-delete_condition
        iv_icon      = icon_delete
        iv_text      = 'Condition'
    ).
  ENDMETHOD.
...
*.... create the tree model for the association tree
    mr_join_tree = NEW #(
        ir_parent           = mr_splitter->get_container( column = 1 row = 2 )
    ...
    DATA(lr_events) = mr_join_tree->get_events( ).

    SET HANDLER:
      on_node_double_click      FOR lr_events,
      on_node_keypress          FOR lr_events,
      on_node_ctx_menu_request  FOR lr_events,
      on_node_ctx_menu_select   FOR lr_events,
      on_button_click           FOR lr_events.
    ...
    TRY.
        ...
        lr_events->add_key_for_keypress( iv_key =  cl_tree_model=>key_delete ).

        mr_join_tree->create_tree_control( ).
        mr_join_tree->update_view( ).
      CATCH zcx_uitb_tree_error INTO DATA(lx_tree_error).
  ...
  METHOD delete_all_conditions.
    DATA: lt_node_type_to_delete TYPE ltt_node_type_range.

    DATA(lr_join_table) = mr_join->get_table( is_node_map-tabname ).

    CASE is_node_map-node_type.

      WHEN c_node_type-table_fields.

        IF ir_node->has_children( ) AND
    ...
    ir_node->delete_children( ).

*.. Delete all child nodes and all node map entries for the given type
    DELETE mt_node_map WHERE tabname   = is_node_map-tabname
...
*...... Delete the condition
        CASE <ls_node>-node_type.
          ...
          WHEN c_node_type-table_filter OR
               c_node_type-table_filter_or.
            lr_join_table->delete_filter( <ls_node>-node_key ).
            delete_filter_condition(
                ir_node       = lr_selected_node
                ir_join_table = lr_join_table
                is_node_map   = <ls_node>
            ).
  ...
  METHOD delete_field_condition.
    ir_join_table->delete_field( is_node_map-node_key ).

*.. Reset the 'AND' text of the node if necessary
    DATA(lr_parent) = ir_node->get_parent( ).
    DATA(lr_first_child) = lr_parent->get_first_child( ).
    IF ir_node->mv_node_key = lr_first_child->mv_node_key AND
       lr_parent->get_nr_of_children( ) > 1.
      lr_first_child->get_next_sibling( )->get_item(
  ...
  METHOD delete_filter_condition.
    ir_join_table->delete_filter( is_node_map-node_key ).
...
*.. first get previous sibling to update selection after node was deleted
    DATA(lr_next_node) = ir_node->get_next_relat_node( IMPORTING ef_is_sibling = lf_relat_is_sibling ).

*.. delete the node
    mr_join_tree->get_nodes( )->delete_node( is_node_map-node_key ).
...
*.. have been deleted and the group itself has to be deleted as well
      DATA(ls_new_node_map) = mt_node_map[ node_key = lr_next_node->mv_node_key ].
...
*.... Call the same method again to delete the or group as well
    ...
    ELSE.
*.... update selection
      mr_join_tree->get_selections( )->select_nodes( VALUE #( ( lr_next_node->mv_node_key ) ) ).
    ENDIF.
  ...
  METHOD delete_or_group.
    ...
    LOOP AT ir_node->get_children( ) INTO DATA(lr_or_filter_node).
      ir_join_table->delete_filter( lr_or_filter_node->mv_node_key ).
      DELETE mt_node_map WHERE node_key = lr_or_filter_node->mv_node_key.
    ENDLOOP.

    DELETE mt_node_map WHERE node_key = ir_node->mv_node_key.

    ir_node->delete_children( ).

    DATA(lr_filters_node) = ir_node->get_parent( ).
    IF lr_filters_node->get_nr_of_children( ) = 2 AND
       ir_node->is_first_child( ).
*.... set 'AND' text to '' as after this group is deleted the other one will
*.... be the only remaining filter node
      DATA(lt_children) = lr_filters_node->get_children( ).
      DATA(lr_2nd_child) = lt_children[ 2 ].
      lr_2nd_child->get_item( mr_join_tree->c_hierarchy_column )->set_text( '' ).
    ENDIF.

    DATA(lr_next_node) = ir_node->get_next_relat_node( ).

*.. finally delete the node itself
    mr_join_tree->get_nodes( )->delete_node( ir_node->mv_node_key ).
  ...
  METHOD delete_table.
    ...
    TRY .
        ...
        IF if_all = abap_true.
          ...
          CLEAR: mt_node_map.
          mr_join->delete_all_tables( ).
          mr_join_tree->get_nodes( )->delete_all_nodes( ).
        ELSE.
*........ Get selected node
          DATA(lt_nodes) = mr_join_tree->get_selections( )->get_selected_nodes( ).
          CHECK lines( lt_nodes ) = 1.
...
*........ Delete the table
          mr_join->delete_table( <ls_node>-tabname ).
*........ delete the node
          mr_join_tree->get_nodes( )->delete_node( <ls_node>-node_key ).
*........ the node map entry also has to be deleted
  ...
  METHOD do_on_first_screen_call.
    ...
    fill_tree_with_join( ).
*.. set focus on tree
    mr_join_tree->zif_uitb_gui_control~focus( ).
  ENDMETHOD.
  ...
  METHOD fill_tree_with_join.
    ...
    mr_join = NEW #( mv_primary_table ).
    mf_init_mode = abap_true.

    ASSIGN mr_s_join_def->* TO <ls_join_def>.
...
*..... get node map entry for join tables fields node
      ASSIGN mt_node_map[ tabname   = <ls_join_table>-add_table
      ...
      mr_relat_node = mr_join_tree->get_nodes( )->get_node( <ls_join_tab_fields_node>-node_key ).
...
*..... get node map entry for join table's filters node
      ...
      DATA(lr_join_tab_filters_node) = mr_join_tree->get_nodes( )->get_node( <ls_join_tab_filters_node>-node_key ).
      mv_new_cond_relationship = cl_tree_model=>relat_last_child.
      ms_new_cond_relat_nodemap = <ls_join_tab_filters_node>.
      mr_relat_node = lr_join_tab_filters_node.
...
*.. clear all changes flag of the join
  ...
  METHOD fill_value_item_content.
    ...
    DATA(ls_filter) = ir_filter_condition->get_filter( ).

    TRY.

        DATA(lr_value1_item) = ir_node->get_item( c_columns-value ).
        lr_value1_item->set_style( zif_uitb_c_ctm_style=>default ).

        DATA(lr_value2_item) = ir_node->get_item( c_columns-value2 ).
        lr_value2_item->set_style( zif_uitb_c_ctm_style=>default ).

        DATA(lr_comparator2_item) = ir_node->get_item( c_columns-comparator2 ).

        IF ls_filter-operator = zif_dbbr_c_operator=>between.
          lr_comparator2_item->set_text( 'and' ).
        ELSE.
          lr_comparator2_item->set_text( '' ).
        ENDIF.
...
*.......... Convert into display format for better readability
            ...
            lr_value1_item->set_text( |'{ lv_value1 }'| ).
            IF ls_filter-operator = zif_dbbr_c_operator=>between.
              lr_value2_item->set_text( |'{ lv_value2 }'| ).
            ELSE.
              lr_value2_item->set_text( || ).
            ENDIF.
...
*.......... Display the current system value
            ...
            lr_value1_item->set_text(
              |{ ls_filter-value } | &&
              |({ zcl_dbbr_system_helper=>get_disp_val_for_system_va( |{ ls_filter-value }| ) })|
            ).
            lr_value1_item->set_style( zif_uitb_c_ctm_style=>light_yellow ).
  ...
  METHOD on_before_output.
    ...
    er_callback->deactivate_functions(
      VALUE #( ( zif_uitb_template_prog=>c_save ) )
    ).
  ENDMETHOD.
  ...
  METHOD on_delete_request_from_join.
    ...
    TRY .
        mr_join_tree->get_nodes( )->delete_node( ev_node_key ).
      CATCH zcx_uitb_tree_error INTO DATA(lx_tree_error).
        lx_tree_error->print_message(
            iv_msg_type        = 'I'
        ).
...
*.... data loss
  ...
  METHOD on_node_ctx_menu_request.
    ...
    DATA(lr_node) = mr_join_tree->get_nodes( )->get_node( <ls_node_map>-node_key ).
    ...
    CASE <ls_node_map>-node_type.
      ...
      WHEN c_node_type-table_fields.
        ls_flags-add_and = abap_true.
        ls_flags-delete = lr_node->has_children( ).
...
*.. Retrieve information about node from map
    ...
    CASE <ls_node>-node_type.
      ...
      WHEN c_node_type-table_filters OR
        ...
        TRY.
            DATA(lr_node) = mr_join_tree->get_nodes( )->get_node( <ls_node>-node_key ).
            lr_node->toggle( ).
          CATCH zcx_uitb_tree_error.
        ENDTRY.
  ...
  METHOD on_node_keypress.
    ...
    on_toolbar_function( lv_function ).
  ...
  METHOD on_toolbar_function.
    ...
    CASE ev_fcode.
      ...
      WHEN c_functions-expand_all_nodes.
        mr_join_tree->get_nodes( )->expand_root_nodes( ).

      WHEN c_functions-collapse_all_nodes.
        mr_join_tree->get_nodes( )->collapse_all_nodes( ).
    ...
    TRY.
        CASE ev_function_id.
          ...
          WHEN zif_uitb_template_prog=>c_func_f8.
            validate_join( ).
            transfer_join( ).
            mr_template_prog->leave_program( ).

          WHEN zif_uitb_template_prog=>c_func_shift_f8.
            test_join( ).

          WHEN zif_uitb_template_prog=>c_func_f9.
            zcl_dbbr_help_repository=>show_help( zcl_dbbr_help_repository=>c_help_id-join_manager ).

          WHEN OTHERS.
*...... Try Toolbar of tree for same function code
  ...
  METHOD test_join.
    DATA(ls_join_def) = mr_join->to_structure( mr_join_tree ).
    ...
    NEW zcl_uitb_popup_editor(
        iv_text         = lv_string
        iv_editor_title = 'FROM SQL Clause for Join definition'
    )->zif_uitb_view~show( ).
  ENDMETHOD.


  METHOD transfer_join.
    mr_s_join_def->* = mr_join->to_structure( mr_join_tree ).
  ...
  METHOD update_or_group_of_filter.
    ...
    DATA(lr_filter) = CAST lcl_join_filter( ir_filter_node->get_user_object( ) ).

    IF iv_node_type = c_node_type-table_filter.
      lr_filter->set_or_group_node( '' ).
    ELSE.
*.... get the parent of the node to update the or group node key of the filter
      lr_filter->set_or_group_node(
        COND #( WHEN iv_or_group_node IS NOT INITIAL THEN
                  iv_or_group_node
                ELSE
                  ir_filter_node->get_parent( )->mv_node_key )
      ).
    ENDIF.
  ENDMETHOD.
