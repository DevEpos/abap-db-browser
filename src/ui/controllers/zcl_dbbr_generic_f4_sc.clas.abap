...
*&---------------------------------------------------------------------*
    ...
    IF ir_selfield_ref->has_cust_f4_help = abap_true.
      ...
      zcl_dbbr_custom_f4_factory=>delete_f4_assignments( iv_tabname   = ir_selfield_ref->tabname
...
*&---------------------------------------------------------------------*
  ...
  METHOD zif_uitb_screen_controller~set_status.
    ...
    IF mv_display_mode = zif_dbbr_c_global=>c_display_modes-view.
    ...
    DATA(lv_title) = SWITCH string(
      mv_display_mode
      WHEN zif_dbbr_c_global=>c_display_modes-view   THEN |{ 'Display' }|
      WHEN zif_dbbr_c_global=>c_display_modes-create THEN |{ 'Create' }|
      WHEN zif_dbbr_c_global=>c_display_modes-edit   THEN |{ 'Edit' }| ).
