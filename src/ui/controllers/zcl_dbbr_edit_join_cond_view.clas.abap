"! <p class="shorttext synchronized">Controlling for editing a single join table</p>
CLASS zcl_dbbr_edit_join_cond_view DEFINITION
  ...
  PUBLIC SECTION.
    ...
    ALIASES was_not_cancelled FOR zif_uitb_screen_controller~was_not_cancelled.

    CONSTANTS c_field_mode TYPE i VALUE 1 ##NO_TEXT.
    CONSTANTS c_value_mode TYPE i VALUE 2 ##NO_TEXT.

    "! <p class="shorttext synchronized">Created new filter condition</p>
    EVENTS created_filter_condition
      EXPORTING
        VALUE(es_filter) TYPE zdbbr_joinfil.
    "! <p class="shorttext synchronized">Created new field condition</p>
    ...
    "! <p class="shorttext synchronized">Value Help for Source Field</p>
    METHODS call_source_field_f4.
    "! <p class="shorttext synchronized">Value Helpr for Target Field</p>
    METHODS call_target_field_f4.
    "! <p class="shorttext synchronized">Value Help for Value1 field</p>
    METHODS call_value1_f4.
    "! <p class="shorttext synchronized">Value Help for Value2 field</p>
    ...
    "! <p class="shorttext synchronized">Gets the updated condition</p>
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Data element (semantic domain)</p>
    ...
    "! <p class="shorttext synchronized">Join Field Condition</p>
    DATA ms_field_condition TYPE zdbbr_joinfld.
    "! <p class="shorttext synchronized">Join Filter Condition</p>
    ...
    "! <p class="shorttext synchronized">Validate Parameter input</p>
    METHODS validate_parameter.

    "! <p class="shorttext synchronized">Call Built-In Value help for Source Field</p>
    ...
    "! <p class="shorttext synchronized">Clear field attribute fields</p>
    METHODS clear_field_attributes.
    "! <p class="shorttext synchronized">Convert values to internal format</p>
    METHODS convert_values_to_internal.
    "! <p class="shorttext synchronized">Conver values to display format</p>
    METHODS convert_values_to_display.
    "! <p class="shorttext synchronized">Fills list options for comparator field</p>
    METHODS fill_comparator_list.

    "! <p class="shorttext synchronized">Fill attribute fields for source/target table field</p>
    ...
    "! <p class="shorttext synchronized">Sends new condition via event and initializes screen values</p>
    METHODS send_new_condition_via_event.
    "! <p class="shorttext synchronized">Set function icons/texts</p>
    METHODS set_functions.
    "! <p class="shorttext synchronized">Transfer Values before screen is left</p>
    METHODS transfer_values.
    "! <p class="shorttext synchronized">Transfer values to screen if entry is to be changed</p>
    METHODS transfer_values_to_screen.
    "! <p class="shorttext synchronized">Validate screen parameters</p>
    METHODS validate.
    "! <p class="shorttext synchronized">Validation of entered system field</p>
...
CLASS zcl_dbbr_edit_join_cond_view IMPLEMENTATION.
  ...
  METHOD call_built_in_f4_for_value.
    ...
    DATA(lo_screen_field_manager) = NEW zcl_uitb_screen_field_manager( iv_repid = zif_dbbr_c_report_id=>main ).

    lo_screen_field_manager->read_values( IMPORTING et_field_values = DATA(lt_field_values) ).
    ...
    zcl_dbbr_f4_helper=>call_built_in_f4( EXPORTING iv_tablename     = get_entity_for_alias( lv_tabname )
  ...
  METHOD call_source_field_f4.
    ...
    IF mv_mode = c_field_mode.
      zcl_dbbr_f4_helper=>call_table_field_f4( EXPORTING iv_repid              = zif_dbbr_c_report_id=>main
                                                         iv_tablename          = mv_source_entity
                                                         iv_dynpname_fieldname = |{ c_p_join_src_field }|
                                               CHANGING  cv_fieldname          = mr_join_source_field->* ).
      IF mf_allow_offset = abap_true.
        fill_field_attribute_fields( if_source    = abap_true
    ...
    ELSE.
      ...
      DATA(lr_join_field_f4) = NEW zcl_dbbr_tabfield_tree_f4( iv_screen_title     = 'Field F4 for Source'
                                                              io_tree_node_filler = NEW zcl_dbbr_table_treeno_fill(
                                                                  it_join_tables = mt_target_entity_list ) ).
      lr_join_field_f4->set_preferred_entity( iv_entity_alias    = ms_source_entity-add_table_alias
                                              if_collapse_others = abap_true ).
      lr_join_field_f4->display_value_help( IMPORTING ev_chosen_field       = DATA(lv_field)
                                                      ev_chosen_table       = DATA(lv_table)
                                                      ev_chosen_table_alias = DATA(lv_table_alias) ).

      IF     lv_field IS NOT INITIAL
         AND lv_table IS NOT INITIAL.
        ...
        lr_screen_field_manager->set_field_value( iv_name             = |{ c_p_join_src_tab }|
  ...
  METHOD call_target_field_f4.
    ...
    DATA lv_source_field TYPE fieldname.
    ...
    IF     lv_source_field IS NOT INITIAL
       AND lv_source_tab   IS NOT INITIAL.
    ...
    DATA(lr_join_field_f4) = NEW zcl_dbbr_tabfield_tree_f4( iv_screen_title     = 'Field F4 for Target'
                                                            io_tree_node_filler = NEW zcl_dbbr_table_treeno_fill(
                                                                it_join_tables     = mt_target_entity_list
    ...
    IF     lv_field IS NOT INITIAL
      ...
      " .... update target field via screen field manager
      lo_screen_field_manager->set_field_value( iv_name  = |{ c_p_join_trgt_tab }|
                                                iv_value = lv_alias ).
      mr_join_target_tab->* = lv_alias.

      IF mf_allow_offset = abap_true.
        fill_field_attribute_fields( iv_tabname              = |{ lv_table }|
  ...
  METHOD call_value1_f4.
    IF mr_join_cond_val_type->* = zif_sat_c_join_cond_val_type=>typed_input.
      call_built_in_f4_for_value( EXPORTING iv_dynp_fieldname = |{ c_p_join_cond_value1 }|
  ...
  METHOD call_value2_f4.
    IF mr_join_cond_val_type->* = zif_sat_c_join_cond_val_type=>typed_input.
      call_built_in_f4_for_value( EXPORTING iv_dynp_fieldname = |{ c_p_join_cond_value2 }|
  ...
  METHOD convert_values_to_display.
    ...
    TRY.
        IF    mr_join_cond_value1->* IS NOT INITIAL
           OR mr_join_cond_value2->* IS NOT INITIAL.

          zcl_sat_data_converter=>convert_selopt_to_disp_format(
            EXPORTING iv_tabname   = get_entity_for_alias( mr_join_source_tab->* )
                      iv_fieldname = mr_join_source_field->*
            CHANGING  cv_value1    = mr_join_cond_value1->*
                      cv_value2    = mr_join_cond_value2->* ).
        ENDIF.
      CATCH zcx_sat_conversion_exc INTO DATA(lx_conv_error).
        lx_conv_error->print_message( iv_msg_type  = 'E'
                                      if_to_screen = abap_false ).
        zcx_sat_validation_exception=>raise_from_sy( iv_parameter = c_p_join_cond_value1 ).
    ENDTRY.
  ENDMETHOD.

  METHOD convert_values_to_internal.
    CHECK mv_mode = c_value_mode.
    CHECK    mr_join_cond_val_type->* = zif_sat_c_join_cond_val_type=>typed_input
          OR mr_join_cond_val_type->* = zif_sat_c_join_cond_val_type=>system_value_input.
    ...
    IF mr_join_cond_val_type->* = zif_sat_c_join_cond_val_type=>typed_input.
      ...
      TRY.
          ...
          ENDIF.
        CATCH zcx_sat_conversion_exc INTO DATA(lx_conv_error).
          lx_conv_error->print_message( iv_msg_type  = 'S'
                                        if_to_screen = abap_false ).
          zcx_sat_validation_exception=>raise_from_sy( iv_parameter = c_p_join_cond_value1 ).
      ENDTRY.
...
*          ENDIF.
        ...
        WHEN zif_sat_c_join_cond_val_type=>system_value_input.
          ...
          " ........ current operator value has to be updated as well
          IF     mr_join_cond_comp1->* <> zif_sat_c_operator=>equals
             AND mr_join_cond_comp1->* <> zif_sat_c_operator=>not_equals.
    ...
    CALL FUNCTION 'VRM_SET_VALUES'
      EXPORTING id     = CONV vrm_id( c_p_join_cond_comparator1 )
                values = lt_list.
  ENDMETHOD.

  METHOD fill_field_attribute_fields.
    DATA lv_fieldname TYPE fieldname.
    DATA lv_length_fieldname TYPE dynfnam.
    DATA lv_datatype_fieldname TYPE dynfnam.
    DATA lv_tabname TYPE tabname.

    FIELD-SYMBOLS <lv_length> TYPE ddleng.
    FIELD-SYMBOLS <lv_datatype> TYPE datatype_d.
    ...
    DATA(lr_screen_field_manager) = COND #( WHEN io_screen_field_manager IS BOUND
                                            THEN io_screen_field_manager
                                            ELSE NEW zcl_uitb_screen_field_manager( get_report_id( ) ) ).
    ...
    ELSE.
      lr_screen_field_manager->get_value( EXPORTING iv_fieldname = COND #( WHEN if_source = abap_true
                                                                           THEN c_p_join_src_field
                                                                           ELSE c_p_join_trgt_field )
                                          IMPORTING ev_value     = lv_fieldname ).
      lr_screen_field_manager->get_value( EXPORTING iv_fieldname = COND #( WHEN if_source = abap_true
                                                                           THEN c_p_join_src_tab
                                                                           ELSE c_p_join_trgt_tab )
    ...
    ELSE.
      ...
      DATA(ls_field_info) = zcl_sat_ddic_repo_access=>get_table_field_info( iv_tablename = lv_tabname
                                                                            iv_fieldname = lv_fieldname ).
    ...
    lr_screen_field_manager->set_field_value( iv_name  = lv_datatype_fieldname
                                              iv_value = <lv_datatype> ).
    lr_screen_field_manager->set_field_value( iv_name  = lv_length_fieldname
                                              iv_value = <lv_length> ).
  ...
  METHOD transfer_values.
    IF mv_mode = c_field_mode.
      ms_field_condition = VALUE zdbbr_joinfld( field           = mr_join_source_field->*
    ...
    ELSE.
      ms_filter_condition = VALUE zdbbr_joinfil( tabname       = get_entity_for_alias( mr_join_source_tab->* )
  ...
  METHOD validate.
    ...
    CASE mv_mode.
      ...
      WHEN c_field_mode.
        ...
        DATA(ls_source) = zcl_dbbr_ddic_util=>validate_table_field( iv_table_field = mr_join_source_field->*
        ...
        IF mr_join_target_field->* IS INITIAL.
          zcx_sat_validation_exception=>raise_with_text( iv_text      = |Target Field must be filled|
                                                         iv_parameter = c_p_join_trgt_field ).
        ENDIF.
        ...
        DATA(ls_target) = zcl_dbbr_ddic_util=>validate_table_field( iv_table_field = mr_join_target_field->*
      ...
      WHEN c_value_mode.
        ...
        " ...... Validate that the source field exists in the current join table
        zcl_dbbr_ddic_util=>validate_table_field( iv_table_field = mr_join_source_field->*
  ...
  METHOD validate_system_field.
    ...
    DATA lt_values TYPE STANDARD TABLE OF lty_values.
    ...
    lt_values = VALUE #( ( value     = mr_join_cond_value1->*
    ...
    LOOP AT lt_values ASSIGNING FIELD-SYMBOL(<ls_value>).
      ...
      IF <ls_value>-value IS INITIAL.
        zcx_sat_validation_exception=>raise_with_text( iv_text      = |No valid system field was supplied|
                                                       iv_parameter = <ls_value>-fieldname ).
      ELSEIF <ls_value>-value NP 'SY-*'.
        " ...... Only allow fields from SY-structure
        zcx_sat_validation_exception=>raise_with_text( iv_text      = |Only Fields from SY - structure are allowed here|
                                                       iv_parameter = <ls_value>-fieldname ).
      ELSE.
  ...
  METHOD zif_uitb_screen_controller~call_screen.
    ...
    zcl_uitb_screen_util=>call_screen( iv_screen_id    = get_screen_id( )
                                       ...
                                       it_object_map   = VALUE #( ( variable_name = c_r_edit_join_cond_view
                                                                    global_ref    = me ) )
                                       iv_start_column = 10
                                       iv_start_line   = 2 ).
  ENDMETHOD.
  ...
  METHOD zif_uitb_screen_controller~handle_user_command.
    ...
    TRY.
        ...
        IF     lv_function <> c_functions-comparator1_changed
           AND lv_function <> c_functions-value_type_changed.
          validate( ).
        ENDIF.
  ...
  METHOD zif_uitb_screen_controller~pbo.
    ...
    DATA lv_value2_active TYPE screen-active VALUE 0.
    DATA lv_valtype_required TYPE screen-required.
    DATA lv_target_field_active TYPE screen-active.
    ...
    LOOP AT SCREEN.
      IF    screen-name = c_p_join_trgt_fld_datatype
         OR screen-name = c_p_join_source_fld_datatype
         OR screen-name = c_p_join_trgt_fld_length
         OR screen-name = c_p_join_source_fld_length.
        screen-input      = 0.
        screen-value_help = 0.
        MODIFY SCREEN.
      ENDIF.

      IF    screen-name = c_p_join_trgt_tab
         OR screen-name = c_p_join_src_tab.
        IF     screen-name = c_p_join_src_tab
           AND mv_mode     = c_field_mode.
          screen-input = 0.
        ENDIF.
        screen-value_help = 0.
        MODIFY SCREEN.
      ...
      IF    screen-name = c_p_join_src_field
         OR screen-name = c_p_join_trgt_field.
        screen-required = '2'.
        MODIFY SCREEN.
      ENDIF.

      IF    screen-group1 = 'VAL'
         OR screen-group1 = 'VL2'.
  ...
  METHOD zif_uitb_screen_controller~set_status.
    set_functions( ).
    zcl_dbbr_screen_helper=>set_selscreen_status( iv_status              = 'EDIT_DIALOG_STATUS'
                                                  iv_repid               = zif_dbbr_c_report_id=>main
                                                  it_excluding_functions = COND #(
                                                    WHEN mf_is_new = abap_false
                                                    THEN VALUE #( ( c_functions-save_and_continue ) ) ) ).
  ENDMETHOD.
