"! <p class="shorttext synchronized">Mangaer for Object Browser search favorites</p>
CLASS zcl_dbbr_ob_fav_manager DEFINITION
  PUBLIC
  INHERITING FROM zcl_uitb_gui_modal_dialog FINAL
  CREATE PUBLIC.
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Creates favorite tree</p>
    METHODS create_tree
      IMPORTING
        io_parent TYPE REF TO cl_gui_container.

    "! <p class="shorttext synchronized">Change name of favorite</p>
    METHODS edit_favorite_name.
    "! <p class="shorttext synchronized">Delete favorite</p>
    METHODS delete_favorite.

    "! <p class="shorttext synchronized">Event handler for node double click</p>
...
CLASS zcl_dbbr_ob_fav_manager IMPLEMENTATION.
  ...
  METHOD create_content.
    create_control_toolbar( EXPORTING io_parent  = io_container
                                      it_button  = VALUE #( ( function  = c_functions-edit_name
                                                              icon      = icon_change
                                                              text      = |{ 'Change'(007) }| )
  ...
  METHOD create_tree.
    ...
    mo_tree = NEW zcl_uitb_column_tree_model( ir_parent           = io_parent
                                              is_hierarchy_header = VALUE #( heading = |{ 'Object Type/Query'(003) }|
                                                                             width   = 100 )
                                              if_auto_node_key    = abap_true ).

    mo_tree->get_columns( )->add_column( iv_colname     = c_cols-description
                                         iv_width       = 50
    ...
    lt_fav_groups = VALUE #( ( type = zif_dbbr_c_object_browser_mode=>cds_view       name = 'CDS view' )
    ...
    LOOP AT lt_fav_groups ASSIGNING FIELD-SYMBOL(<ls_fav_group>).
      DATA(lo_type_node) = lo_nodes->add_node( if_folder        = abap_true
      ...
      LOOP AT lt_favorites ASSIGNING FIELD-SYMBOL(<ls_fav>) WHERE entity_type = <ls_fav_group>-type.
        lo_nodes->add_node( iv_relative_node_key = lo_type_node->mv_node_key
  ...
  METHOD edit_favorite_name.
    ...
    IF lv_result <> lr_user_data->favorite_name.
      ...
      IF zcl_dbbr_ob_fav_factory=>change_favorite_name( iv_id   = lr_user_data->id
  ...
  METHOD delete_favorite.
    DATA(lo_selected_node) = mo_tree->get_selections( )->get_selected_node( ).
    IF lo_selected_node IS NOT BOUND.
      RETURN.
    ENDIF.
    ...
    IF zcl_dbbr_appl_util=>popup_to_confirm(
      ...
      IF NOT zcl_dbbr_ob_fav_factory=>delete_favorite( lr_user_data->id ).
        RETURN.
      ENDIF.
