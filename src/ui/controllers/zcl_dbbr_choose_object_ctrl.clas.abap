"! <p class="shorttext synchronized">Controller for choosing a table/query</p>
...
CLASS zcl_dbbr_choose_object_ctrl IMPLEMENTATION.
  ...
  METHOD validate_user_input.
    ...
    IF iv_function_code = zif_dbbr_c_global=>c_function_codes-cancel.
      RETURN.
    ENDIF.
    ...
    IF mr_s_entity->table_mode = abap_true.
      ...
      zcl_dbbr_ddic_util=>validate_table_name( iv_table_name               = CONV #( mr_s_entity->tab_name )
    ...
    ELSEIF mr_s_entity->query_mode = abap_true. " query mode is active
      ...
      DATA(ls_query) = lr_query_f->get_query( iv_query_name      = mr_s_entity->query_name
                                              if_load_completely = abap_false ).

      IF ls_query IS INITIAL.
        MESSAGE e035(zdbbr_info) WITH mr_s_entity->query_name INTO DATA(lv_msg) ##NEEDED.
        zcx_sat_validation_exception=>raise_from_sy( ).
      ELSEIF     ls_query-is_global = abap_false
             AND mf_global_fav_mode = abap_true
             AND mf_for_fav_menu    = abap_true.
  ...
  METHOD zif_uitb_screen_controller~pbo.
    ...
    DATA lv_tab_name_input TYPE screen-input.
    DATA lv_query_name_input TYPE screen-input.
