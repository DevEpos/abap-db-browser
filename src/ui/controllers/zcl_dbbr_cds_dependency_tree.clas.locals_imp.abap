...
CLASS lcl_usage_alv IMPLEMENTATION.
  ...
  METHOD on_double_click.
    ...
    mo_dependency_tree->execute_command( NEW zcl_uitb_gui_simple_command( iv_function = c_functions-open_with_adt
                                                                          ir_params   = NEW lty_s_command_info(
  ...
  METHOD create_and_fill_usage_alv.
    mo_alv = zcl_uitb_alv=>create_alv( ir_data      = REF #( mt_usages )
                                       ir_container = io_container ).

    DATA(lo_cols) = mo_alv->get_columns( ).
    lo_cols->set_column_position( iv_columnname = 'TYPE_ICON'
                                  iv_position   = 1 ).
    lo_cols->set_column_position( iv_columnname = 'RAW_NAME'
                                  iv_position   = 2 ).
    lo_cols->set_column_position( iv_columnname = 'DESCRIPTION'
                                  iv_position   = 3 ).
    lo_cols->set_column_position( iv_columnname = 'OCCURRENCES'
                                  iv_position   = 4 ).
    lo_cols->set_column_position( iv_columnname = 'USED_ENTITIES'
                                  iv_position   = 5 ).
    lo_cols->set_column_position( iv_columnname = 'JOIN_COUNT'
                                  iv_position   = 6 ).
    lo_cols->set_column_position( iv_columnname = 'SET_OPERATION_COUNT'
                                  iv_position   = 7 ).
    ...
    WHILE lo_col_iterator->has_next( ).
      ...
      CASE lo_col->get_name( ).
        ...
        WHEN 'OCCURRENCES'.
          lo_col->set_descriptions( iv_long    = 'Occurrence'
                                    iv_tooltip = 'Times the entity occurrs in the depend.' ).
          lo_col->set_optimized( ).

        WHEN 'USED_ENTITIES'.
          lo_col->set_descriptions( iv_long    = 'Used Entities'
                                    iv_tooltip = 'Number of used Entities' ).
          lo_col->set_optimized( ).

        WHEN 'JOIN_COUNT'.
          lo_col->set_descriptions( iv_long    = 'Used Joins'
                                    iv_tooltip = 'Number of Joins in Entity' ).
          lo_col->set_optimized( ).

        WHEN 'SET_OPERATION_COUNT'.
          lo_col->set_descriptions( iv_long    = 'Used Set Operations'
                                    iv_tooltip = 'Number of Set Operations in Entity' ).
          lo_col->set_optimized( ).
    ...
    lo_functions->add_function( iv_name    = c_functions-show_ddl_source
                                iv_icon    = |{ icon_abap }|
                                iv_text    = |{ TEXT-045 }|
                                iv_tooltip = |{ TEXT-044 }| ).
    lo_functions->add_function( iv_type = zcl_uitb_alv_functions=>separator ).
    lo_functions->add_function( iv_name    = c_functions-exec_with_dbbrs
                                iv_icon    = |{ icon_table_settings }|
                                iv_text    = |{ TEXT-047 }|
                                iv_tooltip = |{ TEXT-046 }| ).
    lo_functions->add_function( iv_name    = c_functions-exec_with_dbbrs_new_window
                                iv_icon    = |{ icon_table_settings }|
                                iv_text    = |{ TEXT-049 }|
                                iv_tooltip = |{ TEXT-048 }| ).

    lo_functions->add_function( iv_name = c_functions-toggle_tree
    ...
    SET HANDLER on_context_menu FOR lo_events.
