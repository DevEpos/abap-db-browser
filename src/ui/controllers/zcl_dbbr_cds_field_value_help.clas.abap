"! <p class="shorttext synchronized">Value Help for CDS View fields</p>
...
CLASS zcl_dbbr_cds_field_value_help IMPLEMENTATION.
  ...
  METHOD create_content.
    ...
    mo_main_splitter = NEW zcl_uitb_gui_splitter_cont( iv_elements = 2
                                                       iv_size     = '30:70'
                                                       io_parent   = lo_container ).

    DATA(lo_filter_container) = mo_main_splitter->get_container( 1 ).
    mo_filter_alv = NEW zcl_dbbr_f4_alv_filter( io_parent   = lo_filter_container
                                                it_selfield = VALUE #( ) ).
    " TODO: variable is assigned but never used (ABAP cleaner)
