"! <p class="shorttext synchronized">Controlller zum Speichern/Lesen/L̦schen von Varianten</p>
CLASS zcl_dbbr_variant_controller DEFINITION
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Mode for Selection Screen of DB Browser</p>
    ...
    "! <p class="shorttext synchronized">Delete variant</p>
    METHODS delete_variant.

    "! <p class="shorttext synchronized">Create new variant</p>
    METHODS create_variant.

    "! <p class="shorttext synchronized">Creates/updates default variant for given entity</p>
    METHODS create_default_variant.

    "! <p class="shorttext synchronized">Returns 'X' if there are selection criteria</p>
    METHODS exists_criteria
      RETURNING
        VALUE(rf_exists) TYPE abap_bool.

    "! <p class="shorttext synchronized">Persists the variant from the current data</p>
...
*&---------------------------------------------------------------------*
    ...
    zcl_dbbr_variant_factory=>find_variants( EXPORTING iv_variant_name = ms_ui_refs-variant_name->*
                                                       iv_entity_id    = mv_entity_id
                                                       iv_entity_type  = mv_selscreen_mode
                                             IMPORTING et_variant_info = DATA(lt_variant_info) ).

    lt_variants = CORRESPONDING #( lt_variant_info ).

    DATA(lv_return) = zcl_dbbr_f4_helper=>call_int_table_f4( it_table_search      = lt_variants
  ...
  METHOD constructor.
    ...
    ms_ui_refs-variant_name            = CAST zdbbr_variant_name( lr_data_cache->get_data_ref(
                                                                      zif_dbbr_main_report_var_ids=>c_p_varnam ) ).
    ms_ui_refs-variant_text            = CAST ddtext( lr_data_cache->get_data_ref(
                                                          zif_dbbr_main_report_var_ids=>c_p_vartxt ) ).
    ms_ui_refs-xsort_fields            = CAST boolean( lr_data_cache->get_data_ref(
                                                           zif_dbbr_main_report_var_ids=>c_p_xsort ) ).
    ms_ui_refs-xcolumn_widths          = CAST abap_bool( lr_data_cache->get_data_ref(
                                                             zif_dbbr_main_report_var_ids=>c_p_has_defined_column_widths ) ).
    ms_ui_refs-xfilter_fields          = CAST #( lr_data_cache->get_data_ref(
                                                     zif_dbbr_main_report_var_ids=>c_p_has_selection_criteria ) ).
    ms_ui_refs-xoutput_fields          = CAST boolean( lr_data_cache->get_data_ref(
                                                           zif_dbbr_main_report_var_ids=>c_p_xfield ) ).
    ms_ui_refs-variant_table_name      = CAST #( lr_data_cache->get_data_ref( zif_dbbr_main_report_var_ids=>c_p_tab ) ).
    ms_ui_refs-variant_for_table_title = CAST syst_title( lr_data_cache->get_data_ref(
                                                              zif_dbbr_main_report_var_ids=>c_tvar_t ) ).
    ms_ui_refs-variant_for_query_title = CAST syst_title( lr_data_cache->get_data_ref(
                                                              zif_dbbr_main_report_var_ids=>c_svar_t ) ).
    ms_ui_refs-variant_for_cds_title   = CAST syst_title( lr_data_cache->get_data_ref(
                                                              zif_dbbr_main_report_var_ids=>c_t_cds_view_variant ) ).
    ms_ui_refs-variant_query_name      = CAST #( lr_data_cache->get_data_ref( zif_dbbr_main_report_var_ids=>c_p_scrnam ) ).
    ms_ui_refs-variant_cds_name        = CAST #( lr_data_cache->get_data_ref(
                                                     zif_dbbr_main_report_var_ids=>c_p_cds_view_name ) ).
  ENDMETHOD.
  ...
  METHOD create_variant.
    IF zcl_dbbr_variant_factory=>variant_exists( iv_variant_name = ms_ui_refs-variant_name->*
      ...
      zcl_dbbr_variant_factory=>get_variant( EXPORTING iv_variant_name    = ms_ui_refs-variant_name->*
                                                       iv_entity_id       = mv_entity_id
                                                       iv_entity_type     = mv_selscreen_mode
                                                       if_load_completely = abap_false
                                             IMPORTING es_variant         = DATA(ls_existing_variant) ).
    ENDIF.
  ...
  METHOD persist_variant.
    DATA(ls_variant) = zcl_dbbr_variant_creator=>create_variant(
        ...
        it_selfields           = COND #( WHEN ms_ui_refs-xfilter_fields->* = abap_true AND mr_t_selfields IS BOUND
                                         THEN mr_t_selfields->* )
        it_multi_selfields     = COND #( WHEN ms_ui_refs-xfilter_fields->* = abap_true AND mr_t_selfields_multi IS BOUND
                                         THEN mr_t_selfields_multi->* )
        it_multi_or            = COND #( WHEN ms_ui_refs-xfilter_fields->* = abap_true AND mr_multi_or_itab IS BOUND
                                         THEN mr_multi_or_itab->* )
        is_global_data         = mr_s_global_data->* ).

    " .. create user/ddic sort order for output / sorting fields
    IF    ms_ui_refs-xoutput_fields->* = abap_true
       OR ms_ui_refs-xsort_fields->*   = abap_true.
      ...
      DATA(lt_fields_ui) = lr_tabfields->get_active_fields( if_consider_output   = ms_ui_refs-xoutput_fields->*
  ...
  METHOD exists_criteria.
    ...
    IF mr_t_selfields IS NOT BOUND.
      RETURN.
    ENDIF.
  ...
  METHOD zif_uitb_screen_controller~call_screen.
    IF     mv_mode = c_modes-save
       AND ms_ui_refs-variant_name->* IS INITIAL.
  ...
  METHOD zif_uitb_screen_controller~pbo.
    ...
    LOOP AT SCREEN.
      ...
      IF screen-group1 = 'ALV' AND mf_alv_output = abap_false.
        ...
        CONTINUE.
      ENDIF.
      IF    screen-name = zif_dbbr_main_report_var_ids=>c_p_tab
         OR screen-name = zif_dbbr_main_report_var_ids=>c_p_scrnam
         OR screen-name = zif_dbbr_main_report_var_ids=>c_p_cds_view_name.
        screen-input = 0.
        MODIFY SCREEN.
      ELSEIF screen-group1 = 'GLO'.
        screen-active = 0.
        MODIFY SCREEN.
      ELSEIF     (    screen-name   = zif_dbbr_main_report_var_ids=>c_p_vartxt
                   OR screen-group1 = 'TXT' )
             AND mv_mode <> c_modes-save.
        screen-active = 0.
        MODIFY SCREEN.
  ...
  METHOD zif_uitb_screen_controller~set_status.
    zcl_uitb_screen_util=>set_selscreen_status( iv_status = '0600'
    ...
    DATA(lv_title) = SWITCH #(
      ...
      WHEN abap_false THEN SWITCH string(
          mv_mode
          WHEN c_modes-save   THEN 'Save variant'(v02)
          WHEN c_modes-delete THEN 'Delete variant'(v03) )
      WHEN abap_true  THEN SWITCH string(
           mv_mode
           WHEN c_modes-save
           THEN 'Save Default Variant'(017) ) ).
