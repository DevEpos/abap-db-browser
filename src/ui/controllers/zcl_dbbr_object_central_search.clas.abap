CLASS zcl_dbbr_object_central_search DEFINITION
  PUBLIC
  INHERITING FROM zcl_uitb_gui_screen FINAL
  CREATE PUBLIC.
  ...
  PRIVATE SECTION.
    ...
    DATA mr_current_selected TYPE REF TO zcl_dbbr_object_central_search=>ty_s_result.

    METHODS show_settings.

    "! <p class="shorttext synchronized">Create dynamic document for search input</p>
    "!
    METHODS create_dd_input
      IMPORTING
        io_container TYPE REF TO cl_gui_container.

    "! <p class="shorttext synchronized">Create ALV List output</p>
    "!
    METHODS create_alv_output
      IMPORTING
        io_container TYPE REF TO cl_gui_container.

    "! <p class="shorttext synchronized">Trigger new entity search</p>
    "!
    METHODS trigger_new_search.
    "! <p class="shorttext synchronized">Fill alv toolbar</p>
    "!
    METHODS fill_toolbar.

    "! <p class="shorttext synchronized">Retrieve currently selected row in ALV</p>
    "!
    ...
    "! <p class="shorttext synchronized">Open selected result in various ways specified by action</p>
    "!
    METHODS open_in
      IMPORTING
        iv_action TYPE zdbbr_entity_browser_link_mode OPTIONAL.

    "! <p class="shorttext synchronized">Show content of currently seleted result entry</p>
    "!
    ...
    "! <p class="shorttext synchronized">User command handerl for ALV action</p>
    "!
    ...
    "! <p class="shorttext synchronized">Context menu handler for ALV</p>
    "!
    ...
    "! <p class="shorttext synchronized">Link Click handler for ALV</p>
    "!
...
CLASS zcl_dbbr_object_central_search IMPLEMENTATION.
  METHOD constructor.
    " TODO: parameter IV_INITIAL_SEARCH_VALUE is never used (ABAP cleaner)
  ...
  METHOD create_content.
    DATA(lo_splitter) = NEW zcl_uitb_gui_splitter_cont( iv_elements = 2
                                                        iv_size     = '55:*'
                                                        io_parent   = io_container
      )->set_all_sash_properties( if_visible = abap_false ).

    create_dd_input( io_container = lo_splitter->get_container( 1 ) ).
    create_alv_output( io_container = lo_splitter->get_container( 2 ) ).
  ENDMETHOD.

  METHOD do_before_dynpro_output.
    io_callback->deactivate_function( zif_uitb_c_gui_screen=>c_functions-save ).

    io_callback->map_fkey_functions( VALUE #( ( fkey            = zif_uitb_c_gui_screen=>c_functions-f7
                                                mapped_function = c_functions-db_browser_content
                                              ...
                                              ( fkey            = zif_uitb_c_gui_screen=>c_functions-f9
                                                ...
                                                text            = |{ 'Customize Search Settings'(014) }| ) ) ).
  ENDMETHOD.
  ...
  METHOD navigate_on_chosen_result.
    DATA(lv_mode) = COND zdbbr_entity_browser_link_mode( WHEN iv_action IS NOT INITIAL
                                                         THEN iv_action
                                                         ELSE ms_settings-link_mode ).

    IF     iv_entity_type = zif_sat_c_entity_type=>query
       AND lv_mode        = zif_dbbr_c_eb_link_mode=>open_with_adt.
    ...
    IF mf_start_transaction_mode = abap_true OR lv_mode = zif_dbbr_c_eb_link_mode=>open_with_adt.
      ...
      CASE lv_mode.
        ...
        WHEN zif_dbbr_c_eb_link_mode=>open_with_adt.
          ...
          TRY.
              zcl_sat_adt_util=>jump_adt( iv_obj_name = CONV #( to_upper( iv_entity_id ) )
        ...
        WHEN zif_dbbr_c_eb_link_mode=>show_content.
          show_content( iv_entity_type = iv_entity_type
                        iv_entity_id   = iv_entity_id
  ...
  METHOD create_dd_input.
    ...
    lo_form->add_select_element(
      EXPORTING tooltip        = |{ 'Object Category'(002) }|
                options        = VALUE #(
                    ( value = zif_dbbr_c_object_browser_mode=>cds_view       text = 'CDS View'(003) )
                    ( value = zif_dbbr_c_object_browser_mode=>database_table text = 'Database Table/View'(004) )
                    ( value = zif_dbbr_c_object_browser_mode=>database_view  text = 'Database View'(018) )
    ...
    CASE mv_search_entity_type.
      ...
      WHEN zif_sat_c_entity_type=>table.
        mo_search_type_select->set_value( |{ zif_dbbr_c_object_browser_mode=>database_table }| ).

      WHEN zif_sat_c_entity_type=>view.
        mo_search_type_select->set_value( |{ zif_dbbr_c_object_browser_mode=>database_view }| ).
    ...
    lo_form->line_with_layout( start            = abap_true
                               no_leading_break = abap_true ).
    lo_form->add_input_element( EXPORTING size          = 100
  ...
  METHOD create_alv_output.
    ...
    mo_alv = zcl_uitb_alv=>create_alv( ir_data      = REF #( mt_result )
                                       ir_container = io_container
                                       if_editable  = abap_false ).

    mo_alv->get_selections( )->set_mode( value = zif_uitb_c_alv_selection=>cell ).

    SET HANDLER on_user_command FOR mo_alv->get_events( ).
    SET HANDLER on_link_click FOR mo_alv->get_events( ).
    SET HANDLER on_alv_context_menu FOR mo_alv->get_events( ).
    ...
    TRY.
        ...
        lo_sorts->add_sort( iv_column_name = 'ENTITY_ID_RAW'
  ...
  METHOD fill_toolbar.
    ...
    lo_functions->set_function( iv_name   = zif_uitb_c_alv_functions=>layout_change
                                if_enable = abap_false ).
    ...
    lo_functions->add_function( iv_name    = c_functions-settings
                                iv_icon    = |{ icon_personal_settings }|
                                iv_tooltip = |{ 'Customize Search Settings'(014) }| ).
    update_search_settings_menu( ).
    lo_functions->add_function( iv_name    = c_functions-search_settings
    ...
    lo_functions->add_function( iv_name    = c_functions-show_help
  ...
  METHOD trigger_new_search.
    ...
    IF mo_search_query->mv_max_rows IS INITIAL.
      mo_search_query->set_max_rows( CONV #( ms_settings-max_hits ) ).
    ENDIF.
    ...
    TRY.
        CASE mv_current_search_type.
          ...
          " New CDS View Search
          WHEN zif_dbbr_c_object_browser_mode=>cds_view OR
               zif_dbbr_c_object_browser_mode=>database_view OR
               zif_dbbr_c_object_browser_mode=>database_table OR
  ...
  METHOD on_alv_context_menu.
    ...
    IF mr_current_selected->entity_type = zif_sat_c_entity_type=>cds_view.
      er_menu->add_function( fcode = zcl_dbbr_object_central_search=>c_functions-show_cds_source
                             text  = |{ 'Show CDS Source'(008) }| ).
      er_menu->add_separator( ).
    ENDIF.

    er_menu->add_function( fcode = zcl_dbbr_object_central_search=>c_functions-db_browser_content
                           text  = |{ 'Show Content'(012) }| ).
    er_menu->add_function( fcode = zcl_dbbr_object_central_search=>c_functions-db_browser_content_new_task
                           text  = |{ 'Show Content (New Window)'(013) }| ).
    er_menu->add_separator( ).
    er_menu->add_function( fcode = zcl_dbbr_object_central_search=>c_functions-adt
                           text  = |{ 'Open with ADT Tools'(009) }| ).
    er_menu->add_function( fcode = zcl_dbbr_object_central_search=>c_functions-db_browser
                           text  = |{ 'Open with DB Browser'(010) }| ).
    er_menu->add_function( fcode = zcl_dbbr_object_central_search=>c_functions-db_browser_new_task
  ...
  METHOD get_selected_index.
    ...
    IF lt_selected_rows IS INITIAL.
      ...
      lt_selected_rows = VALUE #( FOR cell IN lt_selected_cells
                                  ( cell-row ) ).
  ...
  METHOD open_in.
    ...
    IF lv_selected_row IS INITIAL.
      RETURN.
    ENDIF.

    DATA(lr_selected) = REF #( mt_result[ lv_selected_row ] ).

    navigate_on_chosen_result( iv_entity_id   = lr_selected->entity_id
  ...
  METHOD update_search_settings_menu.
    ...
    mo_search_settings_menu->add_function( fcode   = c_functions-use_and_instead_of_or
