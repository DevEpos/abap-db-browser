"! <p class="shorttext synchronized">Controller for saving a query</p>
CLASS zcl_dbbr_save_query_ctrl DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Returns the name of the saved query</p>
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">List of selection fields</p>
    DATA mr_t_selfields TYPE REF TO zdbbr_selfield_itab.
    "! <p class="shorttext synchronized">List of selection fields</p>
    DATA mr_t_multi_selfields TYPE REF TO zdbbr_selfield_itab.
    "! <p class="shorttext synchronized">List of OR selection tables</p>
...
CLASS zcl_dbbr_save_query_ctrl IMPLEMENTATION.
  ...
  METHOD save_query.
    ...
    DATA(lt_fields_ui) = mr_tabfield_list->get_active_fields( if_consider_selected = abap_true
    ...
    DATA(ls_query_data) = VALUE zdbbr_query_data(
        ...
        created_by          = COND #( WHEN ls_query_existing-created_by IS NOT INITIAL
                                      THEN ls_query_existing-created_by
                                      ELSE sy-uname )
        created_date        = COND #( WHEN ls_query_existing IS NOT INITIAL
                                      THEN ls_query_existing-created_date
                                      ELSE sy-datum )
...
*...... Retrieve stored jump destinations
        jump_fields         = COND #( WHEN ls_query_existing-query_id IS NOT INITIAL
                                      THEN CORRESPONDING #( DEEP
                                              NEW zcl_dbbr_jump_destination_f( )->get_jump_destinations(
                                                      ls_query_existing-query_id ) ) )
  ...
  METHOD zif_uitb_screen_controller~set_status.
    zcl_uitb_screen_util=>set_selscreen_status( iv_status              = '0710'
