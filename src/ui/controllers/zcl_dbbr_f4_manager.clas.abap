...
CLASS zcl_dbbr_f4_manager IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    " register events
    SET HANDLER on_before_output FOR mr_template_prog.
    SET HANDLER on_user_command FOR mr_template_prog.
    SET HANDLER on_exit FOR mr_template_prog.

    mt_edit_functions = VALUE #( ( c_functions-define_built_in )
  ...
  METHOD create_f4_alv.
    ...
    cl_salv_table=>factory( EXPORTING r_container  = mr_template_prog->get_container( )
                            IMPORTING r_salv_table = mr_f4_helps_alv
                            CHANGING  t_table      = mt_f4_overview ).
    ...
    lr_columns->set_column_position( columnname = 'DESCRIPTION'
                                     position   = 1 ).
    lr_columns->set_column_position( columnname = 'SEARCH_TABLE'
                                     position   = 2 ).
    lr_columns->set_column_position( columnname = 'SEARCH_FIELD'
                                     position   = 3 ).
  ...
  METHOD delete_value_helps.
    ...
    DATA(lt_f4_id) = VALUE zif_sat_ty_global=>ty_t_selopt( FOR selected IN lt_selected_rows
  ...
  METHOD do_before_first_screen_call.
    mr_template_prog->add_function( iv_function_id = zif_uitb_template_prog=>c_func_ctrl_f1
                                    iv_text        = 'Display/Edit'
                                    iv_icon        = icon_toggle_display_change ).

    mr_template_prog->add_function( iv_function_id = c_functions-select_all
                                    iv_text        = 'Select All'
                                    iv_icon        = icon_select_all ).
    mr_template_prog->add_function( iv_function_id = c_functions-deselect_all
                                    iv_text        = 'Deselect All'
                                    iv_icon        = icon_deselect_all ).

    mr_template_prog->add_function( iv_function_id = c_functions-sort_ascending
                                    iv_text        = 'Sort Up'
                                    iv_icon        = icon_sort_up ).

    mr_template_prog->add_function( iv_function_id = c_functions-sort_descending
                                    iv_text        = 'Sort Down'
                                    iv_icon        = icon_sort_down ).

    mr_template_prog->add_function( iv_function_id = c_functions-define_built_in
                                    iv_text        = 'Define existing Value Help'
                                    iv_quickinfo   = 'Define existing Value Help'
                                    iv_icon        = icon_create ).

    mr_template_prog->add_function( iv_function_id = c_functions-view_f4
                                    iv_text        = 'Display Value Help'
                                    iv_icon        = icon_display ).

    mr_template_prog->add_function( iv_function_id = c_functions-change_f4
                                    iv_text        = 'Edit Value Help'
                                    iv_icon        = icon_change ).

    mr_template_prog->add_function( iv_function_id = c_functions-delete_f4
                                    iv_text        = 'Delete Value Help'
                                    iv_icon        = icon_delete ).

    mr_template_prog->add_function( iv_function_id = c_functions-export_f4
                                    iv_text        = 'Export Value Helps'
                                    iv_icon        = icon_export ).

    mr_template_prog->add_function( iv_function_id = c_functions-import_f4
  ...
  METHOD export_value_helps.
    ...
    DATA(lr_f4_exporter) = NEW zcl_dbbr_f4_exporter( VALUE #( FOR row IN lt_selected_rows
  ...
  METHOD manage_f4.
    ...
    DATA(lr_f4_screen_controller) = NEW zcl_dbbr_custom_f4_sc( iv_f4_id        = lr_f4->f4_id
...
*      RAISE EXCEPTION TYPE ZCX_DBBR_alv_no_data.
  ...
  METHOD toggle_mode.
    ...
    DATA lv_title_suffix TYPE string.
    DATA lv_locked_by_user TYPE sy-uname.
  ...
  METHOD zif_uitb_view~show.
    ...
    mr_template_prog->zif_uitb_view~show( iv_end_column   = iv_end_column
