"! <p class="shorttext synchronized">Selection Dialog for Database entity</p>
CLASS zcl_dbbr_db_entity_sel_dialog DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Creates dialog for finding and selected db entities</p>
...
CLASS zcl_dbbr_db_entity_sel_dialog IMPLEMENTATION.
  METHOD constructor.
    super->constructor( iv_title         = 'Choose Database entity'
  ...
  METHOD get_db_entities.
    ...
    lo_dialog->show( iv_top    = 4
                     iv_left   = 20
                     iv_width  = 120
                     iv_height = 20 ).

    IF lo_dialog->is_multi_select( ).
      rt_entity = VALUE #( FOR <ls_entity> IN lo_dialog->mt_db_entity
  ...
  METHOD get_filtered_data.
    ...
    LOOP AT lt_filter INTO DATA(lv_filter).
      lt_entity_range = VALUE #( BASE lt_entity_range
                                 ( sign = 'I' option = 'CP' low = |{ to_upper( lv_filter ) }| ) ).
    ENDLOOP.
