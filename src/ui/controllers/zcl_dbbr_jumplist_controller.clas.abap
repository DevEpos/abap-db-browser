...
CLASS zcl_dbbr_jumplist_controller IMPLEMENTATION.
  METHOD add_new_jump_field.
    ...
    lr_new_line->jump_target_type = zif_dbbr_c_global=>c_jump_call_types-normal.
  ...
  METHOD load_query.
    ...
    LOOP AT lt_table_list ASSIGNING FIELD-SYMBOL(<ls_table>).
      ...
      IF mf_join_is_active = abap_true.
        ...
        ELSE.
          <ls_table>-alias = VALUE #( ls_join_def-tables[
                                          add_table_alias = <ls_table>-tabname_alias ]-add_table_alias_alv OPTIONAL ).
        ENDIF.
      ENDIF.

      DATA(lv_tabname_alias) = COND #( WHEN ms_query_info-ref_join_id IS NOT INITIAL
                                       THEN <ls_table>-tabname_alias
                                       ELSE <ls_table>-tabname ).
      ...
      LOOP AT lt_query_selfields ASSIGNING FIELD-SYMBOL(<ls_query_field>) WHERE     tabname_alias    = lv_tabname_alias
        ...
        ls_tabfield-field_ddtext   = COND #( WHEN ls_fieldinfo-scrtext_l IS INITIAL
                                             THEN ls_fieldinfo-fieldtext
                                             ELSE ls_fieldinfo-scrtext_l ).
    ...
    mr_table->set_query_infos( ir_tabfield_list      = mr_query_tabfields
  ...
  METHOD save_jump_fields.
    ...
    NEW zcl_dbbr_jump_destination_f( )->save_jump_destinations(
          EXPORTING iv_query_id          = mv_query_id
                    if_update_query_flag = abap_true
          CHANGING  ct_jump_destinations = <lt_jump_definitions> ).
