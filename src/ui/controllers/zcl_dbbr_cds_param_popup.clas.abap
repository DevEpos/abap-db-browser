"! <p class="shorttext synchronized">Popup for selecting CDS view parameters</p>
CLASS zcl_dbbr_cds_param_popup DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Get entered param values</p>
    "!
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Convert parameter to display format</p>
    "!
    ...
    "! <p class="shorttext synchronized">Convert parameters to internal format</p>
    "!
    METHODS convert_params_to_internal.

    "! <p class="shorttext synchronized">Create ALV for input</p>
    "!
    METHODS create_alv
      IMPORTING
        io_container TYPE REF TO cl_gui_container.

    "! <p class="shorttext synchronized">Handler for data changed event</p>
    "!
    ...
    "! <p class="shorttext synchronized">Handler for F4 event</p>
    "!
    METHODS on_f4
...
CLASS zcl_dbbr_cds_param_popup IMPLEMENTATION.
  METHOD constructor.
    ...
    DATA(lr_iterator) = io_tabfields->zif_uitb_data_ref_list~get_iterator( iv_where = 'IS_PARAMETER = abap_true' ).
    WHILE lr_iterator->has_next( ).
      DATA(lr_s_next_param) = CAST zdbbr_tabfield_info_ui( lr_iterator->get_next( ) ).
      DATA(ls_param) = VALUE ty_param( paramname_raw = lr_s_next_param->fieldname_raw
      ...
      IF ls_param-value IS NOT INITIAL.
        convert_param_to_display( ir_s_param  = REF #( ls_param )
  ...
  METHOD convert_params_to_internal.
    LOOP AT mt_parameters ASSIGNING FIELD-SYMBOL(<ls_param>).
      DATA(lr_s_field) = mo_tabfields->get_field_ref( iv_tabname_alias = zif_dbbr_c_global=>c_parameter_dummy_table
                                                      iv_fieldname     = <ls_param>-name ).

      zcl_sat_data_converter=>convert_values_to_int_format( EXPORTING iv_rollname = lr_s_field->rollname
  ...
  METHOD convert_param_to_display.
    zcl_sat_data_converter=>convert_values_to_disp_format( EXPORTING iv_rollname = is_tabfield-rollname
  ...
  METHOD create_alv.
    ...
    SET HANDLER on_data_changed FOR lr_events.
    SET HANDLER on_f4 FOR lr_events.

    mo_alv->get_selections( )->set_current_cell( VALUE #( row    = 1
                                                          column = 'VALUE' ) ).
    mo_alv->display( ).
  ...
  METHOD create_content.
    create_control_toolbar( EXPORTING io_parent = io_container
                                      it_button = VALUE #( ( function  = c_functions-update_params
                                                             icon      = icon_execute_object
                                                             quickinfo = |{ 'Update parameter values (F8)'(004) }| ) )
                            IMPORTING eo_client = DATA(lo_container) ).
  ...
  METHOD on_data_changed.
    ...
    LOOP AT er_change_protocol->mt_good_cells ASSIGNING FIELD-SYMBOL(<ls_mod_cell>).
      ...
      DATA(ls_param_field) = mo_tabfields->get_field( iv_tabname   = zif_dbbr_c_global=>c_parameter_dummy_table
      ...
      TRY.
          ...
          zcl_sat_data_converter=>convert_values_to_disp_format(
            EXPORTING iv_rollname = ls_param_field-rollname
                      ...
                      iv_type     = ls_param_field-inttype
            CHANGING  cv_value1   = lv_value ).
          er_change_protocol->modify_cell( i_row_id    = <ls_mod_cell>-row_id
                                           i_fieldname = 'VALUE'
                                           i_value     = lv_value ).
        CATCH zcx_sat_conversion_exc INTO DATA(lx_conv_error).
          er_change_protocol->add_protocol_entry( i_msgid     = lx_conv_error->if_t100_message~t100key-msgid
                                                  i_msgty     = 'E'
  ...
  METHOD on_f4.
    ...
    IF lr_s_param->has_custom_f4 = abap_true AND mo_custom_f4_map IS BOUND.
      ...
      zcl_dbbr_f4_helper=>call_f4( EXPORTING io_custom_f4_map = mo_custom_f4_map
      ...
      IF ls_selfield-low IS NOT INITIAL.
        ...
        <lt_modified> = VALUE #( ( fieldname = 'VALUE' row_id = es_row_no-row_id value = ls_selfield-low ) ).
      ENDIF.
    ELSEIF ls_param_field-rollname IS NOT INITIAL.
      DATA lv_f4_result TYPE zsat_value.

      zcl_dbbr_f4_helper=>call_built_in_f4( EXPORTING iv_tablename = ls_param_field-rollname
      ...
      IF lv_f4_result IS NOT INITIAL.
        ...
        <lt_modified> = VALUE #( ( fieldname = 'VALUE' row_id = es_row_no-row_id value = lv_f4_result ) ).
      ENDIF.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
