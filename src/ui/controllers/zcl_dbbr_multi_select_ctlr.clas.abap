...
*&---------------------------------------------------------------------*
    ...
    TRY.
        ...
        IF if_multi = abap_true.
          call_selfield_f4_help_multi( if_low       = if_for_low
...
*&---------------------------------------------------------------------*
    ...
    mo_custom_f4_map->read_custom_f4_definition(
      EXPORTING iv_tablename             = mr_ui_multi_select_field->tabname
                iv_fieldname             = mr_ui_multi_select_field->fieldname
                iv_rollname              = mr_ui_multi_select_field->rollname
                is_built_in_type         = VALUE #( datatype = mr_ui_multi_select_field->datatype
    ...
    zcl_dbbr_custom_f4_helper=>call_custom_f4( EXPORTING if_multiple_select = abap_true
                                                         it_f4_definition   = lt_f4_definition
                                                         iv_current_line    = iv_current_line
                                                         if_low             = if_low
                                               CHANGING  cs_selfield        = mr_ui_multi_select_field->*
...
*&---------------------------------------------------------------------*
    ...
    IF mr_ui_multi_select_field->has_cust_f4_help = abap_true.
      call_selfield_f4_custom_multi( iv_current_line = lv_current_line
                                     if_low          = if_low ).
    ELSE.
  ...
  METHOD constructor.
    ...
    mr_ui_multi_select_field = CAST zdbbr_selfield( lr_data_cache->get_data_ref(
                                                        zif_dbbr_main_report_var_ids=>c_s_multi_select ) ).
    mr_ui_multi_select_fields = CAST zdbbr_selfield_itab( lr_data_cache->get_data_ref(
...
*&---------------------------------------------------------------------*
    ...
    LOOP AT lt_clipboard_data ASSIGNING FIELD-SYMBOL(<ls_clipboard_line>).
      CLEAR: mr_ui_multi_select_field->low,
             mr_ui_multi_select_field->high.
      ...
      " convert input
      zcl_sat_data_converter=>convert_selopt_to_int_format( EXPORTING iv_tabname   = mr_ui_multi_select_field->tabname
                                                                      iv_fieldname = mr_ui_multi_select_field->fieldname
...
*&---------------------------------------------------------------------*
  ...
  METHOD transfer_data.
    ...
    " delete old values
    DELETE ct_selfield_multi WHERE     fieldname = cs_selfield-fieldname
                                   AND tabname   = cs_selfield-tabname.
  ...
  METHOD zif_uitb_screen_controller~call_screen.
    DATA(ls_screen) = COND zif_uitb_screen_controller=>ty_s_screen_size(
        WHEN is_screen_size IS NOT INITIAL
        THEN is_screen_size
        ELSE VALUE #( top     = 2
                      left    = 10
                      columns = 70
  ...
  METHOD zif_uitb_screen_controller~handle_user_command.
    ...
    CASE lv_function_code.
      ...
      WHEN 'MULTI_F4'.
        ...
        DATA lv_field TYPE c LENGTH 60.
        ...
        ELSE.
          lf_low = abap_true.
        ENDIF.
        call_f4_help( if_for_low = lf_low
                      if_multi   = abap_true ).
  ...
  METHOD zif_uitb_screen_controller~pbo.
    ...
    LOOP AT SCREEN INTO DATA(ls_screen).
      IF ls_screen-name = 'OPTION_TEMPLATE'.
        mr_ui_option_templ_button->* = zcl_dbbr_selopt_util=>create_option_icon( iv_option = ms_option_template-option
