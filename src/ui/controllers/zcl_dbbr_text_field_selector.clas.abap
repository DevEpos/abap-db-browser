"! <p class="shorttext synchronized">Selection dialog for choosing text fields for columns</p>
CLASS zcl_dbbr_text_field_selector DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "!
    "! @parameter io_tabfields | <p class="shorttext synchronized"></p>
    "! @parameter it_fieldcat  | <p class="shorttext synchronized"></p>
    METHODS constructor
      IMPORTING
        io_tabfields TYPE REF TO zcl_dbbr_tabfield_list
        it_fieldcat  TYPE lvc_t_fcat.

    "! <p class="shorttext synchronized">Returns selected/deselected text fields of the dialog</p>
    "!
    "! @parameter rt_text_fields | <p class="shorttext synchronized">Text fields with visibility status</p>
...
CLASS zcl_dbbr_text_field_selector IMPLEMENTATION.
  ...
  METHOD constructor.
    super->constructor( iv_title          = |{ 'Available Text Fields'(001) }|
  ...
  METHOD get_text_fields.
    ...
    LOOP AT lr_t_fields->* ASSIGNING FIELD-SYMBOL(<ls_text_field>) WHERE is_text_field = abap_true.
      ...
      DATA(lt_addtexts) = zcl_dbbr_addtext_bl=>get_instance( )->get_text_fields(
                              iv_tablename = <ls_text_field>-tabname
    ...
    LOOP AT mt_text_fields ASSIGNING FIELD-SYMBOL(<ls_field>).
      ...
      lr_t_fields->*[ KEY alv_fieldname
                      alv_fieldname = <ls_field>-alv_fieldname ]-output_active = <ls_field>-mark.
      rt_text_fields = VALUE #( BASE rt_text_fields
                                ( alv_fieldname = <ls_field>-alv_fieldname visible = <ls_field>-mark ) ).
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD matches_filter.
    ...
    IF    <ls_text_field>-alv_fieldname CS iv_filter
       OR <ls_text_field>-description   CS iv_filter
       OR <ls_text_field>-tabname       CS iv_filter
       OR <ls_text_field>-fieldname     CS iv_filter.
