"! <p class="shorttext synchronized">Controller for defining sorting fields</p>
CLASS zcl_dbbr_field_sorter_ctrl DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Retrieves the updated table fields</p>
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      ...
      END OF mty_sorted.
    TYPES mtt_sorted TYPE STANDARD TABLE OF mty_sorted.
    TYPES:
    ...
    TYPES mtt_available TYPE STANDARD TABLE OF mty_available.
...
CLASS zcl_dbbr_field_sorter_ctrl IMPLEMENTATION.
  METHOD constructor.
    ...
    WHILE mr_fields->has_more_lines( ).
      ...
      DATA(lv_fullfieldname) = COND #( WHEN lr_current_entry->alias IS NOT INITIAL
                                       THEN |{ lr_current_entry->alias }~{ lr_current_entry->fieldname }|
                                       ELSE lr_current_entry->fieldname_raw ).
      IF lr_current_entry->sort_active = abap_true.
        APPEND VALUE mty_sorted(
            ...
            sort_ascending  = COND #( WHEN lr_current_entry->sort_direction = zif_dbbr_c_global=>c_sort_direction-ascending
                                      THEN zif_dbbr_c_icon=>radio_button
                                      ELSE zif_dbbr_c_icon=>radio_button_empty )
            sort_descending = COND #( WHEN lr_current_entry->sort_direction = zif_dbbr_c_global=>c_sort_direction-descending
                                      THEN zif_dbbr_c_icon=>radio_button
                                      ELSE zif_dbbr_c_icon=>radio_button_empty )
            drop_handled    = abap_true )
               TO mt_sorted_fields.
      ELSE.
        APPEND VALUE mty_available( fullfieldname = lv_fullfieldname
                                    fieldname     = lr_current_entry->fieldname
  ...
  METHOD create_available_fields_alv.
    ...
    DATA(ls_layout) = VALUE lvc_s_layo( grid_title = 'Available Columns'
                                        sel_mode   = 'C'
                                        sgl_clk_hd = abap_true ).

    lt_fieldcat = VALUE lvc_t_fcat( outputlen = 25
    ...
    SET HANDLER on_available_alv_toolbar FOR mr_available_fields_alv.
    SET HANDLER on_available_drop_complete FOR mr_available_fields_alv.
    SET HANDLER on_available_double_click FOR mr_available_fields_alv.

    mr_available_fields_alv->set_table_for_first_display( EXPORTING is_layout       = ls_layout
                                                          CHANGING  it_outtab       = mt_available_fields
  ...
  METHOD create_controls.
    ...
    IF mr_dnd_double_listbox IS INITIAL.
      mr_dnd_double_listbox = NEW cl_alv_dd_double_listbox( i_grid2 = mr_available_fields_alv
  ...
  METHOD create_sorted_alv.
    ...
    SET HANDLER on_sorted_double_click FOR mr_sorted_alv.

    DATA(ls_layout) = VALUE lvc_s_layo( sel_mode = 'C' ).

    lt_fieldcat = VALUE lvc_t_fcat( ( fieldname = 'FULLFIELDNAME'
                                      outputlen = 20
                                      rollname  = 'FIELDNAME'
                                      reptext   = 'Column Name'
                                      coltext   = 'Column Name'
    ...
    mr_sorted_alv->set_table_for_first_display( EXPORTING is_layout       = ls_layout
                                                CHANGING  it_outtab       = mt_sorted_fields
                                                          it_fieldcatalog = lt_fieldcat ).
  ENDMETHOD.
  ...
  METHOD on_available_double_click.
    mr_dnd_double_listbox->movetogrid1( i_ok_code = space
                                        is_stable = VALUE #( row = abap_true
                                                             col = abap_true ) ).
  ENDMETHOD.

  METHOD on_available_drop_complete.
    """ get selected lines of `available fields` alv
  ...
  METHOD on_sorted_double_click.
    mr_dnd_double_listbox->movetogrid2( i_ok_code = space
                                        is_stable = VALUE #( row = abap_true
                                                             col = abap_true ) ).
  ENDMETHOD.
  ...
  METHOD search.
    ...
    cl_gui_control=>get_focus( IMPORTING  control           = DATA(lr_focused_control)
  ...
  METHOD zif_uitb_screen_controller~handle_user_command.
    ...
    IF lv_function <> cl_gui_cfw=>rc_noevent.
      " a control event occurred => exit PAI
      RETURN.
    ENDIF.
    ...
    CASE cv_function_code.
      ...
      WHEN 'MOVE_LEFT'.
        mr_dnd_double_listbox->movetogrid1( i_ok_code = cv_function_code ).

      WHEN 'MOVE_RIGHT'.
        mr_dnd_double_listbox->movetogrid2( i_ok_code = cv_function_code ).
