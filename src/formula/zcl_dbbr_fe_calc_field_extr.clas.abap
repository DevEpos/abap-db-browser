"! <p class="shorttext synchronized">Extracts calculation field out formula</p>
...
CLASS zcl_dbbr_fe_calc_field_extr IMPLEMENTATION.
  METHOD zif_dbbr_fe_field_extractor~extract_field.
    ...
    IF contains( val = lv_type_string
                 sub = '-' ).
      DATA(lv_type_ref_tab)   = substring_before( val = lv_type_string
                                                  sub = '-' ).
      DATA(lv_type_ref_field) = substring_after( val = lv_type_string
                                                 sub = '-' ).
    ENDIF.
