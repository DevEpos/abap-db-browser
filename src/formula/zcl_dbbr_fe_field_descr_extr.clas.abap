"! <p class="shorttext synchronized">Extracts description of formula field</p>
  ...
  METHOD zif_dbbr_fe_field_extractor~extract_field.
    ...
    rs_field = VALUE zif_dbbr_fe_types=>ty_form_field(
                         ...
                         short_description = replace( val   = <lt_tokens>[ 3 ]-str
                                                      regex = |'(.*)'|
                                                      with  = '$1' )
                         long_description  = COND #( WHEN is_statement-token_count > 3
                                                     THEN replace( val   = <lt_tokens>[ 4 ]-str
                                                                   regex = |'(.*)'|
                                                                   with  = '$1' ) ) ).
  ENDMETHOD.
ENDCLASS.
