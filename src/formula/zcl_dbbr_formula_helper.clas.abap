"! <p class="shorttext synchronized">Helper for formulas</p>
...
CLASS zcl_dbbr_formula_helper IMPLEMENTATION.
  METHOD add_form_field_to_tablist.
    " TODO: parameter IF_USE_DEFAULT_CTRL_VALUES is never used (ABAP cleaner)
    ...
    ls_tabfield-field_ddtext     = COND #( WHEN is_form_field-long_description IS NOT INITIAL
                                           THEN is_form_field-long_description
                                           ELSE is_form_field-short_description ).
    ...
    IF is_form_field-unit_field IS NOT INITIAL.
      TRY.
          ...
          DATA(lr_s_unit_field) = COND #(
            WHEN ir_tabfield_list->has_multiple_tables( )
            THEN ir_tabfield_list->get_field_ref_by_alv_name( iv_alv_fieldname = CONV #( lv_unit_field ) )
            ELSE ir_tabfield_list->get_field_ref( iv_fieldname = CONV #( lv_unit_field )  ) ).
  ...
  METHOD determine_calculation_fields.
    ...
    LOOP AT lt_stmnt ASSIGNING FIELD-SYMBOL(<ls_stmnt>) WHERE type <> 'U'.
      LOOP AT <ls_stmnt>-tokens ASSIGNING FIELD-SYMBOL(<ls_token>) WHERE is_row_field = abap_true.
        ...
        DATA(lv_row_field_raw) = substring_after( val = <ls_token>-str
                                                  sub = '-' ).

        TRY.
            " check if there is another hyphen because than it is a join field
            IF matches( val   = lv_row_field_raw
                        regex = |\\w-.*| ).
              DATA(lv_alias) = substring_before( val = lv_row_field_raw
                                                 sub = '-' ).
              DATA(lv_field) = substring_after( val = lv_row_field_raw
                                                sub = '-' ).
          ...
          CATCH cx_sy_itab_line_not_found.
            RAISE EXCEPTION TYPE zcx_dbbr_formula_exception
              EXPORTING textid = zcx_dbbr_formula_exception=>needed_calc_fld_not_in_list
  ...
  METHOD get_raw_row_field.
    result = replace( val  = substring_after( val = iv_row_field
                                              sub = 'ROW-' )
                      sub  = '-'
                      with = '_' ).
  ENDMETHOD.
  ...
  METHOD is_icon_field.
    result = xsdbool(     ir_tabfield->is_formula_field = abap_true
                      AND (    ir_tabfield->rollname = zif_dbbr_c_fe_global=>c_icon_type
                            OR ir_tabfield->rollname = zif_dbbr_c_fe_global=>c_icon_tt_type ) ).
  ENDMETHOD.
  ...
  METHOD update_tabflds_from_formula.
    ...
    determine_calculation_fields( ir_formula   = ir_formula
    ...
    LOOP AT lt_form_fields ASSIGNING FIELD-SYMBOL(<ls_form_field>).
      ...
      IF lf_use_default_ctrl_values = abap_true.
        IF lf_set_output_order = abap_true.
          ls_tabfield_control-output_order = ls_tabfield_control-output_order + 1.
        ENDIF.
      ELSE.
        ls_tabfield_control = CORRESPONDING #(
          VALUE #( it_form_selfields[ is_formula_field = abap_true
                                      tabname          = zif_dbbr_c_global=>c_formula_dummy_table
                                      fieldname        = <ls_form_field>-field ] OPTIONAL ) ).
      ENDIF.

      " ... update formula fields
      zcl_dbbr_formula_helper=>add_form_field_to_tablist( is_form_field              = <ls_form_field>
    ...
    IF sy-subrc = 0.
      ir_tabfields->add_table( VALUE zdbbr_entity_info( active_selection     = abap_false
                                                        tabname              = zif_dbbr_c_global=>c_formula_dummy_table
